...
void channelNotice(iClient* srcClient, Channel* theChan, const string& message) {
  for (Channel::userIterator userItr = theChan->userList_begin();
    ...
    if (servicesClient != 0 && servicesClient->isOnChannel(theChan) &&
      ...
      servicesClient->OnChannelNotice(srcClient, theChan, message);
      continue;
    }
    ...
    ownerClient->OnFakeChannelNotice(srcClient, targetClient, theChan, message);
  } // for()
}
...
bool msg_O::Execute(const xParameters& Param) {
  if (Param.size() < 3) {
    elog << "msg_O> Invalid number of arguments" << endl;
    return false;
  }
  ...
  if ('#' == Param[1][0]) {
    ...
    if (0 == theChan) {
      elog << "msg_O> Unable to locate channel: " << Param[1] << endl;
      return false;
    }
  }
  ...
  if (0 == srcClient) {
    elog << "msg_O> Unable to find source client: " << Param[0] << endl;
    return false;
  }
  ...
  if ((0 == theChan) && (strchr(Param[1], '@') != 0)) {
    ...
    if (0 == targetClient) {
      ...
      if (0 == fakeTarget) {
        elog << "msg_O> Unable to find local client: " << Param[1] << endl;
        return true;
      }
    }
  } else if (0 == theChan) {
    // TODO
    elog << "msg_O> Unknown target: " << Param[1] << endl;
  ...
  if (fakeTarget != 0) {
    ...
    if (0 == ownerClient) {
      elog << "msg_O> Fake client without owner: " << *fakeTarget << endl;
      return true;
    }
  }
  ...
  if (theChan != 0) {
    ...
    if (fakeTarget != 0) {
      ownerClient->OnFakePrivateNotice(srcClient, fakeTarget, message, secure);
    } else {
      targetClient->OnPrivateNotice(srcClient, message, secure);
    } // else()
  } // else( theChan != 0 )
