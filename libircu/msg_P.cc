...
void channelCTCP(iClient* srcClient, Channel* theChan, const string& command,
  ...
  for (Channel::userIterator userItr = theChan->userList_begin();
    ...
    if (servicesClient != 0 && servicesClient->isOnChannel(theChan) &&
        !servicesClient->getMode(iClient::MODE_DEAF)) {
      // xClient, invoke OnChannelMessage()
      servicesClient->OnChannelCTCP(srcClient, theChan, command, message);
      continue;
    }

    // Not an xClient, check if it's a fake client
    iClient* targetClient = Network->findFakeClient(userItr->second->getClient());
    if (0 == targetClient) {
      // Nope
      continue;
    }
    ...
    if (0 == ownerClient) {
      elog << "msg_P::channelMessage> Unable to "
           << "find owner of client: " << *targetClient << ", in channel: " << *theChan << endl;
      continue;
    }

    ownerClient->OnFakeChannelCTCP(srcClient, targetClient, theChan, command, message);
  } // for()
}

void channelMessage(iClient* srcClient, Channel* theChan, const string& message) {
  for (Channel::userIterator userItr = theChan->userList_begin();
       userItr != theChan->userList_end(); ++userItr) {
    unsigned int intYYXXX = userItr->second->getIntYYXXX();
    ...
    if (servicesClient != 0 && servicesClient->isOnChannel(theChan) &&
        !servicesClient->getMode(iClient::MODE_DEAF)) {
      // xClient, invoke OnChannelMessage()
      servicesClient->OnChannelMessage(srcClient, theChan, message);
      continue;
    }
    ...
    ownerClient->OnFakeChannelMessage(srcClient, targetClient, theChan, message);
  } // for()
}
...
bool msg_P::Execute(const xParameters& Param) {
  ...
  if ((0 == theChan) && (strchr(Param[1], '@') != 0)) {
    ...
    if (0 == targetClient) {
      ...
      if (0 == fakeTarget) {
        ...
        //				<< st[ 0 ]
        //				<< endl ;
        bool controlNick = theServer->findControlNick(st[0]);
        if (controlNick) {
          theServer->ControlCommand(srcClient, Param[2]);
          // All done
          return true;
        }
  ...
  } else if ((0 == theChan) && (Param[1][0] == theServer->getCharYY()[0]) &&
  ...
  if (CTCP) {
    if (theChan != 0) {
      ...
      if (fakeTarget != 0) {
        ownerClient->OnFakeCTCP(srcClient, fakeTarget, command, message, secure);
      } else {
        targetClient->OnCTCP(srcClient, command, message, secure);
      }
    }
  } else {
    if (theChan != 0) {
      ...
      if (fakeTarget != 0) {
        ownerClient->OnFakePrivateMessage(srcClient, fakeTarget, message, secure);
      } else {
        targetClient->OnPrivateMessage(srcClient, message, secure);
      } // else()
    } // else( theChan != 0 )
  } // else( CTCP )
