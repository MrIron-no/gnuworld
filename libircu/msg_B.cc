...
void msg_B::parseBurstUsers(Channel* theChan, const string& theUsers) {
  ...
  string chanName = theChan->getName(); // Added for fixing crash when Channel gets destroyed in the
                                        // result of PostChannelEvent() below
  ...
  for (StringTokenizer::const_iterator ptr = st.begin(); ptr != st.end(); ++ptr) {
    ...
    // Has to be done to prevent a crash for cases where PostChannelEvent() results in kicking the
    // burst user right in the middle of this function. If the user was alone in the channel, the
    // channel is destroyed and referenced later, causing a segmentation fault.
