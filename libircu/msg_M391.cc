 ...
 * USA.
 *
 * $Id: msg_M391.cc,v 1.1 2009/07/25 18:12:34 hidden1 Exp $
 */
...
bool msg_M391::Execute(const xParameters& Param) {
  ...
  if (NULL == theClient) {
    elog << "msg_M391> Unable to find nick: " << Param[1] << endl;
    return false;
  }

  iServer* tmpServer = Network->findServer(Param[0]);
  if (NULL == tmpServer) {
    elog << "msg_M391> Unable to find server: " << Param[0] << endl;
    return false;
  }

  string tMessage = string(tmpServer->getCharYY()) + " 391 ";
