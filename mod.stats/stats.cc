...
void stats::dumpStats(iClient* theClient) {
  ...
  if (!usersPerChannelFile) {
    cout << "stats::dumpStats> Failed to open channel info file: " << channelInfoFileName << endl;
  }
  ...
  if (!channelsPerUserFile) {
    cout << "stats::dumpStats> Unable to open user info file: " << userInfoFileName.c_str() << endl;
  }
