...
private:
   ...
   */
  typedef std::map<unsigned int, std::set<unsigned int>> reservedNumericMapType;

  /**
   * This is an iterator type definition for the
   * reservedNumericMapType.
   */
  typedef reservedNumericMapType::iterator reservedNumeric_iterator;

  /**
   * This type maps fake iClient's to their owning
   * xClient's, keyed by the iClient's intYYXXX numeric.
   */
  typedef std::map<unsigned int, std::pair<iClient*, xClient*>> fakeClientMapType;

  /**
   * This type stores fake (juped) servers.
   */
  typedef std::map<unsigned int, std::pair<iServer*, xClient*>> fakeServerMapType;
...
public:
   ...
   */
  virtual bool addFakeClient(iClient* theClient, xClient* ownerClient);
   ...
   */
  virtual void addNetConf(const std::string& key, const std::string& value, time_t timestamp) {
   ...
   */
  virtual iClient* findClient(const unsigned int& YY, const unsigned int& XXX) const;

  /**
   * Find a remote client given the client's integer numeric.
   * Returns NULL if not found.
   */
  virtual iClient* findClient(const unsigned int& intYYXXX) const;

  /**
   * Find a fake client given the client's integer numeric.
   * Returns NULL if not found.
   */
  virtual iClient* findFakeClient(const unsigned int& YY, const unsigned int& XXX) const;

  /**
   * Find a remote client given the client's character numeric.
   * Returns NULL if not found.
   */
  virtual iClient* findClient(const std::string& yyxxx) const;

  /**
   * Find a fake client by numeric.
   * Returns NULL if not found.
   */
  virtual iClient* findFakeClient(const std::string& yyxxx) const;
   ...
   * Returns NULL if not found.
   */
  virtual iClient* findNick(const std::string& nickName) const;

  /**
   * Find a local (services) client by its character numeric.
   * Returns NULL if not found.
   */
  virtual xClient* findLocalClient(const std::string& yyxxx) const;

  /**
   * Find a local (services) client by its integer numeric.
   * Returns NULL if not found.
   */
  virtual xClient* findLocalClient(const unsigned int& intYYXXX) const;

  /**
   * Find a local (services) client by its case-insensitive nickname.
   * Returns NULL if not found.
   */
  virtual xClient* findLocalNick(const std::string& nickName) const;

  /**
   * Lookup a fake (juped) iClient by nickname.
   */
  virtual iClient* findFakeNick(const std::string& nickName) const;
   ...
   * Returns NULL if not found.
   */
  virtual iServer* findServerName(const std::string& name) const;

  /**
   * Find a fake server by (case insensitive) name.
   */
  virtual iServer* findFakeServerName(const std::string&) const;

  /**
   * Find a remote server by a wildmask name.
   * Returns NULL if not found.
   */
  virtual iServer* findExpandedServerName(const std::string& name) const;
  /**
   ...
   */
  virtual iClient* removeClient(const unsigned int& intYYXXX);
   ...
   * Returns NULL if not found.
   */
  virtual iClient* removeClient(const unsigned int& YY, const unsigned int& XXX);
   ...
   * Returns NULL if not found.
   */
  virtual iServer* removeServer(const unsigned int& YY, bool postEvent = false);
   ...
   */
  virtual void rehashNick(const std::string& yyxxx, const std::string& newNick,
                          const time_t& newTS);
   ...
   */
  inline const nickMapType clients() const { return nickMap; }

  /**
   * Return a mutator to the beginning of the network client
   * data structure.  The client referenced is mutable.
   */
  inline clientIterator clients_begin() { return nickMap.begin(); }

  /**
   * Return a mutator to the end of the network client
   * data structure.  The client referenced is mutable.
   */
  inline clientIterator clients_end() { return nickMap.end(); }
   ...
   */
  inline const_clientIterator clients_begin() const { return nickMap.begin(); }

  /**
   * Return a mutator to the end of the network client
   * data structure.  The client referenced is immutable.
   */
  inline const_clientIterator clients_end() const { return nickMap.end(); }
   ...
   */
  inline const serverMapType servers() const { return serverMap; }

  /**
   * Return a non-const iterator to the beginning of the
   * remote servers table.
   */
  inline serverIterator servers_begin() { return serverMap.begin(); }

  /**
   * Return a non-const iterator to the end of the
   * remote servers table.
   */
  inline serverIterator servers_end() { return serverMap.end(); }

  /**
   * Return a const iterator to the beginning of the
   * remote servers table.
   */
  inline const_serverIterator servers_begin() const { return serverMap.begin(); }

  /**
   * Return a const iterator to the end of the remote
   * servers table.
   */
  inline const_serverIterator servers_end() const { return serverMap.end(); }
   ...
   * local clients table.
   */
  inline localClientIterator localClient_begin() { return localClients.begin(); }

  /**
   * Return a non-const iterator to the end of the local
   * clients table.
   */
  inline localClientIterator localClient_end() { return localClients.end(); }

  /**
   * Return a const iterator the beginning of the
   * local clients table.
   */
  inline const_localClientIterator localClient_begin() const { return localClients.begin(); }

  /**
   * Return a const iterator to the end of the local
   * clients table.
   */
  inline const_localClientIterator localClient_end() const { return localClients.end(); }
   ...
   */
  inline const_channelIterator channels_begin() const { return channelMap.begin(); }

  /**
   * Returns an iterator to the end of the channels structure
   */
  inline const_channelIterator channels_end() const { return channelMap.end(); }
   ...
   * Returns an iterator to the begining of the channels structure
   */
  inline channelIterator channels_begin() { return channelMap.begin(); }

  /**
   * Returns an iterator to the end of the channels structure
   */
  inline channelIterator channels_end() { return channelMap.end(); }
   ...
   * client structure.
   */
  inline const_fakeClientIterator fakeClient_begin() const { return fakeClientMap.begin(); }

  /**
   * Return a const iterator the end of the fake
   * client structure.
   */
  inline const_fakeClientIterator fakeClient_end() const { return fakeClientMap.end(); }
   ...
   * client structure.
   */
  inline fakeClientIterator fakeClient_begin() { return fakeClientMap.begin(); }

  /**
   * Return a mutable iterator the end of the fake
   * client structure.
   */
  inline fakeClientIterator fakeClient_end() { return fakeClientMap.end(); }

  /**
   * const_iterator to the fakeServerMap.
   */
  typedef fakeServerMapType::const_iterator const_fakeServerIterator;

  /**
   * Return a const_iterator to the beginning of the fake
   * server structure.
   */
  inline const_fakeServerIterator fakeServers_begin() const { return fakeServerMap.begin(); }

  /**
   * Return a const_iterator to the end of the fake
   * server structure.
   */
  inline const_fakeServerIterator fakeServers_end() const { return fakeServerMap.end(); }
   ...
   * server structure.
   */
  inline fakeServerIterator fakeServers_begin() { return fakeServerMap.begin(); }

  /**
   * Return a mutable iterator to the end of the fake
   * server structure.
   */
  inline fakeServerIterator fakeServers_end() { return fakeServerMap.end(); }

  /**
   * Return the number of channels currently stored in the
   * the channel table.
   */
  inline size_t channelList_size() const { return channelMap.size(); }
   ...
   */
  virtual std::list<const iClient*> matchHost(const std::string& wildHost) const;
   ...
   * list of pointers to const iClient's which match.
   */
  virtual std::list<const iClient*> matchUserHost(const std::string&) const;
   ...
   */
  virtual std::list<const Channel*> getChannelsWithKey(const std::string& key) const;
   ...
   * to const Channel's which match.
   */
  virtual std::list<const Channel*> getChannelsWithModes(const std::string& modes) const;
   ...
   * to const Channel's which match.
   */
  virtual std::list<const Channel*> getChannelsWithTopic(const std::string& topic) const;
   ...
   * to const Channel's which match.
   */
  virtual std::list<const Channel*> getChannelsWithTopicBy(const std::string& topicby) const;
#endif
  /**
   ...
   * found.
   */
  virtual std::list<const iClient*> findHost(const std::string& hostName) const;

  /**
   * Perform a similar match as to matchHost(), except return
   * only the number of matches found.
   */
  virtual size_t countMatchingHost(const std::string& wildHost) const;

  /**
   * Perform a similar match as to matchUserHost(), except return
   * only the number of matches found.
   */
  virtual size_t countMatchingUserHost(const std::string& wildUserHost) const;
  /**
   ...
   * list of pointers to const iClient's which match.
   */
  virtual std::list<const iClient*> matchRealHost(const std::string& wildHost) const;
   ...
   * list of pointers to const iClient's which match.
   */
  virtual std::list<const iClient*> matchRealUserHost(const std::string&) const;
   ...
   * found.
   */
  virtual std::list<const iClient*> findRealHost(const std::string& hostName) const;

  /**
   * Perform a similar match as to matchRealHost(), except return
   * only the number of matches found.
   */
  virtual size_t countMatchingRealHost(const std::string& wildHost) const;

  /**
   * Perform a similar match as to matchRealUserHost(), except return
   * only the number of matches found.
   */
  virtual size_t countMatchingRealUserHost(const std::string& wildUserHost) const;
  /**
   ...
   * list of pointers to const iClient's which match.
   */
  virtual std::list<const iClient*> matchRealName(const std::string&) const;
...
protected:
   ...
   * On success, true is returned, false otherwise.
   */
  virtual bool allocateClientNumeric(unsigned int intYY, unsigned int& newIntXXX);
   ...
   */
  virtual void findLeaves(std::vector<unsigned int>& yyVector, const unsigned int intYY) const;
