 ...
 */
template <typename modType, typename argType = std::string> class moduleLoader {
...
protected:
   ...
   * the error value.
   */
  inline void setError(bool newVal = true) { hasError = newVal; }
...
public:
   ...
   * parameter.
   */
  moduleLoader(const std::string& _moduleName) : moduleName(_moduleName), hasError(false) {
   ...
   */
  modType loadObject(argType arg, const std::string& symbolSuffix = std::string()) {
    if (getError()) {
   ...
   */
  inline const char* getLastError() const { return ::lt_dlerror(); }

  /**
   * Return by value the object being loaded from the module.
   */
  inline modType getObject() const { return modPtr; }

  /**
   * Return true if an error occured, false otherwise.
   */
  inline bool getError() const { return hasError; }
};
