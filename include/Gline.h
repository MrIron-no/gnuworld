...
public:
   ...
   * gline.
   */
  Gline(const std::string& _setBy, const std::string& _userHost, const std::string& _reason,
        const time_t& _duration, const time_t& _lastmod = ::time(0))
      : setBy(_setBy), userHost(_userHost), reason(_reason), expiration(_duration + ::time(0)),
        lastmod(_lastmod) {}
  ...
  Gline(const Gline& rhs)
      : setBy(rhs.setBy), userHost(rhs.userHost), reason(rhs.reason), expiration(rhs.expiration),
        lastmod(rhs.lastmod) {}
   ...
   */
  inline const std::string& getSetBy() const { return setBy; }

  /**
   * Retrieve the user@host which has been glined.
   */
  inline const std::string& getUserHost() const { return userHost; }
   ...
   */
  inline const time_t& getExpiration() const { return expiration; }

  /**
   * Retrieve the time at which the gline was last modified
   */
  inline const time_t& getLastmod() const { return lastmod; }

  /**
   * Retrieve the user/server who set this gline.
   */
  inline void setSetBy(const std::string& _setBy) { setBy = _setBy; }

  /**
   * Set the hostmask affected by this gline.
   */
  inline void setHost(const std::string& _host) { userHost = _host; }

  /**
   * Set the reason for this gline.
   */
  inline void setReason(const std::string& _reason) { reason = _reason; }

  /**
   * Set the expiration time on this gline.
   */
  inline void setExpiration(const time_t _expiration) { expiration = _expiration; }

  /**
   * Set the lastmod time
   */
  inline void setLastmod(const time_t _lastmod) { lastmod = _lastmod; }
  ...
  friend ELog& operator<<(ELog& out, const Gline& rhs) {
    out << "Mask: " << rhs.userHost << ' ' << "Reason: " << rhs.reason << ' '
        << "Set by: " << rhs.setBy << ' ' << "Expires: " << rhs.expiration << ' '
