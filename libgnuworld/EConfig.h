...
template <typename T>
concept ConfigType =
    std::is_same_v<T, std::string> || std::is_same_v<T, int> || std::is_same_v<T, unsigned int> ||
...
class EConfig {
   ...
   */
  typedef std::multimap<std::string, std::string, noCaseCompare> mapType;
  ...
  struct lineInfo {
     ...
     */
    lineInfo(const std::string& key) : key(key) {}

    /**
     * Copy constructor, used in assignment (as in the key/value map).
     */
    lineInfo(const lineInfo& rhs) : key(rhs.key), value(rhs.value), mapItr(rhs.mapItr) {}

    /**
     * Constructor receives the key, value, and map iterator.
     */
    lineInfo(const std::string& key, const std::string& value, const mapType::iterator& mapItr)
...
public:
   ...
   * of key/value pairs.
   */
  inline const_iterator begin() const { return valueMap.begin(); }

  /**
   * Obtain a const iterator to the end of the map of
   * key/value pairs.
   */
  inline const_iterator end() const { return valueMap.end(); }

  /**
   * Obtain a iterator to the beginning of the map
   * of key/value pairs.
   */
  inline iterator begin() { return valueMap.begin(); }

  /**
   * Obtain a iterator to the end of the map of
   * key/value pairs.
   */
  inline iterator end() { return valueMap.end(); }

  /**
   * Return the number of significant key/value pairs stored.
   */
  inline size_type size() const { return valueMap.size(); }
   ...
   */
  inline const std::vector<std::string>& getErrors() const { return configErrors; }
   ...
   */
  template <ConfigType T> T Require(const std::string& key, bool fatal = true) {
    ...
    try {
      ...
      msg << "Invalid value for key \"" << key << "\": \"" << value << "\" - " << e.what();
   ...
   */
  template <ConfigType T> T TryRequire(const std::string& key, const T& fallback) {
    try {
   ...
   */
  bool Replace(iterator itr, const std::string& newValue);
