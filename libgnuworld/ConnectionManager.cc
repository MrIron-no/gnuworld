...
ConnectionManager::ConnectionManager(const time_t defaultTimeoutDuration,
    ...
    : timeoutDuration(defaultTimeoutDuration), delimiter(defaultDelimiter),
...
ConnectionManager::~ConnectionManager() {
  ...
  // Connections
  for (handlerMapIterator handlerItr = handlerMap.begin(); handlerItr != handlerMap.end();
       ++handlerItr) {
    for (connectionMapIterator connectionItr = handlerItr->second.begin();
         connectionItr != handlerItr->second.end(); ++connectionItr) {
...
Connection* ConnectionManager::Connect(ConnectionHandler* hPtr, const string& host,
  ...
  Connection* newConnection = new (nothrow) Connection(host, remotePort, delimiter, tlsEnabled);
  ...
  if (::connect(sockFD, reinterpret_cast<sockaddr*>(addr), sizeof(struct sockaddr_in)) < 0) {
  ...
  if (newConnection != 0) {
    ...
    socklen_t sockAddrLen = static_cast<socklen_t>(sizeof(struct sockaddr));
...
bool ConnectionManager::DisconnectByHost(ConnectionHandler* hPtr, const string& hostname,
                                         const unsigned short int remotePort,
...
bool ConnectionManager::DisconnectByIP(ConnectionHandler* hPtr, const string& IP,
                                       const unsigned short int remotePort,
                                       const unsigned short int localPort) {
...
bool ConnectionManager::disconnectAll(ConnectionHandler* hPtr) {
  ...
  for (connectionMapIterator connItr = hmItr->second.begin(); connItr != hmItr->second.end();
       ++connItr) {
...
bool ConnectionManager::Disconnect(ConnectionHandler* hPtr, Connection* cPtr) {
...
void ConnectionManager::Poll(const long seconds, const long milliseconds) {
  // elog	<< "Poll()" << endl ;
  ...
  // FD information
  for (constHandlerMapIterator handlerItr = handlerMap.begin(), handlerEndItr = handlerMap.end();
       handlerItr != handlerEndItr; ++handlerItr) {
    for (constConnectionMapIterator connectionItr = handlerItr->second.begin(),
                                    connectionEndItr = handlerItr->second.end();
         connectionItr != connectionEndItr; ++connectionItr) {
  ...
  // Call select()
  selectRet = ::select(1 + highestFD, &readfds, &writefds, 0, (-1 == seconds) ? NULL : &to);
    ...
    for (connectionMapIterator connectionItr = handlerItr->second.begin(),
      ...
      if (connectionPtr->isConnected()) {
        ...
        if (FD_ISSET(tempFD, &readfds)) {
          ...
          // for read
          connectOK = handleRead(hPtr, connectionPtr);
        } // if( FD_ISSET( read ) )
        ...
        if (connectOK && FD_ISSET(tempFD, &writefds)) {
          connectOK = handleWrite(hPtr, connectionPtr);
        }
      } // if( connectionPtr->isConnected() )
      ...
      // Next let's check if this is a listening (server) socket
        ...
        if (FD_ISSET(tempFD, &readfds)) {
          ...
          connectOK = finishAccept(hPtr, connectionPtr);
        }
      }
      ...
      // Check for pending outgoing connection
        ...
        if (FD_ISSET(tempFD, &writefds)) {
          ...
          connectOK = finishConnect(hPtr, connectionPtr);
        } // if( FD_ISSET() )
        else {
  ...
  //
  for (eraseMapIterator eraseItr = eraseMap.begin(), eraseEndItr = eraseMap.end();
    ...
    //
    handlerMapIterator handlerItr = handlerMap.find(eraseItr->first);
    ...
    //
    for (; (eraseItr != eraseEndItr) && (eraseItr->first == handlerItr->first); ++eraseItr) {
...
bool ConnectionManager::isIpAddress(const string& host) {
  ...
  for (string::size_type i = 0; (i < host.size()) && (decimalPoints < 4); ++i) {
...
void ConnectionManager::closeSocket(int fd) { ::close(fd); }

// Caller handles erasing/closing upon a failed call to handleRead().
bool ConnectionManager::handleRead(ConnectionHandler* hPtr, Connection* cPtr) {
  ...
  if (cPtr->isFile()) {
    // Connected to file
    readResult = ::read(cPtr->getSockFD(), inputBuffer, inputBufferSize);
  } else {
    // Network connection
    if (cPtr->isTLS()) {
#ifdef HAVE_LIBSSL
      ...
      readResult = ::recv(cPtr->getSockFD(), inputBuffer, inputBufferSize, 0);
    }
  }
...
bool ConnectionManager::handleWrite(ConnectionHandler* hPtr, Connection* cPtr) {
  // Don't allow reads if the connection is shutting down.
  if (cPtr->isTLS() && cPtr->isShuttingDownTLS())
    return true;
  ...
  if (!cPtr->isTLS()) {
    writeResult =
        ::send(cPtr->getSockFD(), cPtr->outputBuffer.data(), cPtr->outputBuffer.size(), 0);
  } else {
#ifdef HAVE_LIBSSL
    writeResult =
        SSL_write(cPtr->getTlsState(), cPtr->outputBuffer.data(), cPtr->outputBuffer.size());
    if (writeResult < 0) {
  ...
  if ((ENOBUFS == errno) || (EWOULDBLOCK == errno) || (EAGAIN == errno)) {
    // Nonblocking type error
    // Ignore it for now
    elog << "ConnectionManager::handleWrite> errno: (" << errno << ") " << strerror(errno) << endl;
    return true;
  }
...
// Caller handles erasing/closing upon a failed call to handleWrite().
bool ConnectionManager::handleFlush(ConnectionHandler* hPtr, Connection* cPtr) {
  // protected member, no error checking
...
bool ConnectionManager::finishConnect(ConnectionHandler* hPtr, Connection* cPtr) {
  ...
  // Attempt to connect()
  int connectResult = ::connect(cPtr->getSockFD(), reinterpret_cast<sockaddr*>(cPtr->getAddr()),
...
bool ConnectionManager::finishAccept(ConnectionHandler* hPtr, Connection* cPtr) {
  ...
  int newFD =
      ::accept(cPtr->getSockFD(), reinterpret_cast<sockaddr*>(newConnection->getAddr()), &len);
  ...
  // Update remote port number for this Connection
  newConnection->setRemotePort(ntohs(newConnection->getAddr()->sin_port));
...
bool ConnectionManager::setSocketOptions(int sockFD) {
  ...
  optval = ::setsockopt(sockFD, SOL_SOCKET, SO_KEEPALIVE, reinterpret_cast<const char*>(&optval),
                        sizeof(optval));
  if (optval < 0) {
    return false;
  }
  ...
  optval = ::setsockopt(sockFD, SOL_SOCKET, SO_LINGER, reinterpret_cast<const char*>(&setLinger),
                        sizeof(setLinger));
  if (optval < 0) {
    return false;
  }
...
Connection* ConnectionManager::Listen(ConnectionHandler* hPtr, const unsigned short int localPort) {
...
void ConnectionManager::scheduleErasure(ConnectionHandler* hPtr,
  ...
  for (; (eraseItr != eraseMap.end()) && (eraseItr->first == hPtr); ++eraseItr) {
 ...
 */
Connection* ConnectionManager::ConnectToFile(ConnectionHandler* hPtr, const string& fileName) {
  ...
  if (fd < 0) {
    ...
    elog << "ConnectToFile> Unable to open file " << fileName << ": " << strerror(errno) << endl;
    return 0;
  }

  // Create a new Connection object to represent this open file
  Connection* newConnect = new (std::nothrow) Connection(fileName, fd, delimiter, false);
  assert(newConnect != 0);
