...
public:
   ...
   */
  inline const std::string& operator[](const size_type& sub) const { return getToken(sub); }
   ...
   */
  inline const std::string getOriginal() const { return assemble(); }
   ...
   */
  inline bool validSubscript(const size_type& sub) const { return (sub < size()); }
   ...
   */
  inline const_reverse_iterator rbegin() const { return array.rbegin(); }

  /**
   * Retrieve an immutable reverse iterator to the reverse
   * end of this object's token structure.
   */
  inline const_reverse_iterator rend() const { return array.rend(); }
