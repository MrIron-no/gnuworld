...
public:
   ...
   */
  inline unsigned short int getLocalPort() const { return localPort; }

  /**
   * Return the remote port number for this Connection.
   */
  inline unsigned short int getRemotePort() const { return remotePort; }
   ...
   */
  inline bool isIncoming() const { return (flags & F_INCOMING); }
   ...
   * connection.
   */
  inline bool isListening() const { return (flags & F_LISTEN); }

  /**
   * Return true if this Connection is a file connection, false
   * otherwise.
   */
  inline bool isFile() const { return (flags & F_FILE); }
   ...
   * if necessary.
   */
  inline bool isFlush() const { return (flags & F_FLUSH); }

  /**
   * Return true if the negotiating TLS flag is set, indicating that
   * the TLS handshake has not completed.
   */
  inline bool isNegotiatingTLS() const { return (flags & F_TLS_HANDSHAKING); }

  /**
   * Return true if the TLS connection has initiating shutdown.
   */
  inline bool isShuttingDownTLS() const { return (flags & F_TLS_SHUTTING_DOWN); }

  /**
   * Return the total number of bytes read from this
   * Connection.
   */
  inline size_t getBytesRead() const { return bytesRead; }
   ...
   */
  inline size_t getBytesWritten() const { return bytesWritten; }

  /**
   * Return the time at which this Connection object established
   * connection.  This value is 0 for listening sockets.
   */
  inline time_t getConnectTime() const { return connectTime; }

  /**
   * Return the number of bytes currently in the output
   * buffer.
   */
  inline size_t getOutputBufferSize() const { return outputBuffer.size(); }

  /**
   * Return the number of bytes currently in the input
   * buffer.
   */
  inline size_t getInputBufferSize() const { return inputBuffer.size(); }

  /**
   * Return whether or not this connection has TLS enabled
   */
  inline bool isTLS() const { return tlsEnabled; }

  /**
   * Set the F_FLUSH flag to true.
   */
  inline void Flush() { setFlag(F_FLUSH); }
  ...
  friend std::ostream& operator<<(std::ostream& out, const Connection& con) {
    out << "Host: " << con.getHostname() << ", IP: " << con.getIP()
        << ", localPort: " << con.getLocalPort() << ", remotePort: " << con.getRemotePort()
        << ", sockFD: " << con.getSockFD() << ", TLS: " << (con.isTLS() ? "yes" : "no")
        << ", state: " << (con.isConnected() ? "connected" : "pending");
    if (con.isIncoming()) {
      out << ",incoming";
    }
    if (con.isListening()) {
...
protected:
   ...
   */
  Connection(const std::string& host, const unsigned short int remotePort, const char delimiter,
             bool tlsEnabled);
   ...
   */
  inline void setIncoming() { setFlag(F_INCOMING); }

  /**
   * Mark that this Connection is a listening for connections
   */
  inline void setListen() { setFlag(F_LISTEN); }

  /**
   * Mark that this Connection represents a file
   */
  inline void setFile() { setFlag(F_FILE); }

  /**
   * Mark that this Connection as pending TLS negotiation.
   */
  inline void setNegotiatingTLS() { setFlag(F_TLS_HANDSHAKING); }

  /**
   * Mark that this Connection as pending TLS negotiation.
   */
  inline void clrNegotiatingTLS() { removeFlag(F_TLS_HANDSHAKING); }

  /**
   * Mark that this Connection as pending TLS negotiation.
   */
  inline void setShuttingDownTLS() { setFlag(F_TLS_SHUTTING_DOWN); }

  /**
   * Set this connection's local port number
   */
  inline void setLocalPort(const unsigned short int newLocalPort) { localPort = newLocalPort; }

  /**
   * Set this connection's remote port number
   */
  inline void setRemotePort(const unsigned short int newRemotePort) { remotePort = newRemotePort; }
   ...
   */
  inline void setIP(const std::string& newIP) { IP = newIP; }

  /**
   * Set the Connection's hostname to newHost
   */
  inline void setHostname(const std::string& newHost) { hostname = newHost; }

  /**
   * Set the Connection's FD to the new FD
   */
  inline void setSockFD(int newSockFD) { sockFD = newSockFD; }

  /**
   * Set the Connection absTimeout to the new absTimeOut
   */
  inline void setAbsTimeout(const time_t newAbsTimeout) { absTimeout = newAbsTimeout; }

  /**
   * Return the time at which this connection attempt will
   * be terminated (its absolute timeout value).
   */
  inline time_t getAbsTimeout() const { return absTimeout; }

#ifdef HAVE_LIBSSL
  /**
   * Return the current TLS state for this connection
   */
  inline SSL* getTlsState() const { return tlsState; }

  inline void setTlsState(SSL* newState) { tlsState = newState; }
#endif
