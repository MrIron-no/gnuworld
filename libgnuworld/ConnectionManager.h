...
class ConnectionManager {
   ...
   */
  typedef std::map<ConnectionHandler*, connectionMapType> handlerMapType;
   ...
   * into the eraseMap.
   */
  typedef std::multimap<ConnectionHandler*, connectionMapType::iterator> eraseMapType;
...
public:
   ...
   * passed to OnRead().
   */
  ConnectionManager(const time_t defaultTimeout = 10, const char defaultDelimiter = '\n');
   ...
   */
  void setTimeoutDuration(const time_t newTimeoutDuration) { timeoutDuration = newTimeoutDuration; }
   ...
   */
  virtual Connection* Connect(ConnectionHandler*, const std::string& host,
                              const unsigned short int remotePort, bool tlsEnabled);
   ...
   */
  virtual Connection* ConnectToFile(ConnectionHandler*, const std::string&);
   ...
   * The ConnectionHandler must be non-NULL.
   */
  virtual Connection* Listen(ConnectionHandler*, const unsigned short int localPort);
   ...
   */
  virtual bool DisconnectByHost(ConnectionHandler*, const std::string& hostname,
   ...
   * which case the remotePort will be ignored.
   */
  virtual bool DisconnectByIP(ConnectionHandler*, const std::string& IP,
                              const unsigned short int remotePort,
                              const unsigned short int localPort = 0);
   ...
   */
  virtual bool Disconnect(ConnectionHandler*, Connection*);
   ...
   */
  virtual void Poll(const long seconds = 0, const long milliseconds = 0);
...
protected:
   ...
   */
  virtual void scheduleErasure(ConnectionHandler*, connectionMapType::iterator);
