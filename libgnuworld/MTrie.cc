...
template <typename _valueT> MTrie<_valueT>::~MTrie() {
  for (nodes_iterator itr = nodesMap.begin(); itr != nodesMap.end(); ++itr) {
    delete itr->second;
  }
  nodesMap.clear();
}

template <typename _valueT> bool MTrie<_valueT>::insert(const string& key, const _valueT& value) {
  ...
  for (StringTokenizer::const_reverse_iterator tokenItr = st.rbegin(); tokenItr != st.rend();
       ++tokenItr) {
    if (currentNode->nodesMap.find(*tokenItr) == currentNode->nodesMap.end()) {
      // This node does not exist
      MTrie<_valueT>* newNode = new (std::nothrow) MTrie<_valueT>;
      assert(newNode != 0);
...
template <typename _valueT>
list<typename MTrie<_valueT>::value_type> MTrie<_valueT>::find(const string& key) const {
...
template <typename _valueT>
void MTrie<_valueT>::find(const MTrie<_valueT>* currentNode, const StringTokenizer& tokens,
  ...
  for (const_nodes_iterator nItr = currentNode->nodesMap.begin(),
    ...
    if (foundQuestionMark) {
      if (!gnuworld::match(localKey, nItr->first)) {
        ...
        find(nItr->second, tokens, tokenItr);
...
template <typename _valueT>
void MTrie<_valueT>::recursiveFind(const MTrie<_valueT>* currentNode, bool blindRecursion) const {
  // blindRecursion:
...
template <typename _valueT> string MTrie<_valueT>::getBase() const {
  string retMe;
  retMe.reserve(128);
  bool doneALoop = false;

  for (list<string>::const_iterator rtItr = base.begin(), rtEndItr = base.end(); rtItr != rtEndItr;
       ++rtItr) {
    if (doneALoop) {
      retMe += delimiter;
    }
...
template <typename _valueT>
typename MTrie<_valueT>::size_type MTrie<_valueT>::erase(const string& key) {
...
template <typename _valueT>
typename MTrie<_valueT>::size_type MTrie<_valueT>::erase(MTrie<_valueT>* currentNode,
                                                         list<string>& base, const string& origKey,
                                                         const string& key) {
  ...
  // Everything is set, begin recursion
  // Match localKey against all nodes
  for (nodes_iterator nItr = currentNode->nodesMap.begin(); nItr != currentNode->nodesMap.end();
       ++nItr) {
    ...
    if (foundQuestionMark) {
      ...
      // Reassemble the key
      string newKey;
      for (++tokenItr; tokenItr != tokens.rend(); ++tokenItr) {
        if (!newKey.empty()) {
          newKey.insert(newKey.begin(), delimiter);
        }
      ...
      eraseCount += erase(nItr->second, base, origKey, newKey);
    } else {
    ...
    if (currentNode->valuesList.empty() && currentNode->nodesMap.empty()) {
...
MTrie<_valueT>::recursiveErase(MTrie<_valueT>* currentNode, list<string>& base, const string& key) {
...
template <typename _valueT> void MTrie<_valueT>::dumpDebug(ostream& out) const {
  recursiveDebug(out, 1, this);
}

template <typename _valueT>
void MTrie<_valueT>::recursiveDebug(ostream& out, size_t levelNum,
                                    const MTrie<_valueT>* currentNode) const {
  for (const_nodes_iterator nItr = currentNode->nodesMap.begin();
       nItr != currentNode->nodesMap.end(); ++nItr) {
    recursiveDebug(out, levelNum + 1, nItr->second);
  }
  out << levelNum << ' ' << currentNode->nodesMap.size() << ' ' << currentNode->valuesList.size()
      << ' ' << endl;
}

template <typename _valueT>
void MTrie<_valueT>::levelDebug(ostream& out, size_t searchLevel) const {
  out << "Searching for level " << searchLevel << endl;
  levelDebug(out, 0, searchLevel, this);
}

template <typename _valueT>
void MTrie<_valueT>::levelDebug(ostream& out, size_t currentLevel, size_t searchLevel,
                                const MTrie<_valueT>* currentNode) const {
  if ((currentLevel + 1) == searchLevel) {
    // Next level is the winner
    for (const_nodes_iterator nItr = currentNode->nodesMap.begin();
         nItr != currentNode->nodesMap.end(); ++nItr) {
      out << "Key: " << nItr->first << ", number of values: " << value_size(nItr->second) << endl;
    }
  } else {
    for (const_nodes_iterator nItr = currentNode->nodesMap.begin();
         nItr != currentNode->nodesMap.end(); ++nItr) {
      levelDebug(out, currentLevel + 1, searchLevel, currentNode);
    }
  }
}

template <typename _valueT>
size_t MTrie<_valueT>::value_size(const MTrie<_valueT>* currentNode) const {
...
template <typename _valueT> list<string> MTrie<_valueT>::findMinLength(size_t minLength) const {
  base.clear();
  list<string> retMe;
...
template <typename _valueT>
void MTrie<_valueT>::findMinLength(size_t minLength, list<string>& retMe,
                                   const MTrie<_valueT>* currentNode) const {
  if ((base.size() >= minLength) && !currentNode->valuesList.empty()) {
    ...
    for (const_nodes_iterator nItr = currentNode->nodesMap.begin();
         nItr != currentNode->nodesMap.end(); ++nItr) {
      base.push_front(nItr->first);
      findMinLength(minLength, retMe, nItr->second);
      base.pop_front();
    }
  }
}
