...
public:
   ...
   * the Buffer.
   */
  virtual size_type size() const { return buf.size(); }

  /**
   * Return the the number of characters currently in
   * the Buffer.
   */
  virtual size_type length() const { return buf.length(); }

  /**
   * Return true if the buffer is empty, false otherwise.
   */
  virtual bool empty() const { return buf.empty(); }
   ...
   * Buffer.
   */
  inline std::string substr(const size_type& index, const size_type& len) const {
   ...
   */
  inline const std::string& toString() const { return buf; }

  /**
   * Return a const reference to a C++ string representation
   * of this Buffer.
   */
  inline const std::string& operator*() const { return buf; }
   ...
   */
  inline void append(const char* appendMe, size_t numBytes) { buf.append(appendMe, numBytes); }
