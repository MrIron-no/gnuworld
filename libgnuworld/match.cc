...
int match(const string& s1, const string& s2) {
    ...
    if (isUserHost(s1) && isUserHost(s2)) {
        ...
        if (match(extractUser(s1).c_str(), extractUser(s2).c_str()))
            return 1;
        /* If passed, we deal only with the hostip part, and because match() handles cidr addresses
         * too our matching is complete.
         */
...
int mmatch(const char* old_mask, const char* new_mask) {
    ...
    while (1) {
         ...
         */
        if ((*m == '*' && !mq) || ((!mq || nq) && ToLower(*m) == ToLower(*n)) ||
            (*m == '?' && !mq && (*n != '*' || nq))) {
            if (*m)
                m++;
            if (*n)
...
int mmexec(const char* wcm, int wminlen, const char* rcm, int rminlen) {
    ...
    if (!*r) {
        ...
        /* match the chunks */
        while (1) { /* This loop can't break but only return   */
    ...
    while (1) {
        ...
        if (!(rx < rz)) /* head finished            */
            break;
        for (bw++, (br = ++rx); (br < rz) && (*bw) && ((*bw == *br) || (*bw == 'A'));
             br++, bw++) { /* noop */
        };
        ...
        if (*bw != 'Z') {
            eat = 0;
            if (!(br < rz)) {       /* If we failed because we got the end of head */
    ...
    if (*r++) {
        while (*r) {
            ...
            if (!*r)
                break;
            for ((br = ++r), bw++; (*br) && (*br != 'Z') && ((*bw == *br) || (*bw == 'A'));
                 br++, bw++) { /* noop */
            };
            if (*br == 'Z')
                while (*bw == 'A')
            ...
            if (*bw != 'Z') {
                eat = 0;
                if ((!*br) || (*r == 'Z')) { /* If we hit the end of rm or a star in it */
    ...
    while (r >= rx) {
        ...
        if (!(r >= rx))
            return 1;
        for ((br = --r), bw++; (*bw) && (br >= rx) && ((*bw == *br) || (*bw == 'A'));
             br--, bw++) { /* noop */
        };
        if (!*bw)
            return 0;
