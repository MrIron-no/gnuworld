...
inline uint32_t F(uint32_t xx, uint32_t yy, uint32_t zz) { return ((xx & yy) | (~xx & zz)); }

inline uint32_t G(uint32_t xx, uint32_t yy, uint32_t zz) { return ((xx & zz) | (yy & ~zz)); }

inline uint32_t H(uint32_t xx, uint32_t yy, uint32_t zz) { return xx ^ yy ^ zz; }

inline uint32_t I(uint32_t xx, uint32_t yy, uint32_t zz) { return yy ^ (xx | ~zz); }

// ROTATE_LEFT rotates x left n bits.
inline uint32_t ROTATE_LEFT(uint32_t xx, int32_t nn) { return (xx << nn) | (xx >> (32 - nn)); }

// FF, GG, HH, and II transformations for rounds 1, 2, 3, and 4.
// Rotation is separate from addition to prevent recomputation.
inline void FF(uint32_t& aa, uint32_t bb, uint32_t cc, uint32_t dd, uint32_t xx, int32_t ss,
               uint32_t ac) {
  aa += F(bb, cc, dd) + xx + ac;
...
inline void GG(uint32_t& aa, uint32_t bb, uint32_t cc, uint32_t dd, uint32_t xx, int32_t ss,
               uint32_t ac) {
  aa += G(bb, cc, dd) + xx + ac;
  aa = ROTATE_LEFT(aa, ss);
  aa += bb;
...
inline void HH(uint32_t& aa, uint32_t bb, uint32_t cc, uint32_t dd, uint32_t xx, int32_t ss,
               uint32_t ac) {
  aa += H(bb, cc, dd) + xx + ac;
  aa = ROTATE_LEFT(aa, ss);
  aa += bb;
...
inline void II(uint32_t& aa, uint32_t bb, uint32_t cc, uint32_t dd, uint32_t xx, int32_t ss,
               uint32_t ac) {
  aa += I(bb, cc, dd) + xx + ac;
  aa = ROTATE_LEFT(aa, ss);
  aa += bb;
...
md5Digest::md5Digest(void) { clear(); }

md5Digest::~md5Digest(void) { clear(); }

void md5Digest::clear(void) {
  memset(m_data, 0, sizeof(m_data));
  return;
}

unsigned char& md5Digest::operator[](size_t ii) // lvalue operator[].
{
...
unsigned char md5Digest::operator[](size_t ii) const // rvalue operator[].
{
  if (ii >= MD5_DIGEST_LENGTH) { // Note that size_t is unsigned.
...
int32_t operator==(const md5Digest& lhs, const md5Digest& rhs) {
...
int32_t operator!=(const md5Digest& lhs, const md5Digest& rhs) { return !(lhs == rhs); }

std::istream& operator>>(std::istream& stream, md5Digest& digest) {
  stream.read(reinterpret_cast<char*>(digest.m_data), MD5_DIGEST_LENGTH);
...
std::ostream& operator<<(std::ostream& stream, const md5Digest& digest) {
  stream.write(reinterpret_cast<const char*>(digest.m_data), MD5_DIGEST_LENGTH);
  //   for( int32_t ii = 0; ii < MD5_DIGEST_LENGTH; ii++ ) {
  //      stream << digest.m_data[ii];
  //   }
  return stream;
...
void md5::init(void) {      // Load magic initialization constants.
...
void md5::clear(void) { // Zero the data memebers of this class.
...
void md5::update(const unsigned char* input,
                 size_t inputLen) { // MD5 block update operation. Continues an MD5 message-digest
...
void md5::report(
    md5Digest& digest) { // MD5 finalization. Ends an MD5 message-digest operation, writing the
...
void md5::transform(
    const unsigned char block[64]) { // MD5 basic transformation. Transforms state based on block.
  const int32_t S11 = 7;
...
void md5::encode(unsigned char* output, const uint32_t* input,
                 int32_t len) { // Encodes input (uint32_t) into output (unsigned char).
...
void md5::encode(md5Digest& digest) { // Encodes m_state into digest.
  int32_t ii, jj;
...
void md5::decode(uint32_t* output, const unsigned char* input,
                 int32_t len) { // Decodes input (unsigned char) into output (uint32_t).
                                // Assumes len is a multiple of 4.
  int32_t ii, jj;
  for (ii = 0, jj = 0; jj < len; ii++, jj += 4) {
    output[ii] = ((uint32_t)input[jj]) | (((uint32_t)input[jj + 1]) << 8) |
                 (((uint32_t)input[jj + 2]) << 16) | (((uint32_t)input[jj + 3]) << 24);
  }
  return;
}
