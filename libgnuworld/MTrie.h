 ...
 */
template <typename _valueT> class MTrie {
public:
  /**
   * Initialize an MTrie.
   */
   ...
   * in this MTrie.
   */
  inline size_type size() const { return numElements; }
   ...
   */
  virtual void levelDebug(std::ostream&, size_t searchLevel) const;

  /**
   * Return all keys that have at least as many tokens as
   * minLength.
   */
  virtual std::list<std::string> findMinLength(size_t minLength) const;

protected:
  /**
   * Recursive helper method to the public levelDebug()
   */
  virtual void levelDebug(std::ostream&, size_t currentLevel, size_t searchLevel,
   ...
   */
  virtual void findMinLength(size_t minLength, std::list<std::string>& retMe,
                             const MTrie<data_type>*) const;
   ...
   */
  virtual void find(const MTrie<data_type>*, const StringTokenizer&,
                    StringTokenizer::const_reverse_iterator) const;

  /**
   * Recursive erase method that handles all the hard work
   * matching across multiple levels.
   */
  virtual size_type erase(MTrie<data_type>*, std::list<std::string>&, const std::string& origKey,
   ...
   * strings.
   */
  virtual void recursiveFind(const MTrie<data_type>*, bool blindRecursion = false) const;

  /**
   * Recursive method used only for erasing '*' matched
   * strings.
   */
  virtual size_type recursiveErase(MTrie<data_type>*, std::list<std::string>& base,
                                   const std::string& key);

  /**
   * A recursive method used to output some debugging information.
   */
  virtual void recursiveDebug(std::ostream&, size_t levelNum, const MTrie<data_type>*) const;
