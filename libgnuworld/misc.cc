...
int strcasecmp(const string& s1, const string& s2) { return ::strcasecmp(s1.c_str(), s2.c_str()); }
...
int atoi(const std::string& val) { return ::atoi(val.c_str()); }
...
const string prettyDuration(int duration) {
  ...
  int res = ::time(NULL) - duration, secs = res % 60, mins = (res / 60) % 60,
      hours = (res / 3600) % 24, days = (res / 86400);

  sprintf(tmpBuf, "%i day%s, %02d:%02d:%02d", days, (days == 1 ? "" : "s"), hours, mins, secs);
...
const string escapeSQLChars(const string& theString) {
  ...
  for (string::const_iterator ptr = theString.begin(); ptr != theString.end(); ++ptr) {
    if (*ptr == '\'') {
      // retMe += "\\\047" ;
      retMe += "''";
    }
...
void stripModes(std::string& tempModes, const std::string& modesToStrip) {
  ...
  for (char mode : modes) {
    if (modesToStrip.find(mode) == std::string::npos) {
      newModes += mode;
      if (modesWithArgs.find(mode) != std::string::npos &&
          modeArgMap.find(mode) != modeArgMap.end())
