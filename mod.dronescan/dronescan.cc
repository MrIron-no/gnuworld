...
dronescan::dronescan(const string& configFileName) : xClient(configFileName) {
    ...
    /* Spy client periodic quit / rejoin cooldown / 2nd-client join defaults */
    spyClientQuitMinSec = atoi(dronescanConfig->Require("spyClientQuitMinSec")->second.c_str());
    spyClientQuitMaxSec = atoi(dronescanConfig->Require("spyClientQuitMaxSec")->second.c_str());
    randRejoinMinSec = atoi(dronescanConfig->Require("randRejoinMinSec")->second.c_str());
    randRejoinMaxSec = atoi(dronescanConfig->Require("randRejoinMaxSec")->second.c_str());
    secondSpyJoinIntervalMinDefault =
        atoi(dronescanConfig->Require("secondSpyJoinIntervalMin")->second.c_str());
    ...
    RegisterCommand(
        new SPAMCommand(this, "SPAM", "<EVENT|RULE|ACTION|EXCLUSION|SPYCLIENT|CHAN> <verb> ..."));
 ...
 */
void dronescan::OnChannelCTCP(iClient* Sender, Channel* theChan, const std::string& CTCPCommand,
...
void dronescan::OnPrivateMessage(iClient* theClient, const string& Message, bool) {
    ...
    if (!theUser && Command != "HELLO") {
        // log(INFO, "Oper %s is not authenticated for using command %s",
        // theClient->getNickName().c_str(), Command.c_str());
 ...
 */
void dronescan::OnFakePrivateMessage(iClient* Sender, iClient* Target, const std::string& Message,
                                     bool secure) {
    if (!Sender || currentState != RUN)
        return;
 ...
 * with the NOTICE bit.
 */
void dronescan::OnFakePrivateNotice(iClient* Sender, iClient* Target, const std::string& Message,
                                    bool secure) {
    if (!Sender || currentState != RUN)
        return;
 ...
 */
void dronescan::OnNetworkKick(Channel* theChan, iClient* srcClient, iClient* destClient,
    ...
    // still be covering others. May promote a 2nd visitor to primary.
    bool promoted =
        removeSpyClientFromChannelTracking(scId, chanKey, /*scheduleReplacement=*/false, forcejoin);

    // Report to console
    const string kickerStr = srcClient ? srcClient->getNickName() : "<server>";
    Message(consoleChannel, "[SpyClient] %s was kicked from %s by %s (%s).",
            destClient->getNickName().c_str(), theChan->getName().c_str(), kickerStr.c_str(),
            kickMessage.c_str());
    ...
    if (track.count >= 2) {
        ...
        // If a 2nd visitor was promoted to primary above, part it too -
        // we're stopping monitoring of this channel entirely.
        if (promoted) {
            chanActiveSpyMapType::const_iterator ait = chanActiveSpyMap.find(chanKey);
            if (ait != chanActiveSpyMap.end())
                partSpyClientFromChannel(ait->second, theChan->getName());
        }
    ...
    // Schedule a replacement - use a different spy client if possible, unless
    // a 2nd visitor was already promoted to primary above.
    if (!promoted) {
...
void dronescan::logCommandMessage(const iClient* theClient, const string& Message) {
    ...
    insertQ << "INSERT into comlog (ts, username, command) VALUES (" << ::time(0) << ", '"
            << escapeSQLChars(username) << "', '" << escapeSQLChars(Message) << "')" << std::ends;
...
void dronescan::refreshSpamCaches() {
    ...
    for (spamRegexCacheType::iterator it = spamRegexCache.begin(); it != spamRegexCache.end();
         ++it) {
        if (it->second)
...
void dronescan::relinkSpamGraph() {
    for (spamEventsMapType::const_iterator it = spamEventsMap.begin(); it != spamEventsMap.end();
         ++it)
        it->second->clearRules();

    for (spamRulesMapType::const_iterator it = spamRulesMap.begin(); it != spamRulesMap.end();
         ++it) {
...
void dronescan::compileEventRegex(sqlSpamEvent* ev) {
    ...
    pcre2_code* re = pcre2_compile(reinterpret_cast<PCRE2_SPTR>(ev->getParam().c_str()),
                                   PCRE2_ZERO_TERMINATED, 0, &errcode, &erroffset, nullptr);
    if (re)
        spamRegexCache[ev->getId()] = re;
    else
        elog << "dronescan::compileEventRegex> PCRE2 compile failed for event " << ev->getId()
             << " at offset " << erroffset << std::endl;
}
...
void dronescan::compileRepeatExclusionRegex(sqlSpamEvent* ev) {
    ...
    pcre2_code* re =
        pcre2_compile(reinterpret_cast<PCRE2_SPTR>(ev->getRepeatExclusionRegex().c_str()),
...
void dronescan::preloadSpamEvents() {
    ...
    elog << "dronescan::preloadSpamEvents> Loaded " << spamEventsMap.size() << " spam events."
         << std::endl;
}
...
void dronescan::preloadSpamRules() {
    ...
    elog << "dronescan::preloadSpamRules> Loaded " << spamRulesMap.size() << " spam rules."
         << std::endl;
}
...
void dronescan::preloadSpamActions() {
    ...
    elog << "dronescan::preloadSpamActions> Loaded " << spamActionsMap.size() << " spam actions."
         << std::endl;
}
...
void dronescan::preloadSpamRuleEvents() {
    ...
    elog << "dronescan::preloadSpamRuleEvents> Loaded " << total << " rule-event links."
         << std::endl;
}
...
void dronescan::preloadSpamRuleActions() {
    ...
    elog << "dronescan::preloadSpamRuleActions> Loaded " << total << " rule-action links."
         << std::endl;
}
...
void dronescan::preloadSpyClients() {
    for (spyClientsMapType::iterator it = spyClientsMap.begin(); it != spyClientsMap.end(); ++it)
        delete it->second;
    spyClientsMap.clear();
    ...
    elog << "dronescan::preloadSpyClients> Loaded " << spyClientsMap.size() << " spy clients."
         << std::endl;
}
...
void dronescan::preloadSpamRuleChannels() {
    ...
    elog << "dronescan::preloadSpamRuleChannels> Loaded " << total << " rule-channel entries."
         << std::endl;
}
...
void dronescan::processRepeatEvent(sqlSpamEvent* ev, const SpamActor& actor,
    ...
    if (xit != spamRepeatExclusionCache.end() && xit->second) {
        ...
        int rc = pcre2_match(xit->second, reinterpret_cast<PCRE2_SPTR>(text.c_str()), text.size(),
                             0, 0, mdata, nullptr);
    ...
    const string scope = channel_name.empty() ? string("privmsg") : string_lower(channel_name);
...
void dronescan::processSpamText(iClient* theClient, const std::string& text, int target_bit,
    ...
    const std::string displayChannels =
        channel_name.empty() ? monitoredChannelNamesForClient(theClient) : channel_name;
    ...
    for (size_t i = 0; i < spamEventsList.size(); ++i) {
        ...
        int rc = pcre2_match(rit->second, reinterpret_cast<PCRE2_SPTR>(text.c_str()), text.size(),
                             0, 0, mdata, nullptr);
        pcre2_match_data_free(mdata);
...
void dronescan::evaluateSpamRules(const SpamActor& actor, const std::string& channel_name,
    ...
    for (spamRulesMapType::const_iterator rit = spamRulesMap.begin(); rit != spamRulesMap.end();
         ++rit) {
        ...
        if (!channel_name.empty()) {
            spamRuleChannelsMapType::const_iterator rcit = spamRuleChannelsMap.find(rule->getId());
            bool inList = false;
...
// overrides), before it is dispatched or scheduled. Purely transient -
// built fresh per fireRuleActions() call, never stored.
struct ResolvedSpamAction {
...
std::string buildSpamReportLine(const std::string& ruleName, const std::string& nick,
    ...
    if (resolved.empty()) {
        ...
        for (size_t i = 0; i < resolved.size(); ++i) {
            if (i > 0)
                line << ", ";
            line << resolved[i].actionType;
            if (resolved[i].actionType == "GLINE")
                line << ' ' << resolved[i].duration << 's';
            if (resolved[i].delay <= 0)
                line << " (now)";
            else
                line << " (in " << resolved[i].delay << "s)";
        }
    }
...
void dronescan::fireRuleActions(sqlSpamRule* rule, const SpamActor& actor,
    ...
    std::vector<ResolvedSpamAction> resolved;
    ...
    for (size_t i = 0; i < actions.size(); ++i) {
        ...
        ResolvedSpamAction r;
        r.actionType = ra->getActionType();

        // Determine effective reason
        r.reason = ra->getActionReasonOverride().empty() ? act->getReason()
                                                         : ra->getActionReasonOverride();
        if (r.reason.empty())
            r.reason = "Spam detected";

        // Determine effective duration (GLINE only; harmless for KILL)
        r.duration = (ra->getActionDurationOverride() >= 0) ? ra->getActionDurationOverride()
                                                            : act->getDuration();
        if (r.duration < 0)
            r.duration = 3600;
        ...
        r.delay = (ra->getDelayOverride() >= 0) ? ra->getDelayOverride() : act->getDelay();
        if (r.delay < 0)
            r.delay = 0;
        ...
        if (act->getRandMin() >= 0 && act->getRandMax() >= 0) {
            ...
            if (hi < lo)
                std::swap(lo, hi);
            r.delay += lo + (rand() % (hi - lo + 1));
        }

        r.prefixAuto = act->isPrefixAuto();
    ...
    for (size_t i = 0; i < resolved.size(); ++i) {
        const ResolvedSpamAction& r = resolved[i];
        ...
        pending.actionType = r.actionType;
        pending.reason = r.reason;
        pending.duration = r.duration;
...
/**
 * executeSpamAction: run a single already-resolved action (GLINE/KILL) for
 * the offending actor. Uses the captured SpamActor identity so GLINE still
 * works even if the offender has since quit (crossuser TEXT_REPEAT, or a
 * delayed action outliving the offender's connection); KILL is the one
 * action that needs the offender still connected and is silently skipped
 * (logged only, no console line) otherwise. Never touches the console -
 * the combined "[S]" report line is printed once, up front, by
 * fireRuleActions(); this only queues the GLINE / calls Kill() and writes
 * the log4cplus audit line.
 */
void dronescan::executeSpamAction(const std::string& actionType, const std::string& reason,
    ...
    if (actionType == "GLINE") {
        const string mask =
            glineNeedsIdent(actor.ip) ? (actor.user + "@" + actor.ip) : ("*@" + actor.ip);
...
iClient* dronescan::introduceSpyClient(sqlSpyClient* sc) {
    ...
    if (Network->findClient(nick) != nullptr) {
        ...
        if (attempts >= 200) {
            Message(consoleChannel,
                    "[SpyClient] Could not find a free nick for spy client id %d (%s) after 200 "
                    "attempts.",
                    sc->getId(), sc->getNickname().c_str());
            return nullptr;
        }
        if (!found)
            Message(consoleChannel, "[SpyClient] Nick collision for %s - using %s instead.",
    ...
    if (ipmask_parse(sc->getIp().c_str(), &ip, &ipmask_len)) {
        ...
        base64ip = "AAAAAA";
        elog << "dronescan::introduceSpyClient> Invalid IP string '" << sc->getIp()
             << "' for spy client " << sc->getId() << endl;
    }

    // YYXXX: the core reassigns the numeric via AttachClient
    string yyxxx(MyUplink->getCharYY() + "]]]");

    iClient* ic = new iClient(MyUplink->getIntYY(), yyxxx, nick, sc->getUsername(), base64ip,
                              sc->getHostname(), sc->getHostname(), sc->getModes(),
                              string(), // account - set via UserLogin() if needed
                              0,        // account_id - set via UserLogin() if needed
                              0,        // account_flags
                              string(), // tls_fingerprint
                              sc->getRealname(), ::time(nullptr));

    if (!MyUplink->AttachClient(ic, this)) {
        delete ic;
        elog << "dronescan::introduceSpyClient> AttachClient failed for spy client " << sc->getId()
             << endl;
        return nullptr;
    }

    if (!sc->getAccount().empty()) {
        MyUplink->UserLogin(ic, sc->getAccount(), static_cast<unsigned int>(sc->getAccountId()), 0,
                            this);
    }
    ...
    // Arm this client's next spontaneous QUIT cycle.
    scheduleSpyClientPersonalQuit(sc->getId());

    elog << "dronescan::introduceSpyClient> Introduced " << nick << " (id=" << sc->getId()
         << ") to the network." << endl;
    return ic;
}
...
/**
 * Removes scId's association with chanKey, whether it was covering it as
...
bool dronescan::removeSpyClientFromChannelTracking(int scId, const std::string& chanKey,
    ...
    chanActiveSpyMapType::iterator ait = chanActiveSpyMap.find(chanKey);
    if (ait != chanActiveSpyMap.end() && ait->second == scId) {
        chanActiveSpyMap.erase(ait);
        wasPrimary = true;
    }

    if (!wasPrimary) {
        chanSecondSpyMapType::iterator sit2 = chanSecondSpyMap.find(chanKey);
        if (sit2 != chanSecondSpyMap.end() && sit2->second == scId) {
            chanSecondSpyMap.erase(sit2);
            wasSecondary = true;
        }
    }

    if (!wasPrimary && !wasSecondary)
        return false; // scId had no tracked role for this channel

    bool promoted = false;
    if (wasPrimary) {
        chanSecondSpyMapType::iterator promIt = chanSecondSpyMap.find(chanKey);
        if (promIt != chanSecondSpyMap.end()) {
            chanActiveSpyMap[chanKey] = promIt->second;
            chanSecondSpyMap.erase(promIt);
            promoted = true;
        }
    }
...
void dronescan::partSpyClientFromChannel(int scId, const std::string& chanName) {
    ...
    // Admin-triggered path - no auto-replacement.
    removeSpyClientFromChannelTracking(scId, chanKey, /*scheduleReplacement=*/false, false);
}
...
void dronescan::detachSpyClient(int scId) {
    ...
    // It's leaving the pool for good, unlike a voluntary periodic quit.
    cancelSpyClientPersonalQuit(scId);

    // Part it from every channel it is monitoring (copy first - the
    // tracking helper mutates spyClientChanMap[scId] as it goes).
    spyClientChanMapType::iterator sit = spyClientChanMap.find(scId);
    if (sit != spyClientChanMap.end()) {
        std::set<string> chans = sit->second;
        for (std::set<string>::const_iterator cit = chans.begin(); cit != chans.end(); ++cit) {
            const string& chanName = *cit;
            Channel* theChan = Network->findChannel(chanName);
            if (theChan && theChan->findUser(ic))
                MyUplink->PartChannel(ic, chanName, "");
            // Admin-triggered path - no auto-replacement.
            removeSpyClientFromChannelTracking(scId, chanName, /*scheduleReplacement=*/false,
                                               false);
        }
    }
...
int dronescan::selectFromCandidates(const std::vector<int>& candidateIds,
    ...
    for (size_t ci = 0; ci < candidateIds.size(); ++ci) {
        int candId = candidateIds[ci];
        if (candId == excludeId)
            continue;

        spyClientsMapType::const_iterator it = spyClientsMap.find(candId);
        ...
        // channel; opportunistically prune expired entries.
...
int dronescan::findBestSpyClient(const std::string& chanName, bool forcejoin) {
    return selectSpyClient(chanName, forcejoin, /*requireIdle=*/false, /*excludeId=*/-1);
}
 ...
 */
void dronescan::scheduleSpyClientJoin(const std::string& chanName, bool forcejoin, int minDelay,
                                      int maxDelay) {
...
std::string dronescan::randomQuitReason() const {
    ...
    static const size_t numReasons = sizeof(reasons) / sizeof(reasons[0]);
    return reasons[rand() % numReasons];
}

/** Arms scId's next spontaneous QUIT, in [spyClientQuitMinSec, spyClientQuitMaxSec]. */
void dronescan::scheduleSpyClientPersonalQuit(int scId) {
    int delay = spyClientQuitMinSec;
    if (spyClientQuitMaxSec > spyClientQuitMinSec)
...
void dronescan::cancelSpyClientPersonalQuit(int scId) {
    for (pendingSpyQuitTimersType::iterator it = pendingSpyQuitTimers.begin();
...
/**
 * Fired when a spy client's personal quit timer expires: QUITs it from the
 * network with a random reason, frees/reschedules the channels it covered
 * (promoting a 2nd visitor to primary where one is present), then
 * reconnects it (idle) and re-arms its next quit cycle.
 */
void dronescan::handleSpyClientPersonalQuit(int scId) {
    liveSpyClientsMapType::iterator lit = liveSpyClientsMap.find(scId);
    if (lit == liveSpyClientsMap.end())
...
void dronescan::retireAndReplaceSpyClient(int scId) {
    ...
    spyClientChanMapType::iterator sit = spyClientChanMap.find(scId);
    if (sit != spyClientChanMap.end()) {
        std::set<string> chans = sit->second; // copy - helper mutates spyClientChanMap
        for (std::set<string>::const_iterator cit = chans.begin(); cit != chans.end(); ++cit) {
            const string& chanKey = *cit;
            monitoredChannelsMapType::const_iterator mc = monitoredChannelsMap.find(chanKey);
            bool forcejoin = (mc != monitoredChannelsMap.end()) ? mc->second->isForceJoin() : false;
            removeSpyClientFromChannelTracking(scId, chanKey, /*scheduleReplacement=*/true,
                                               forcejoin);
        }
    }
...
/**
 * Periodic sweep: for monitored channels with a primary spy client but no
 * 2nd visitor yet, checks whether the channel's (per-channel or global
 * default) 2nd-join interval has elapsed and, if so, brings in another
 * idle spy client for temporary double coverage.
 */
void dronescan::checkSecondSpyJoins() {
    time_t now = ::time(nullptr);
    ...
    for (monitoredChannelsMapType::const_iterator it = monitoredChannelsMap.begin();
        ...
        chanActiveSpyMapType::const_iterator ait = chanActiveSpyMap.find(chanKey);
        if (ait == chanActiveSpyMap.end())
            continue; // no primary yet, nothing to double up
        if (chanSecondSpyMap.count(chanKey))
            continue; // already has a 2nd visitor

        int overrideMin = mc->getSecondSpyJoinIntervalMin();
        int effectiveMin = (overrideMin >= 0) ? overrideMin : secondSpyJoinIntervalMinDefault;
        if (effectiveMin <= 0)
            continue; // disabled

        time_t& last = chanSecondJoinLastMap[chanKey];
        if (now - last < static_cast<time_t>(effectiveMin) * 60)
            continue;
        last = now; // pace attempts every interval regardless of hit/miss

        int primaryId = ait->second;
        int scId2 =
            selectSpyClient(mc->getName(), /*forcejoin=*/false, /*requireIdle=*/true, primaryId);
        if (scId2 < 0)
            continue; // no idle client available this time

        liveSpyClientsMapType::const_iterator lit = liveSpyClientsMap.find(scId2);
        if (lit == liveSpyClientsMap.end())
            continue;
...
void dronescan::resyncSpyClients() {
    ...
    for (std::vector<int>::iterator it = toDetach.begin(); it != toDetach.end(); ++it) {
        detachSpyClient(*it);
        Message(consoleChannel,
                "[SpyClient] Detached spy client id %d (removed or disabled in DB).", *it);
    }
    ...
    for (spyClientsMapType::const_iterator it = spyClientsMap.begin(); it != spyClientsMap.end();
        ...
        if (!it->second->isEnabled())
            continue;
        if (liveSpyClientsMap.count(it->first))
            continue;
        introduceSpyClient(it->second);
    }

    // Step 3: schedule replacement joins for orphaned channels
    for (std::vector<string>::iterator it = orphanedChans.begin(); it != orphanedChans.end();
         ++it) {
        const string& chanName = *it;
        if (kickStoppedChannels.count(chanName))
            continue;
        // A 2nd visitor may already have been promoted to primary when the
        // stale client was detached above - nothing to do in that case.
        if (chanActiveSpyMap.count(chanName))
            continue;

        // Find the monitored channel config
        monitoredChannelsMapType::const_iterator mc = monitoredChannelsMap.find(chanName);
        if (mc == monitoredChannelsMap.end())
            continue;
        if (!mc->second->isEnabled())
            continue;
        if (mc->second->isJoinAsService())
            continue;
