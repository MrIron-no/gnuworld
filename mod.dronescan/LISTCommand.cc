...
void LISTCommand::Exec(const iClient* theClient, const string& Message, const sqlUser*) {
  ...
  if ("ACTIVE" == Name) {
    ...
    for (dronescan::droneChannelsType::const_iterator itr = bot->droneChannels.begin();
      ...
      if (theChannel) {
        bot->Reply(theClient, "  [%4u] (%u) %s", theChannel->size(), itr->second->getLastJoin(),
                   theChannel->getName().c_str());
      } else {
        bot->Reply(theClient, "  [   0] (%u) %s", itr->second->getLastJoin(), itr->first.c_str());
      }
    }
  }
  ...
  if ("FAKECLIENTS" == Name) {
    ...
    for (dronescan::fcMapType::const_iterator itr = bot->fakeClients.begin();
      ...
      bot->Reply(theClient, "  (%02u %1s) %s", theFake->getId(), theFake->getFlagsString().c_str(),
  ...
  if ("EXCEPTIONALCHANNELS" == Name) {
    ...
    for (dronescan::exceptionalChannelsType::const_iterator itr = bot->exceptionalChannels.begin();
