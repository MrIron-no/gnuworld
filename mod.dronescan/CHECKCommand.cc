...
void CHECKCommand::Exec(const iClient* theClient, const std::string& Message, const sqlUser*) {
  ...
  if (st[1][0] == '#') {
    ...
    if (!theChannel) {
      bot->Reply(theClient, "Unable to find channel %s.", st[1].c_str());
      return;
    }

    bot->Reply(theClient, "Checking channel %s:", theChannel->getName().c_str());
    ...
    for (; chanItr != theChannel->userList_end(); ++chanItr) {
      ...
      bot->Reply(theClient, "[%s] (%s) %0.3lf %s (%s)", bot->isNormal(targetClient) ? "N" : "A",
                 targetClient->getCharYY().c_str(), userEntropy,
    ...
    bot->Reply(theClient, "min/avg/max channel entropy : %0.3lf / %0.3lf / %0.3lf", minEntropy,
               averageEntropy, maxEntropy);
    bot->Reply(theClient, "Range above/below/total     : %0.3lf / %0.3lf / %0.3lf",
               maxEntropy - averageEntropy, averageEntropy - minEntropy, maxEntropy - minEntropy);
    ...
    if (!targetClient) {
      bot->Reply(theClient, "Unable to find user %s.", st[1].c_str());
      return;
    }

    bot->Reply(theClient, "Checking user %s:", targetClient->getNickName().c_str());
