...
sqlUser::sqlUser(dbHandle* _SQLDb)
    : user_name(), last_seen(0), last_updated_by(), last_updated(0), flags(0), SQLDb(_SQLDb) {}
...
#ifdef LOG_SQL
  elog << "sqlUser::commit> " << queryString << endl;
#endif
...
bool sqlUser::insert() {
  ...
  insertString << "INSERT INTO users "
               ...
               << "VALUES "
               << "("
               << "'" << escapeSQLChars(user_name) << "', " << created << ", " << last_seen << ", "
               << last_updated << ", "
               << "'" << escapeSQLChars(last_updated_by) << "', " << flags << ", " << access << ")";

#ifdef LOG_SQL
  elog << "sqlUser::insert> " << insertString << endl;
#endif
