...
bool MAXCHANSTest::isNormal(const Channel* theChannel) {
  ...
  for (; chanItr != theChannel->userList_end(); ++chanItr) {
    ...
    if (theClient->channels_size() >= maxChans)
      ++normals;
    if (normals >= 3)
      return true;
  }
...
const std::string MAXCHANSTest::getVariable() const { return "maxChans"; }

bool MAXCHANSTest::setVariable(const std::string& var, const std::string& value) {
  if ("MAXCHANS" != var)
    return false;
