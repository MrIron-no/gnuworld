...
bool SCANCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (showUsers) {
    ...
    string ClientInfo;
    for (clientsList::iterator cptr = cList.begin(); ((cptr != cList.end()) && (shown < 15));
         ++cptr) {
      curClient = *cptr;
  ...
  if (showIdentReport) {
    ...
    bot->Notice(theClient, "Ident report: %d unidented clients, %d unique idents", ident_nonidented,
                ident_count);
  }
