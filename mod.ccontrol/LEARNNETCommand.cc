...
bool LEARNNETCommand::Exec(iClient* theClient, const string& Message) {
  ...
  for (; ptr != end; ptr++) {
    ...
    if ((0 == Network->findFakeServer(CurServer)) &&
        (strcmp(CurServer->getName().c_str(), bot->getUplinkName().c_str()))) {
      if (!bot->getServer(CurServer->getName())) {
        // If the server isnt in the database , update it
        ccServer* NewServer = new (std::nothrow) ccServer(bot->SQLDb);
        assert(NewServer != 0);
        ...
        bot->Write("%s V :%s\n", bot->getCharYYXXX().c_str(), CurServer->getCharYY().c_str());
  ...
  bot->Notice(theClient,
              "Finished learning the network. Learned a "
              "total of %d servers\n",
              AddedServers);
