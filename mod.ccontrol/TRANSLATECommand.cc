...
bool TRANSLATECommand::Exec(iClient* theClient, const string& Message) {
  ...
  if ((Target->isModeR()) && (Target->isModeX())) {
    bot->Notice(theClient, "%s is %s (%s) on server %s", st[1].c_str(),
    ...
    bot->Notice(theClient, "%s is %s on server %s", st[1].c_str(),
                Target->getNickUserHost().c_str(), theServer->getName().c_str());
  }
