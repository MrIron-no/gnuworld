...
bool USERINFOCommand::Exec(iClient* theClient, const string& Message) {
  ...
  for (ptr = bot->usersmap_begin(); ptr != bot->usersmap_end(); ++ptr) {
    tempUser = ptr->second;
    if (!(match(st[1], tempUser->getUserName())) || !(match(st[1], tempUser->getServer()))) {
      ...
      if (tempUser->getClients().size() > 0) {
        ...
        for (Itr = Clients.begin(); Itr != Clients.end(); Itr++) {
          ...
          bot->Notice(theClient, "    %s (%s.*)", tClient->getRealNickUserHost().c_str(),
                      st2[0].c_str());
        }
      } else
      ...
      if (Suspended) {
        bot->Notice(theClient, "User was suspended by: %s until %s", SuspendedBy.c_str(),
      ...
      bot->Notice(theClient,
                  "Single Sign On: \002%s\002  (only if opered: \002%s\002)   AutoOp: \002%s\002",
                  SSO, SSOOO, AutoOp);
      ...
      if ((st.size() > 2) && (!strcasecmp(st[2], "-cl"))) {
        bot->Notice(theClient, "Password last changed: %s ago",
                    (tempUser->getPassChangeTS() == 0) ? "N/A" : Ago(tempUser->getPassChangeTS()));
        ...
        for (ccontrol::constCommandIterator cmdptr = bot->command_begin();
          ...
          if ((ComLevel & commandLevel::flg_NOLOGIN) ||
              ((tempUser) && (tempUser->gotAccess(cmdptr->second)))) {
