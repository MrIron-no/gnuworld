...
bool ccUser::Update() {
  ...
  theQuery << Main << escapeSQLChars(Password) << "', Access = " << Access
           ...
           << ",isCoder = " << (IsCoder ? "'t'" : "'n'")
           << ",GetLogs = " << (GetLogs ? "'t'" : "'n'") << ",GetLag = " << (GetLag ? "'t'" : "'n'")
           << ",Sso = " << (Sso ? "'t'" : "'n'") << ",Ssooo = " << (Ssooo ? "'t'" : "'n'")
           << ",AutoOp = " << (AutoOp ? "'t'" : "'n'") << ",LastPassChangeTS = " << PassChangeTS
           << ",AccountTS = " << AccountTS << ",NeedOp = " << (NeedOp ? "'t'" : "'n'")
           << ", Email = '" << escapeSQLChars(Email) << "', Account = '" << escapeSQLChars(Account)
