...
bool ADDCOMMANDCommand::Exec(iClient* theClient, const string& Message) {
  ...
  while ((!strcasecmp(st[pos], "-allopers")) || (!strcasecmp(st[pos], "-alladmins")) ||
         (!strcasecmp(st[pos], "-allsmts")) || (!strcasecmp(st[pos], "-allcoders"))) {
  ...
  if (!AllOpers && !AllAdmins && !AllSmts && !AllCoders) {
    ...
    if (!theUser) {
      bot->Notice(theClient, "I can't find oper %s", st[pos].c_str());
      return false;
    }
    pos++;
  } else
  ...
  if (!Comm) {
    bot->Notice(theClient, "Command '%s' does not exist!", st[pos].c_str());
    return false;
  }
  ...
  if (!AClient->gotAccess(Comm) &&
      AClient->getType() != operLevel::CODERLEVEL) // Coders can add any command
  {
    bot->Notice(theClient, "You must have access to a command in order to add it.");
    return false;
  }
  ...
  for (list<ccUser*>::iterator Itr = ccList.begin(); Itr != ccList.end(); Itr++) {
    ...
    if (Forced) {
      if ((AClient->getType() < operLevel::SMTLEVEL) &&
          ((bot->findCommandInMem(st[pos]))->getMinLevel() > theUser->getType())) {
      ...
      if (!sentOnce) {
        if (AClient->getType() >= operLevel::SMTLEVEL)
          bot->Notice(theClient, "The minimum level required to use this command "
    ...
    if (theUser->gotAccess(Comm)) {
      if (!AllOpers && !AllAdmins && !AllSmts && !AllCoders) {
        bot->Notice(theClient, "%s already has access to '%s'", theUser->getUserName().c_str(),
                    st[pos].c_str());
        return false;
      } else
        continue;
    ...
    if (theUser->Update()) {
      bot->Notice(theClient, "Successfully added the command for %s",
                  theUser->getUserName().c_str());
      if (Forced)
        bot->MsgChanLog("%s is using -fr to add '%s' to %s", theClient->getNickName().c_str(),
                        st[pos].c_str(), theUser->getUserName().c_str());
    ...
    bot->Notice(theClient, "Error while adding command for %s", theUser->getUserName().c_str());
    return false;
  }
