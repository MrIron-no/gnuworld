  ...
  while (pos < st.size()) {
    if (!strcasecmp(st[pos], "-p")) // Trying to change the password ?
    {
      ...
      case password::PASS_OK: {
        ...
        if (tmpUser->Update()) {
          bot->Notice(theClient, "Password for %s has been changed to %s", st[1].c_str(),
                      st[pos + 1].c_str());
        } else {
    ...
    } else if (!strcasecmp(st[pos], "-ah")) // Trying to add a new host ?
    {
      ...
      if (!validUserMask(st[pos + 1])) {
        ...
        bot->Notice(theClient, "'%s' already has the host (%s) listed.", st[1].c_str(),
                    st[pos + 1].c_str());
      } else if (bot->AddHost(tmpUser, st[pos + 1])) {
        bot->Notice(theClient, "Mask %s added for %s", st[pos + 1].c_str(), st[1].c_str());
      } else {
        bot->Notice(theClient, "Error while adding the mask (%s) for %s", st[pos + 1].c_str(),
                    st[1].c_str());
      }
      pos += 2;
    } else if (!strcasecmp(st[pos], "-dh")) // Trying to delete an host ?
    {
      ...
      if (!bot->UserGotHost(tmpUser, st[pos + 1])) {
        bot->Notice(theClient, "%s doesn't have the host '%s' in my access list", st[1].c_str(),
                    st[pos + 1].c_str());
      } else if (bot->DelHost(tmpUser, st[pos + 1])) {
        bot->Notice(theClient, "Mask '%s' was deleted from %s's access list", st[pos + 1].c_str(),
                    st[1].c_str());
      } else {
    ...
    else if (!strcasecmp(st[pos], "-ssooo")) // Trying to toggle the Single Sign On if Opered Only
    {
      ...
      if (!strcasecmp(st[pos + 1], "on")) {
        ...
        bot->Notice(theClient, "Single sign on with oper requirement has been turned on for %s",
                    st[1].c_str());
      } else if (!strcasecmp(st[pos + 1], "off")) {
        // Only a smt+ can set Ssooo to off for an oper
        if (AdFlag < operLevel::SMTLEVEL) {
          bot->Notice(theClient,
                      "Single sign on with oper requirement can only be turned off by SMT+");
          return false;
        }
        tmpUser->setSsooo(false);
        bot->Notice(theClient, "Single sign on with oper requirement has been turned off for %s",
                    st[1].c_str());
      } else {
    ...
    } else if (!strcasecmp(st[pos], "-mt")) // Trying to change the message type
    {
      ...
      if ((!strcasecmp(st[pos + 1], "N")) || (!strcasecmp(st[pos + 1], "NOTICE"))) {
        ...
        bot->Notice(theClient, "unknown option %s for -mt must be n/m/notice/message",
                    st[pos + 1].c_str());
        return false;
      }
      tmpUser->setLast_Updated_By(theClient->getRealNickUserHost());
    ...
    else if (!strcasecmp(st[pos], "-s")) // Trying to change the user server
    {
      ...
      if (!strcasecmp(tmpUser->getServer(), SName)) {
        ...
        if (tmpUser->Update()) {
          ...
          bot->Notice(theClient, "Error while associating %s with %s", st[1].c_str(),
                      SName.c_str());
          return false;
        }
        pos += 2;
    ...
    else if (!strcasecmp(st[pos], "-x")) // Trying to set account
    {
      ...
      if (theClient->isModeR()) {
        ...
        if ((otherUser = bot->GetOperByAC(cAC))) {
          bot->Notice(theClient, "This account already belongs to another user: %s",
                      otherUser->getUserName().c_str());
          return false;
        }
