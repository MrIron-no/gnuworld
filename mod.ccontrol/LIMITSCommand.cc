...
bool LIMITSCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (!strcasecmp(st[1].c_str(), "info")) {
    ...
    if (st.size() < 5) {
      ...
      // bot->Notice(theClient, "-f: forcecount - use it if you want limits to still be enforced
      // even if the netblocks" 	" of this isp are not the closest match for a client");
      // bot->Notice(theClient, "-i: inactive - deactivates glines and does not prevent new clients
      // from connecting using iauth. It does reports only.");
      return true;
    }
    if (st[2].size() > 32) {
    ...
    if (IpLisp != 0) {
      ...
      if (bot->insertIpLisp(theClient, st[2], atoi(st[3].c_str()), atoi(st[4].c_str()), email, 1,
                            0)) {
    ...
    if (IpLisp == 0) {
      ...
      if (bot->insertIpLnb(theClient, st[3], IpLisp->getID(), false)) {
        bot->Notice(theClient, "Successfully added '%s' to '%s'", st[3].c_str(), st[2].c_str());
      }
    }
    ...
    if (!IpLisp->updateData()) {
      ...
      bot->Notice(theClient, "Limits are %s enforced on %s", res ? "now" : "no longer",
    ...
    if (!IpLisp->updateData()) {
      ...
      bot->Notice(theClient, "G-lines on %s %sactivated.%s", IpLisp->getName().c_str(),
                  res ? "de" : "", res ? " Limits will keep being enforced via iauth." : "");
    ...
    if (!IpLisp->updateData()) {
      ...
      bot->Notice(theClient, "group for %s is now %sabled", IpLisp->getName().c_str(),
                  res ? "en" : "dis");
    ...
    if (!IpLisp->updateData()) {
      ...
      bot->Notice(theClient, "glunidented for %s is now %sabled", IpLisp->getName().c_str(),
                  res ? "en" : "dis");
    ...
    if (!IpLisp->updateData()) {
      ...
      bot->Notice(theClient, "forcecount for %s is now %sabled", IpLisp->getName().c_str(),
                  res ? "en" : "dis");
    ...
    if (IpLisp == 0) {
      ...
      if (!IpLisp->updateData()) {
        ...
        bot->Notice(theClient, "%s is now checking clones on /%ds. Limit: %d",
                    IpLisp->getName().c_str(), atoi(st[3].c_str()), IpLisp->getLimit());
    }
    ...
    if (IpLisp == 0) {
      ...
      if (!IpLisp->updateData()) {
        ...
        bot->Notice(theClient, "New Ident limit for %s is now %d", IpLisp->getName().c_str(),
                    IpLisp->getIdentLimit());
    }
    ...
    if (IpLisp == 0) {
      ...
      if (!IpLisp->updateData()) {
        ...
        bot->Notice(theClient, "New limit for %s is now %d", IpLisp->getName().c_str(),
                    IpLisp->getLimit());
    }
