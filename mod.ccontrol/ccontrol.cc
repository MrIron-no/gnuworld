...
ccontrol::ccontrol(const string& configFileName) : xClient(configFileName) {
  ...
  */
  ...
  RegisterCommand(new LIMITSCommand(
      this, "LIMITS",
      "(addisp / addnetblock / delisp / delnetblock / list / chlimit / chilimit / chname / chemail "
      "/ forcecount / glunidented / active / nogline / group / chccidr / clearall / userinfo / "
      "info / ipinfo)",
  ...
  RegisterCommand(new SAYCommand(
      this, "DO", "<-s/-b> <#chan/nick> Forces the bot to \"/me\" as the uplink or the bot", false,
      commandLevel::flg_SAY, false, true, false, operLevel::CODERLEVEL, true));
  ...
  RegisterCommand(new UNJUPECommand(this, "UNJUPE", "<server> removes a jupiter on a server", false,
                                    commandLevel::flg_UNJUPE, false, true, false,
                                    operLevel::OPERLEVEL, true));
...
void ccontrol::OnAttach() {
  ...
  if (SendReport) {
    ...
    time_t theTime =
        ::time(0) + ((24 - Now.tm_hour) * 3600 - (Now.tm_min) * 60); // Set the daily timer to 24:00
...
void ccontrol::OnServerMessage(iServer* Server, const string& Message, bool) {
  ...
  if (!strcasecmp(st[1], "RO")) {
    ...
    // LAG_REPORT_INTERVAL)) { 	tmpServer->setLastLagReport(::time(0)); 	MsgChanLag("[lag] %s is %ds
    //lagged", Server->getName().c_str(), (int) (lagTime / 1000));
    // }
  }
  ...
  if (!strcasecmp(st[1], "391")) {
    ...
    if ((lagTime < MAX_LAG_TIME_FOR_SETTIME) && (timeDiff > MAX_TIME_DIFF_FOR_SETTIME) &&
        ((::time(0) - timediffServersMap[Server->getName() + "$td"]) > TIMEDIFF_SETTIME_INTERVAL)) {
...
void ccontrol::OnCTCP(iClient* theClient, const string& CTCP, const string& Message, bool) {
  ...
  if ((!theUser) && !(theClient->isOper())) { // We need to add the flood points for this user
    ...
    if (st.empty()) {
      ...
      xClient::DoCTCP(theClient, CTCP, "Thats a question i am still trying to find the answer to");
      return;
    } else if (st[0] == "SEX") {
    ...
    for (serversconstiterator ptr = serversMap_begin(); ptr != serversMap_end(); ++ptr) {
      ...
      if (TmpServer && (TmpServer->getNetServer()) && (!TmpServer->getNetServer()->isJupe())) {
          ...
          if ((::time(0) - TmpServer->getLastLagSent()) >= MAX_LAG_TIME) {
            if ((::time(0) - TmpServer->getLastLagReport()) > LAG_REPORT_INTERVAL) {
              ...
              if ((ccHub == NULL) || (ccHub->getLagTime() < 14000) ||
                ...
                elog << "ccontrol(rpingCheck)> DEBUG: " << TmpServer->getName().c_str() << " >"
                     << (TmpServer->getLagTime() / 1000)
                     << "s lag getLastLagSent()=" << TmpServer->getLastLagSent()
                     << " getLastLagRecv()=" << TmpServer->getLastLagRecv() << " now=" << ::time(0)
                     << endl;
              }
            }
          }
        ...
        Write("%s RI %s %s %d %s :%d %s", getCharYY().c_str(),
              TmpServer->getNetServer()->getCharYY().c_str(), getCharYYXXX().c_str(), ::time(0),
              s.str().c_str(), ::time(0), s.str().c_str());
...
void ccontrol::handleAC(iClient* theClient) {
  ...
  if ((theUser->getAccountID() != theClient->getAccountID()) && (theUser->getAccountID() == 0)) {
    ...
    Notice(theClient, "This ensures that if your username expires and gets re-registered, I will "
                      "see the difference. You should never see this message again");
  }
...
void ccontrol::OkAuthUser(iClient* theClient, ccUser* theUser) {
  ...
  for (Itr = Clients.begin(); Itr != Clients.end(); Itr++) {
    ...
    if (tClient != theClient) // there is already a user authenticated under that nick
    {
      ...
      iClient* tClient = *Itr;
      Notice(tClient, "New authentification as you (%s) by %s", theUser->getUserName().c_str(),
...
void ccontrol::handleNewClient(iClient* NewUser) {
    ...
    if (irc_in_addr_valid(&NewUser->getIP())) // avoid 0:: (0.0.0.0) ip addresses
    {
      ...
      for (ipLretStructListType::iterator lItr = retList.begin(); lItr != retList.end(); lItr++) {
        ...
        if (!ipLRetVal && nb->isActive()) {
          ...
          for (ipLnbVectorType::iterator nItr = ipLnbVector.begin(); nItr != ipLnbVector.end();
               nItr++) {
            if (group) {
              nb = nItr->second;
            ...
            if (nb->isNoGline()) {
              ...
              if (rs.type != 'd') {
                ...
                if ((rs.type != 'd') &&
                    ((Itr = ExcessiveConnNotif.find(GlineMask)) == ExcessiveConnNotif.end() ||
                     (::time(0) - Itr->second.lastReportTS) >= ExcessConnReportInterval ||
                     (ipLconncount - Itr->second.lastReportCount) >= 100)) {
                  MsgChanLog(
                      "Excessive connections%s from subnet %s [ref: %s's %s] (will _NOT_ GLINE)",
              ...
              elog << "mod.ccontrol::HandleNewClient()> Gline exists on " << exGline->getHost()
                   << " and is <30 secs old. Not readding" << endl;
            } else {
              sprintf(Log, "Glining %s for %s connections%s [ref: %s's %s]", GlineMask, glineType,
                      glineLimit, nb->ipLisp->getName().c_str(), nb->getCidr().c_str());
              AffectedUsers = ipLconncount;
              /* set the gline reason */
              if (isUserban) {
                sprintf(GlineReason,
                        "AUTO [%d] %sAutomatically banned for excessive IDENT connections (%s@)%s",
                        AffectedUsers,
                        group ? string("[" + nb->ipLisp->getName() + "] ").c_str() : "",
                        user.c_str(), url_excessive_conn.c_str());
              } else if (isUnidentedBan) {
                sprintf(GlineReason,
                        "AUTO Please make sure identd is installed and properly configured on your "
                        "router and/or firewall to allow connections from the internet on port 113 "
                        "before you reconnect%s",
  ...
  if (!IsAuth(NewUser)) {
    ...
    // TODO: Fix auto auth after netsplit. It will only work if it's the latest client that authed
    // that splits.
      ...
      if ((tUser->getLastAuthTS() == NewUser->getNickTS()) &&
        ...
        if (AuthUser(tUser, NewUser))
          if (!(isSuspended(tUser)))
            ...
            Notice(NewUser,
                   "Automatic authentication after netsplit successful, however you are suspended",
                   tUser->getUserName().c_str());
        else
        ...
        for (Itr = Clients.begin(); Itr != Clients.end(); Itr++) {
          ...
          if (tClient != NewUser) // there is already a user authenticated under that nick
          {
            count++;
            if (count == 1) {
              Notice(NewUser, "The following clients are already authenticated as %s:",
                     tUser->getUserName().c_str());
            }
            iClient* tClient = *Itr;
            Notice(tClient, "New authentification as you (%s) by %s", tUser->getUserName().c_str(),
                   NewUser->getRealNickUserHost().c_str());
            StringTokenizer st(tClient->getServer()->getName(), '.');
...
bool ccontrol::AddOper(ccUser* Oper) {
  static const char* Main = "INSERT INTO opers "
                            "(user_name,password,access,saccess,last_updated_by,last_updated,flags,"
                            "server,isSuspended,suspend_expires,suspended_by,suspend_level,suspend_"
                            "reason,isUhs,isOper,isAdmin,isSmt,isCoder,GetLogs,NeedOp,Notice,"
                            "GetLag,LastPassChangeTS,Sso,Ssooo,AutoOp,Account,AccountTS) VALUES ('";
  ...
  theQuery << Main << escapeSQLChars(Oper->getUserName()) << "','"
           ...
           << escapeSQLChars(Oper->getServer()) << "' ," << (Oper->getIsSuspended() ? "'t'" : "'n'")
           << "," << Oper->getSuspendExpires() << ",'" << escapeSQLChars(Oper->getSuspendedBy())
           << "'," << Oper->getSuspendLevel() << ",'" << escapeSQLChars(Oper->getSuspendReason())
           << "'," << (Oper->isUhs() ? "'t'" : "'n'") << "," << (Oper->isOper() ? "'t'" : "'n'")
           << "," << (Oper->isAdmin() ? "'t'" : "'n'") << "," << (Oper->isSmt() ? "'t'" : "'n'")
           << "," << (Oper->isCoder() ? "'t'" : "'n'") << "," << (Oper->getLogs() ? "'t'" : "'n'")
           << "," << (Oper->getNeedOp() ? "'t'" : "'n'") << ","
           << (Oper->getNotice() ? "'t'" : "'n'") << "," << (Oper->getLag() ? "'t'" : "'n'")
...
bool ccontrol::CreateReport(time_t From, time_t Til) {
  ...
  tLogFile << "ccontrol log for command issued between " << convertToAscTime(From) << " and up til "
           << convertToAscTime(Til) << endl;
...
bool ccontrol::MailReport(const char* MailTo, const char* ReportFile) {
  ...
  sprintf(SendMail, "%s -f %s %s < Report.mail\n", Sendmail_Path.c_str(), CCEmail.c_str(), MailTo);
  ...
  for (ignoreIterator ptr = ignore_begin(); ptr != ignore_end(); ptr++) {
    ...
    if (tempLogin->getIgnoreExpires() > ::time(0)) {
      Notice(theClient, "Host: %s Expires: %s [%ld]", tempLogin->getIgnoredHost().c_str(),
             convertToAscTime(tempLogin->getIgnoreExpires()), (long)tempLogin->getIgnoreExpires());
    }
  }
  ...
  for (serversconstiterator ptr = serversMap_begin(); ptr != serversMap_end(); ++ptr) {
    ...
    if (tmpServer->getNetServer()) {
      ...
      Notice(theClient, "%3s (%4d) Name: %s Version: %s (\002*MISSING*\002)",
             tmpServer->getLastNumeric().c_str(), base64toint(tmpServer->getLastNumeric().c_str()),
             tmpServer->getName().c_str(), tmpServer->getVersion().c_str());
    } else {
...
bool ccontrol::glineChannelUsers(iClient* theClient, Channel* theChan, const string& reason,
  ...
  for (Channel::const_userIterator ptr = theChan->userList_begin(); ptr != theChan->userList_end();
    ...
    if (isIpOfOper(curIP)) {
      Notice(theClient, "%s opered globally before and uses the same IP this user uses: %s!%s@%s",
  ...
  if (foundException) {
    if (isGlinedException(theChan->getName()) > 0) {
      ...
      if (!(excludeChanWithOper && foundOper))
        Notice(theClient, "There is an exception for at least one host on the channel. Channel "
                          "G-line sent (forced)");
    } else if (!(excludeChanWithOper && foundOper)) {
      Notice(theClient, "There is an exception for at least one host on the channel. Send the "
                        "channel gline again to force.");
  ...
  void ccontrol::announce(iClient * theClient, const string& text) {
    ...
    iClient* newClient = new iClient(MyUplink->getIntYY(), yyxxx, AnnounceNick, "announce",
                                     "AAAAAA", getHostName(), getHostName(), "+ok", string(), 0, 0,
                                     string(), "Announcement Service.", ::time(0));
    assert(newClient != 0);
  ...
  bool ccontrol::listIpLExceptions(iClient * theClient) {
    ...
    Notice(theClient, "%-31s  %-5s  %-13s  %-9s  %-20s  %-10s", "IP", "Count", "Limit", "ISP count",
           "ISP", "Tags");
  ...
  bool ccontrol::ipLuserInfo(iClient * theClient, iClient * target) {
    ...
    */
    ...
    Notice(theClient, "--- Listing limits infos for %s [%s] ---", target->getNickUserHost().c_str(),
           ip.c_str());
  ...
  bool ccontrol::ipLipInfo(iClient * theClient, const string& the_ip) {
    ...
    for (ipLnbIterator nptr = ipLnbVector.begin(); nptr != ipLnbVector.end(); nptr++) {
      ...
      if (nb->ipLisp->isv6() != isv6)
        continue;
      if (match(nb->getCidr(), ip) == 0) {
        /* Only keep going if the cidr < the previous one matched, except for forcecount netblocks,
         * which show first in the list Note: ptr is of type ipLnbVectorType (pair<int, ccIpLnb*>)
         * that is always sorted with netblocks that have forcecount at the top with nptr->first =
         * 129, and all the others have nptr->first == ntpr->second->getCidr2()
         */
        if ((nptr->first < 129) &&
  ...
  bool ccontrol::ipLDropClient(iClient * theClient) {
    ...
    */
    ...
    for (ipLnbListType::iterator nptr = ipLnbList.begin(); nptr != ipLnbList.end(); nptr++) {
      ...
      string userip =
          theClient->getUserName() + "@" + (nb->ipLisp->isGroup() ? nb->ipLisp->getName() : m);
      if (this->getIntYY() ==
  ...
  bool ccontrol::insertIpLnb(iClient * theClient, const string& Cidr, int Isp, bool silent) {
    ...
    */
    ...
    if ((ipLIsp->isv6() == 0) && (isv6)) {
      Notice(theClient,
             "%s is an ipv4 ISP. You need to delete the ISP entirely (not only its netblocks) if "
             "you want to add ipv4 netblocks",
             ipLIsp->getName().c_str());
      return false;
    }
    if ((ipLIsp->isv6() == 1) && (!isv6)) {
      Notice(theClient,
             "%s is an ipv6 ISP. You need to delete the ISP entirely (not only its netblocks) if "
             "you want to add ipv6 netblocks",
             ipLIsp->getName().c_str());
      return false;
    }
    ...
    if ((atoi(st[1]) > clonecidr) && (!ipLIsp->isGroup())) {
      Notice(theClient, "%s's clone limit is per /%d. You can't add a netblock smaller than that",
      ...
      for (ipLnbIterator nptr = ipLnbVector.begin(); nptr != ipLnbVector.end(); nptr++) {
        ...
        if (tnb->getCidr() == Cidr) {
          ...
          if (clonecidr == tclonecidr) {
            Notice(theClient, "ISP %s already has clone matching for /%d on netblock %s",
        ...
        if (tisp == ipLIsp) {
          if (cidrmatch(Cidr, tnb->getCidr()) == 0) {
            Notice(theClient, "Can't add netblock %s: it's overlapping with %s for the same ISP",
    ...
    if (!silent) {
      if (tempIpLnb->isForcecount())
        Notice(theClient,
               "There are currently %c%d users%c online that match %s. %d users were reassigned "
               "from other netblocks",
               2, tempIpLnb->getCount(), 2, Cidr.c_str(), reassignedcount);
      else
        Notice(theClient,
               "There are currently %c%d users%c online that have %s as the narrowest match. %d "
               "users were reassigned from other netblocks",
               2, tempIpLnb->getCount(), 2, Cidr.c_str(), reassignedcount);
    }
    return true;
  }
  ...
  bool ccontrol::delIpLnb(iClient * theClient, const string& IspName, const string& Host,
    ...
    if (IpLnb == 0) {
      Notice(theClient, "Can't find ipL netblock for host %s associated to isp %s", Host.c_str(),
             IspName.c_str());
      return false;
    }
    ...
    if (!silent)
      Notice(theClient, "There were %c%d users%c online associated with %s's %s", 2, cList.size(),
             2, tIspName.c_str(), IpLnb->getCidr().c_str());
    delete IpLnb;
    for (list<iClient*>::iterator lItr = cList.begin(); lItr != cList.end(); lItr++) {
      ipLretStructListType retList;
  ...
  bool ccontrol::ipLcidrChangeCheck(iClient * theClient, ccIpLisp * isp, int newcidr) {
    ...
    if ((isp->isv6() == 0) && (newcidr > 32)) {
      Notice(theClient,
             "Invalid cidr for an ipv6 isp. It must be between 8-32. You'll have to delete the ISP "
             "and readd it if you want to remove the ipv6 tag for this isp");
      return false;
    }
    for (ipLnbIterator nptr = ipLnbVector.begin(); nptr != ipLnbVector.end(); nptr++) {
      ccIpLnb* nb = nptr->second;
    ...
    if (highCidr > newcidr) {
      Notice(theClient,
             "You have at least one netblock that matches connections for a /%d, you cannot set "
             "your limit per /%d"
  ...
  bool ccontrol::loadExceptions() {
    static const char Query[] =
        ...
        "glunidented,isgroup,maxidentlimit,nogline FROM ipLISPs ORDER BY id";
  ...
  size_t ccontrol::iauthXQCheck(iServer * theServer, const string& Routing, const string& Message) {
    ...
    if (ipLRetVal) {
      ...
      for (ipLretStructListType::const_iterator Itr = retList.begin(); Itr != retList.end();
        ...
        ipLretStruct r = *Itr;
        ccIpLnb* nb = r.nb;
        MsgChanLimits("R(%c): %s - %s (%d/%d) - ref: %s's %s", r.type, IP.c_str(), r.mask.c_str(),
                      r.count, r.limit, nb->getIpLisp()->getName().c_str(), nb->getCidr().c_str());
        // ss << "R(" << r.type << "): " << IP << " - " << r.mask << " (" << r.count << "/" <<
        // r.limit << ")" << " - ref: " << nb->getIpLisp()->getName() << "'s " << nb->getCidr();
