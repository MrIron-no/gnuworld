...
bool LISTCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (!strcasecmp(st[1].c_str(), "glines")) {
    ...
    bot->Notice(
        theClient,
        "Please use LIST nomodechannels to get a list of the channels which are marked as no mode");
    ...
    if (!strcasecmp(st[2].c_str(), "key")) {
      ...
      for (channelListT::iterator cptr = channelList.begin();
           (cptr != channelList.end()) && (count < 15); cptr++) {
        curChannel = *cptr;
        bot->Notice(theClient, "Channel: [%4d] %s", curChannel->size(),
                    curChannel->getName().c_str());
        count++;
      }
      bot->Notice(theClient, "--==[ End of 'list channels key %s' - %d found ]==--", st[3].c_str(),
                  channelList.size());
    } else if (!strcasecmp(st[2].c_str(), "modes")) {
      if (st.size() < 4) {
        bot->Notice(theClient,
                    "for 'list channels modes', you must supply the modes to match against!");
        bot->Notice(theClient,
                    "e.g. 'list channels modes +tns-m' to require +tns and require not to have +m");
        bot->Notice(theClient, "(other modes may be set unless explicitly not required as above)");
        return false;
      }
      if (st.size() > 4) {
        bot->Notice(theClient, "You supplied too many arguments.  arguments such as +k or +l here "
                               "may not contain the key or limit!");
        return false;
      }
      ...
      if (!isValid) {
        bot->Notice(theClient,
                    "The modes given are invalid (you supplied '%s', valid modes are klimnpstrD)",
                    st[3].c_str());
        return false;
      }
      /* ok, validated them */
      ...
      for (channelListT::iterator cptr = channelList.begin();
           (cptr != channelList.end()) && (count < 16); cptr++) {
        curChannel = *cptr;
        bot->Notice(theClient, "Channel: [%4d] %s", curChannel->size(),
                    curChannel->getName().c_str());
        count++;
      ...
      if (st.size() < 4) {
        bot->Notice(theClient,
                    "for 'list channels topic', you must supply the topic to search for!");
        return false;
      }
      ...
      if (st.size() < 4) {
        bot->Notice(
            theClient,
            "for 'list channels topicby', you must supply the nick!user@host mask to search for!");
        return false;
      }
      ...
      if (string::npos == atPos) {
        ...
        Mask += "!*@*";
        bot->Notice(theClient,
                    "Mask was not in nick!user@host format, re-writing mask to '%s'.  This may not "
                    "be what you want!",
                    Mask.c_str());
      } else {
        ...
        if ((string::npos == atPos2) || (atPos2 < atPos)) {
          ...
          Mask += "@*";
          bot->Notice(theClient,
                      "Mask was not in nick!user@host format, re-writing mask to '%s'.  This may "
