...
bool KICKCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (NULL == theChan) {
    bot->Notice(theClient, "Unable to find channel: %s", chanName.c_str());
    return true;
  }
  ...
  if (NULL == theChan->findUser(Target)) {
    bot->Notice(theClient,
                ...
                "on channel %s",
                st[1].c_str(), theChan->getName().c_str());
    return true;
  }
  ...
  bot->Notice(theClient, "Kicking %s from channel %s because %s", Target->getNickName().c_str(),
              chanName.c_str(), st.assemble(3).c_str());
