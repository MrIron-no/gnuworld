...
bool GCHANCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if ((st[pos].substr(0, 1) != "#") || (st[pos].size() > channel::MaxName)) {
    ...
    bot->Notice(theClient,
                "GCHAN: Please specify a legal channel name "
  ...
  if (gLength == 0) { // ok its a string, check if it marks permanent gline
    if (!strcasecmp(Length, "-per")) {
      ...
      bot->Notice(theClient, "No duration was set, using %d seconds by default", gLength);
      ResStart = 1;
    }
  } else if (st.size() < 4) {
    Usage(theClient);
