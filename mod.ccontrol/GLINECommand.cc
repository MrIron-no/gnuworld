...
bool GLINECommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (!tmpUser) {
    ...
    if ((gCheck & gline::FU_NEEDED_USERS) && (Ok)) {
      bot->Notice(theClient,
                  "This gline would affect more than %d users, please "
                  "use FORCEGLINE",
                  gline::MFGLINE_USERS);
      Ok = false;
    }
    if ((gCheck & gline::FU_NEEDED_TIME) && (Ok)) {
      bot->Notice(theClient,
                  "Please use FORCEGLINE to gline for more than "
                  "%d seconds",
                  gline::MFGLINE_TIME);
      Ok = false;
    }
    if ((gCheck & gline::FORCE_NEEDED_WILDTIME) && (Ok)) {
      bot->Notice(theClient,
                  "Wildcard gline for more than %d"
                  " seconds (or more than %d without ident), "
                  "please use FORCEGLINE instead",
                  gline::MGLINE_WILD_TIME, gline::MGLINE_WILD_NOID_TIME);
      Ok = false;
    }
    if ((gCheck & gline::HUH_IS_IP_OF_OPER) && (Ok)) {
      if (bot->isGlinedException(gHost) > 0) {
        bot->Notice(theClient,
                    "There is someone who previously opered from that host. G-line sent (forced)");
      } else {
        bot->Notice(theClient,
                    "There is someone who previously opered from that host (%s). Send the gline "
                    "again to force.",
                    bot->getLastNUHOfOperFromIP(hostName).c_str());
    ...
    if ((gCheck & gline::HUH_IS_EXCEPTION) && (Ok)) {
      if (bot->isGlinedException(gHost) > 0) {
        ...
        bot->Notice(theClient,
                    "There is an exception for that host. Send the gline again to force.");
        bot->addGlinedException(gHost);
        Ok = false;
      }
    }
    if (hostName.find('/') != string::npos) {
      ...
      if (!bot->getValidCidr(hostName, tCidr)) {
        bot->Notice(theClient, "Unwanted cidr format: %s  -  Suggestion: %s", hostName.c_str(),
                    tCidr.c_str());
        Ok = false;
      }
    }
