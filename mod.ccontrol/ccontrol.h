...
public:
   ...
   * Return true if this client is on the given channel.
   */
  virtual bool isOnChannel(const Channel* theChan) const { return isOperChan(theChan->getName()); }
  ...
  const unsigned int& getMaxUsers() const { return maxUsers; }

  const unsigned long& getDateMax() const { return dateMax; }

  const unsigned long& getCurUsers() const { return curUsers; }
   ...
   * chan structure.
   */
  inline const_operChanIterator operChan_begin() const { return operChans.begin(); }

  /**
   * Retrieve a constant iterator to the end of the oper chan
   * structure.
   */
  inline const_operChanIterator operChan_end() const { return operChans.end(); }

  /**
   * Retrieve the size of the operchan structure.
   */
  inline operChanSizeType operChan_size() const { return operChans.size(); }

  /**
   * Return true if the oper chan structure is empty.
   */
  inline bool operChan_empty() const { return operChans.empty(); }
   ...
   * table.
   */
  constCommandIterator command_begin() const { return commandMap.begin(); }

  /**
   * Retrieve a constant iterator to the end of the command table.
   */
  constCommandIterator command_end() const { return commandMap.end(); }

  /**
   * Retrieve a constant iterator to a command handler for the
   * given command token.
   */
  constCommandIterator findCommand(const string& theComm) const { return commandMap.find(theComm); }
   ...
   * table.
   */
  commandIterator command_begin() { return commandMap.begin(); }

  /**
   * Retrieve a mutable iterator to the end of the command table.
   */
  commandIterator command_end() { return commandMap.end(); }

  /**
   * Retrieve a mutable iterator to a command handler for the
   * given command token.
   */
  commandIterator findCommand(const string& theComm) { return commandMap.find(theComm); }

  typedef glineListType::iterator glineIterator;

  glineIterator gline_begin() { return glineList.begin(); }

  glineIterator gline_end() { return glineList.end(); }

  glineIterator rngline_begin() { return rnGlineList.begin(); }

  glineIterator rngline_end() { return rnGlineList.end(); }

  typedef ignoreListType::iterator ignoreIterator;

  ignoreIterator ignore_begin() { return ignoreList.begin(); }

  ignoreIterator ignore_end() { return ignoreList.end(); }
  ...
  clientsIpIterator clientsIp_begin() { return clientsIpMap.begin(); }

  clientsIpIterator clientsIp_end() { return clientsIpMap.end(); }

  typedef usersMapType::const_iterator usersconstiterator;

  typedef accountsMapType::const_iterator accountsconstiterator;

  usersconstiterator usersmap_begin() const { return usersMap.begin(); }

  usersconstiterator usersmap_end() const { return usersMap.end(); }

  accountsconstiterator accountsMap_begin() const { return accountsMap.begin(); }

  accountsconstiterator accountsMap_end() const { return accountsMap.end(); }

  typedef serversMapType::const_iterator serversconstiterator;

  serversconstiterator serversMap_begin() const { return serversMap.begin(); }

  serversconstiterator serversMap_end() const { return serversMap.end(); }

  allowedVersionsType VersionsList;

  typedef list<string>::iterator versionsIterator;

  badChannelsIterator badChannels_begin() { return badChannelsMap.begin(); }

  badChannelsIterator badChannels_end() { return badChannelsMap.end(); }

  /**
   * Retrieve the default length of time for glines.
   */
  inline const time_t& getDefaultGlineLength() const { return gLength; }
