...
bool ADDUSERCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (st[1].size() > User::MaxName) {
    bot->Notice(theClient,
                "Oper name can't be more than %d "
  ...
  if (theUser) {
    bot->Notice(theClient,
                "Oper '%s' already exists in my database, "
  ...
  if (!strcasecmp(st[2], "coder")) {
    ...
    bot->Notice(theClient, "Illegal oper type; types are: uhs, oper, admin, smt and coder");
    return false;
  }
  ...
  if (OperFlags < operLevel::ADMINLEVEL) {
    bot->Notice(theClient, "Sorry, but only admins+ can add new opers");
    return false;
  }

  if ((OperFlags < operLevel::SMTLEVEL) && (OperFlags <= NewFlags)) {
    bot->Notice(theClient, "Sorry, but you can't add an oper with a higher or "
                           "equal access to your own");
    return false;
  } else if (OperFlags < NewFlags) {
    bot->Notice(theClient, "Sorry, but you can't add an oper with a higher "
