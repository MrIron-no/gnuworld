...
bool ccGline::Insert() {
  ...
  {
    elog << "ccGline::DeleteOnInsert> SQL Failure: " << SQLDb->ErrorMessage() << endl;
  ...
  // if( PGRES_COMMAND_OK == status )
  {
    ...
    elog << "ccontrol::Gline::Insert> SQL Failure: " << SQLDb->ErrorMessage() << endl;
    return false;
  }
}
...
bool ccGline::Update() {
  ...
  // if( PGRES_COMMAND_OK == status )
  {
    ...
    elog << "ccontrol::Gline::Update> SQL Failure: " << SQLDb->ErrorMessage() << endl;
    return false;
  }
}
...
bool ccGline::loadData(int GlineId) {
  ...
  elog << "ccontrol::glineload> " << theQuery.str().c_str() << endl;

  if (!SQLDb->Exec(theQuery, true))
  // if( PGRES_TUPLES_OK != status )
  {
    elog << "ccGline::load> SQL Failure: " << SQLDb->ErrorMessage() << endl;
...
bool ccGline::loadData(const string& HostName) {
  ...
  // if( PGRES_TUPLES_OK != status )
  {
    elog << "ccGline::loadData> SQL Failure: " << SQLDb->ErrorMessage() << endl;
...
bool ccGline::Delete() {
  ...
  elog << "ccontrol::glineDelete> " << theQuery.str().c_str() << endl;
  ...
  // if( PGRES_COMMAND_OK == status )
  {
    ...
    elog << "ccGline::ccDelete> SQL Failure: " << SQLDb->ErrorMessage() << endl;
