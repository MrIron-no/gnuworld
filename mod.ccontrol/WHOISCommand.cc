...
bool WHOISCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (NULL == targetServer) {
    elog << "WHOISCommand> Unable to find server: " << Target->getIntYY() << endl;
    return false;
  }

  if ((Target->isModeX()) && (Target->isModeR())) {
    bot->Notice(theClient, "%s is %s (%s) [%s]", st[1].c_str(), Target->getNickUserHost().c_str(),
                Target->getRealNickUserHost().c_str(), xIP(Target->getIP()).GetNumericIP().c_str());
  ...
  if (Target->getNickTS() != Target->getFirstNickTS()) {
    bot->Notice(theClient, "%s has used its current nickname for %s [since %ld]", st[1].c_str(),
                Ago(Target->getNickTS()), Target->getNickTS());
  }

  bot->Notice(theClient, "%s has been connected for %s [since %ld]", st[1].c_str(),
              Ago(Target->getFirstNickTS()), Target->getFirstNickTS());
  ...
  if (Target->isOper()) {
    bot->Notice(theClient, "%s is an IRCoperator", st[1].c_str());
  }
  ...
  for (iClient::const_channelIterator ptr = Target->channels_begin(); ptr != Target->channels_end();
       ++ptr) {
    curChannel = "";
    ...
    for (curPlace = 0; curPlace < tChannel.size(); ++curPlace) {
      if (((tChannel[curPlace] > 1) && (tChannel[curPlace] < 4)) || (tChannel[curPlace] == 15) ||
          ((tChannel[curPlace] > 27) && (tChannel[curPlace] < 33)) || (tChannel[curPlace] == 22) ||
          (tChannel[curPlace] == char(160)) ||
          ((unsigned(tChannel[curPlace]) > 252) && (unsigned(tChannel[curPlace]) <= 254))) {
        hasCC = true;
