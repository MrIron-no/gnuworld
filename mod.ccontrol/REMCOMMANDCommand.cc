  ...
  for (list<ccUser*>::iterator Itr = ccList.begin(); Itr != ccList.end(); Itr++) {
    ...
    if ((Admin) && (strcasecmp(AClient->getServer(), theUser->getServer()))) {
      bot->Notice(theClient,
                  "You can only modify a user who is associated with the same server as you");
      return false;
    }
    ...
    if (!(theUser->gotAccess(Comm))) {
      if (!AllOpers && !AllAdmins && !AllSmts && !AllCoders) {
        bot->Notice(theClient, "%s doesn't have access to %s", theUser->getUserName().c_str(),
                    st[2].c_str());
        return false;
      } else
        continue;
    ...
    if (theUser->Update()) {
      bot->Notice(theClient, "Successfully removed the command from %s",
      ...
      bot->Notice(theClient, "Error while removing command from %s",
                  theUser->getUserName().c_str());
      if (!AllOpers && !AllAdmins && !AllSmts && !AllCoders)
        return false;
      else
