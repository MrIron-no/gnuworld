    ...
    if (!strcasecmp(st[pos], "-CClonesTime")) {
      if (st.size() < pos + 2) {
        bot->Notice(theClient,
                    "-CClonesTime must get the duration between announcements per netblock.");
        return true;
      }
      ...
      if (!bot->updateMisc("CClonesTime", g)) {
        ...
        bot->Notice(theClient, "%s was successfully updated to %s", st[pos].c_str(),
                    Duration((long)g));
      }
      pos += 2;
      ...
      if (g < 1800 || g > 172800) {
        bot->Notice(
            theClient,
            "-CClonesGTime value must be between 1800 and 172800 seconds (30 mins - 2 days).");
        return true;
      }
      ...
      if (!IsTimeSpec(st[pos + 1])) {
        bot->Notice(theClient,
                    "-GBInterval must get the number of seconds between each burst - e.g. 3 or 3s");
        return true;
      }
      int g = extractTime(st[pos + 1], 1);
      if (g < 0 || g > 10) {
      ...
      if (st.size() < pos + 2) {
        bot->Notice(theClient, "-GTime must get the duration for the excessive connections gline - "
                               "e.g. 900s, 30m, 1h or 1d.");
        return true;
      }
      if (!IsTimeSpec(st[pos + 1])) {
      ...
      if (st.size() < pos + 2) {
        bot->Notice(theClient,
                    "-SGline must get a yes/no answer indicating whether or not to save glines");
        return true;
      }
