...
public:
  inline static string getIdent(const events::eventType& theEvent) {
    string temp;
    if (theEvent & events::E_ERROR)
      temp += 'E';
    if (theEvent & events::E_WARNING)
      temp += 'W';
    if (theEvent & events::E_INFO)
      temp += 'I';
    if (theEvent & events::E_DEBUG)
      temp += 'D';
    if (theEvent & events::E_SQL)
      temp += 'S';
    if (theEvent & events::E_PROTOCOL)
      temp += 'P';
    return temp;
  }
