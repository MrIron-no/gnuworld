...
extern "C" {
xClient* _gnuwinit(const string& args) { return new nickserv(args); }
}
 ...
 */
nickserv::nickserv(const string& configFileName) : xClient(configFileName) {
    ...
    string dbString = "host=" + dbHost + " port=" + dbPort + " dbname=" + dbDb + " user=" + dbUser +
                      " password=" + dbPass;
...
nickserv::~nickserv() { delete nickservConfig; }
...
void nickserv::BurstChannels() {
    MyUplink->JoinChannel(this, consoleChannel,
                          nickservConfig->Require("consoleChannelModes")->second);
 ...
 */
void nickserv::OnCTCP(iClient* theClient, const string& CTCP, const string& Message, bool Secure) {
 ...
 */
void nickserv::OnPrivateMessage(iClient* theClient, const string& Message, bool) {
    if (!theClient->isModeR()) {
...
void nickserv::precacheUsers() {
    ...
    if (cacheCon->Exec(cacheQuery, true)) {
        ...
        elog << "*** [NickServ:precacheUsers] Error executing query: " << cacheCon->ErrorMessage()
             << std::endl;
        ::exit(0);
    }

    elog << "*** [NickServ:precacheUsers] Done. Loaded a total of " << sqlUserCache.size()
...
int nickserv::addToQueue(iClient* theClient) {
    QueueType::iterator queuePos = find(warnQueue.begin(), warnQueue.end(), theClient);
...
void nickserv::processQueue() {
    ...
    theLogger->log(logging::events::E_DEBUG, "Processing queue - %u entr%s.", warnQueue.size(),
                   (warnQueue.size() == 1) ? ("y") : ("ies"));
    ...
    for (QueueType::iterator queuePos = warnQueue.begin(); queuePos != warnQueue.end();) {
        ...
        if (theData->warned) {
            ...
            Notice(
                theClient,
                "You are using a registered nickname. Please login or you will be disconnected.");
            theData->warned = true;
            theStats->incStat("NS.WARN");
            theLogger->log(logging::events::E_INFO, "Warned: (" + theClient->getCharYYXXX() + ") " +
    ...
    for (QueueType::iterator queuePos = killQueue.begin(); queuePos != killQueue.end();
         queuePos++) {
        iClient* theClient = *queuePos;
        ...
        theLogger->log(logging::events::E_INFO,
                       "Killed: (" + theClient->getCharYYXXX() + ") " + theClient->getNickName());
    ...
    for (vector<string>::iterator itr = jupeQueue.begin(); itr != jupeQueue.end(); ++itr) {

        string theNick = *itr;

        theLogger->log(logging::events::E_DEBUG, "Juping " + theNick);
         ...
         */
        iClient* fakeClient =
            new iClient(MyUplink->getIntYY(), fakenumeric, theNick, "juped", "AAAAAA", "nick.name",
                        "nick.name", "+ikd", string(), 0, 0, string(), "Juped Nick", ::time(0));
...
void nickserv::setConsoleLevel(logging::events::eventType& newMask) {
    if (newMask < logging::events::E_MIN || newMask > logging::events::E_MAX) {
        theLogger->log(logging::events::E_WARNING,
...
sqlUser* nickserv::isRegistered(string theNick) {
    ...
    if (cachedUser == sqlUserCache.end())
        return 0;
