...
dbHandle* sqlManager::getConnection() {
  elog << "*** [sqlManager:getConnection] Attempting DB connection to: " << dbString << endl;

  dbHandle* tempCon = new (std::nothrow) dbHandle(bot, dbString.c_str());
  assert(tempCon != 0);

  if (tempCon->ConnectionBad()) {
    elog << "*** [sqlManager:getConnection] Unable to connect to DB: " << tempCon->ErrorMessage()
         << endl;
    ::exit(0);
  } else {
    elog << "*** [sqlManager:getConnection] Connection established to DB." << endl;
  }
