...
gnutest::gnutest(const string& fileName) : xClient(fileName) {
  ...
  helpTable.insert(std::make_pair("servop <chan> <nick>", "Op a nick in a channel as the server"));
  ...
  helpTable.insert(std::make_pair("removeclient <nick>", "Remove a fake client from the network"));
  helpTable.insert(std::make_pair("removeserver <name>", "Remove a fake server from the network"));
...
void gnutest::OnPrivateMessage(iClient* theClient, const string& message, bool) {
  ...
  if (st[0] == "shutdown") {
    ...
    for (helpTableType::const_iterator hItr = helpTable.begin(); hItr != helpTable.end(); ++hItr) {
...
void gnutest::OnFakeChannelMessage(iClient* srcClient, iClient* destClient, Channel* theChan,
  ...
  if (srcClient->getNickName() == "ripper_") {
    std::stringstream s;
    s << destClient->getCharYYXXX() << " P " << theChan->getName() << " :I agree with ripper_...";
    Write(s.str());
  }
}
...
void gnutest::spawnServer(iClient* requestingClient, const StringTokenizer& st) {
  ...
  string yyxxx("00]]]");
  iServer* newServer = new (std::nothrow) iServer(getIntYY(), yyxxx, name, ::time(0), description);
  assert(newServer != 0);
