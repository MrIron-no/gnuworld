...
static const char* queryHeader = "INSERT INTO levels (channel_id, user_id, access, flags, added, "
                                 "added_by, last_modif, last_modif_by, last_updated) ";
...
bool ADDUSERCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (level <= targetAccess) {
        bot->Notice(theClient, bot->getResponse(theUser, language::access_higher).c_str());
        return false;
    }

    if ((targetAccess <= 0) || (targetAccess > 999)) {
        bot->Notice(theClient, bot->getResponse(theUser, language::inval_access).c_str());
        return false;
    }
    ...
    if (!targetUser) {
        bot->Notice(theClient, bot->getResponse(theUser, language::not_registered).c_str(),
                    st[2].c_str());
        return false;
    }
    ...
    if (levelTest != 0) {
        bot->Notice(theClient, bot->getResponse(theUser, language::already_in_list).c_str(),
                    targetUser->getUserName().c_str(), theChan->getName().c_str(), levelTest);
        return false;
    }
    ...
    if (targetUser->getFlag(sqlUser::F_NOADDUSER)) {
        ...
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::no_adduser,
                             string("%s does not wish to be added to channels at this time."))
                .c_str(),
    ...
    if (theChan->getUserFlags() == 1)
        targetFlags = sqlLevel::F_AUTOOP;
    if (theChan->getUserFlags() == 2)
        targetFlags = sqlLevel::F_AUTOVOICE;
    ...
    theQuery << queryHeader << "VALUES (" << theChan->getID() << "," << targetUser->getID() << ","
             << targetAccess << "," << targetFlags << "," << bot->currentTime() << ","
             << "'" << escapeSQLChars(lastModifMask) << "'," << bot->currentTime() << ","
             << "'" << escapeSQLChars(lastModifMask) << "'," << bot->currentTime() << ");" << ends;

    if (bot->SQLDb->Exec(theQuery)) {
        bot->Notice(theClient, bot->getResponse(theUser, language::add_success).c_str(),
                    targetUser->getUserName().c_str(), theChan->getName().c_str(), targetAccess);

        if (targetUser != theUser)
            bot->NoteAllAuthedClients(
                targetUser,
                bot->getResponse(targetUser, language::acc_new,
                                 string("You have been added to channel %s with access level %i"))
                    .c_str(),
        ...
        newLevel->setLastModifBy("(" + theUser->getUserName() + ") " +
        ...
        if (level < 500) {
            string theMessage = TokenStringsParams(
                "%s added %s to channel %s with access level %i", theUser->getUserName().c_str(),
                targetUser->getUserName().c_str(), theChan->getName().c_str(), targetAccess);
