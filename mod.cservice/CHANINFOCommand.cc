...
bool CHANINFOCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (string::npos == st[1].find_first_of('#')) {
    ...
    if (adminAccess) {
      ...
      queryString << "SELECT message,ts"
                  ...
                  << " AND event = " << sqlUser::EV_COMMENT << " ORDER BY ts DESC LIMIT 3" << ends;
      ...
      if (theUser->getFlag(sqlUser::F_FRAUD)) {
        bot->Notice(theClient, "\002** This account has been tagged as being used in a fraudulent "
                               "channel application **\002");
      }
    ...
    if (adminAccess || (tmpUser == theUser)) {
      if ((tmpUser != theUser) && ((tmpadminLevel > 0 || theUser->getFlag(sqlUser::F_OPER))) &&
          (adminAccess < 800)) {
  ...
  if (!theChan) {
    ...
    theQuery << "SELECT id,status,manager_id,managername,pending.description,decision,created_ts "
    ...
    if (!bot->SQLDb->Exec(theQuery, true)) {
      ...
      */
      ...
      for (SuppDataListType::const_iterator itr = SuppDataList.begin(); itr != SuppDataList.end();
           itr++) {
  ...
  // if( PGRES_TUPLES_OK == status )
  {
    for (unsigned int i = 0; i < bot->SQLDb->Tuples(); i++) {
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::last_seen_info, string("%s - last seen: %s ago"))
              .c_str(),
  ...
  if (!theChan->getDescription().empty()) {
    bot->Notice(theClient, bot->getResponse(theUser, language::desc, string("Desc: %s")).c_str(),
  ...
  if ((adminAccess > 0) && (theChan->getFlag(sqlChannel::F_SUSPEND))) {
    ...
    queryString << "SELECT message,ts FROM channellog WHERE "
                << "channelid = " << theChan->getID() << " AND event = " << sqlChannel::EV_SUSPEND
    ...
    if (suspendreason != "") {
      bot->Notice(theClient, "Channel suspended %s ago by %s, Reason: %s",
                  prettyDuration(suspendts).c_str(), suspender.c_str(), suspendreason.c_str());
    }
  }
