...
static const char* queryHeader =
    "SELECT channels.name,users.user_name,levels.access,users_lastseen.last_seen FROM "
    "levels,channels,users,users_lastseen ";
static const char* queryString =
    "WHERE levels.channel_id=channels.id AND users.id=users_lastseen.user_id AND levels.access = "
    "500 AND levels.user_id = users.id ";
...
bool CHANINFOCommand::Exec(iClient* theClient, const string& Message) {
    ...
    int adminAccess = 0;
    if (tmpUser)
        adminAccess = bot->getAdminAccessLevel(tmpUser);
    ...
    if (string::npos == st[1].find_first_of('#')) {
        ...
        if (!theUser) {
            bot->Notice(theClient,
                        bot->getResponse(tmpUser, language::not_registered,
        ...
        if (adminAccess || (tmpUser == theUser)) {
            if (theUser->getFlag(sqlUser::F_TOTP_REQ_IPR) ||
                theUser->getFlag(sqlUser::F_TOTP_ENABLED)) {
        ...
        if (adminAccess || (tmpUser == theUser)) {
            ...
            autoInviteQuery << "SELECT channel_id from levels"
                            << " where user_id = " << theUser->getID() << " and flags & "
                            << sqlLevel::F_AUTOINVITE << " > 0"
                            << " and deleted = 0" << ends;
        ...
        if (theUser->getFlag(sqlUser::F_INVIS)) {
            ...
            if (!((tmpUser) && bot->getAdminAccessLevel(tmpUser)) && (tmpUser != theUser)) {
                ...
                sprintf(resBuf,
                        bot->getResponse(tmpUser, language::info_about,
                                         string("Information about: %s (%i)"))
                            .c_str(),
                        theUser->getUserName().c_str(), 0);
                bot->Notice(theClient, "%s - %s", resBuf,
                            bot->getResponse(tmpUser, language::no_peeking,
                ...
                bot->Notice(
                    theClient,
                    bot->getResponse(tmpUser, language::status_flags, string("Flags set: %s"))
                        .c_str(),
                    flagsSet.c_str());
        ...
        bot->Notice(
            theClient,
            bot->getResponse(tmpUser, language::info_about, string("Information about: %s (%i)"))
                .c_str(),
        ...
        if (theUser->getID() == 42) {
            bot->Notice(
                theClient,
                " - What do you mean you want to demolish the Undernet to make way for a new"
        ...
        for (sqlUser::networkClientListType::iterator ptr = theUser->networkClientList.begin();
            ...
            if ((tmpadminLevel > 0 || theUser->getFlag(sqlUser::F_OPER)) && adminAccess < 800) {
                bot->Notice(theClient, "  " + (*ptr)->getNickName() + "!" + (*ptr)->getUserName() +
                                           "@" + (*ptr)->getAccount() +
                                           (*ptr)->getHiddenHostSuffix());
            } else {
        ...
        if (!aCount)
            bot->Notice(theClient, "  OFFLINE");

        bot->Notice(
            theClient,
            bot->getResponse(tmpUser, language::status_flags, string("Flags set: %s")).c_str(),
            flagsSet.c_str());

        bot->Notice(theClient,
                    bot->getResponse(tmpUser, language::last_seen, string("Last Seen: %s")).c_str(),
        ...
        if (adminAccess) {
            ...
            queryString << "SELECT message,ts"
                        << " FROM userlog WHERE user_id = " << theUser->getID()
                        << " AND event = " << sqlUser::EV_COMMENT << " ORDER BY ts DESC LIMIT 3"
                        << ends;
            ...
            if (bot->SQLDb->Exec(queryString, true)) {
                for (unsigned int i = 0; i < bot->SQLDb->Tuples(); i++) {
                    ...
                    bot->Notice(theClient, "\002Admin Comment\002: %s ago (%s)",
                                prettyDuration(theTime).c_str(), userComments.c_str());
                }
            }

            if (theUser->getFlag(sqlUser::F_FRAUD)) {
                bot->Notice(theClient, "\002** This account has been tagged as being used in a "
                                       "fraudulent channel application **\002");
            }
            ...
            if (theUser->getFlag(sqlUser::F_GLOBAL_SUSPEND)) {
                ...
                bot->Notice(theClient, "Account suspended %s ago, Reason: %s",
                            prettyDuration(theTime).c_str(), reason.c_str());
            } else {
                /*
        ...
        if (adminAccess) {
            bot->Notice(theClient, "EMail: %s", theUser->getEmail().c_str());
        }

        if (adminAccess || (tmpUser == theUser)) {
            if ((tmpUser != theUser) &&
                ((tmpadminLevel > 0 || theUser->getFlag(sqlUser::F_OPER))) && (adminAccess < 800)) {
                bot->Notice(theClient, "Last Hostmask: Not Available");
            } else {
                bot->Notice(theClient, "Last Hostmask: %s", theUser->getLastHostMask().c_str());
                // Show ip only to admins
                if (adminAccess > 0) {
                    bot->Notice(theClient, "Last IP: %s", theUser->getLastIP().c_str());
                }
            }
        ...
        */
        if (adminAccess >= adminlevel::linkedusers) {
            ...
            linkedQuery << "SELECT * FROM get_linked_users(" << theUser->getID() << ")" << ends;
            ...
            if (bot->SQLDb->Exec(linkedQuery, true)) {
                if (bot->SQLDb->Tuples() > 0) {
                    ...
                    if (allUsernames.length() >= 2 && allUsernames[0] == '{' &&
                    ...
                    if (totalUsernames > 0) {
                        ...
                        string header =
                            "Suspected other usernames (" + std::to_string(totalUsernames) + "): ";
                        ...
                        for (size_t i = 0; i < usernames.size(); i++) {
                            ...
                            if (currentLine.length() + usernames[i].length() +
                                    (i < usernames.size() - 1 ? 2 : 0) >
                                400) {
    ...
    if (!theChan) {
        unsigned int lastdays = (unsigned int)bot->currentTime() -
                                (bot->getConfPendingsExpireTime() * bot->getConfdaySeconds());
        theQuery
            << "SELECT id,status,manager_id,managername,pending.description,decision,created_ts "
               "FROM channels,pending WHERE "
            << "registered_ts = 0 AND status <> 3 AND lower(channels.name) = '"
            << escapeSQLChars(string_lower(st[1])) << "'"
            << " AND channels.id = pending.channel_id AND (pending.decision_ts=0 OR "
        ...
        if (!bot->SQLDb->Exec(theQuery, true)) {
            ...
            if ((adminAccess > 0) || (tmpUser == mngrUser) ||
                (!mngrUser->getFlag(sqlUser::F_INVIS)))
                bot->Notice(theClient, "Applicant: %s - last seen: %s ago",
                            mngrUser->getUserName().c_str(),
            ...
            */
            ...
            for (SuppDataListType::const_iterator itr = SuppDataList.begin();
                 itr != SuppDataList.end(); itr++) {
                ...
                for (sqlUser::networkClientListType::iterator ptr =
                         suppUser->networkClientList.begin();
                     ptr != suppUser->networkClientList.end(); ++ptr) {
                    if (nick == "")
                        nick += "/" + (*ptr)->getNickName();
                    else
                        nick += " " + (*ptr)->getNickName();
            ...
            if (showsupplist) {
                ...
                if (adminAccess > 0) {
                    if (objCount > 0 || comCount > 0) {
                        bot->Notice(theClient, "Objections: %i -- Admin Comments: %i", objCount,
                                    comCount);
                    }
                } else if (objCount > 0)
            ...
            bot->Notice(theClient,
                        bot->getResponse(tmpUser, language::chan_not_reg,
                                         string("The channel %s is not registered"))
                            .c_str(),
                        st[1].c_str());
        }
        if (!theChan && adminAccess) {
            ...
            if (theChan) {
                string purgeReason =
                    bot->getLastChannelEvent(theChan, sqlChannel::EV_PURGE, bot->currentTime());
    ...
    theQuery.str("");
    theQuery << queryHeader << queryString << "AND levels.channel_id = " << theChan->getID()
             << ends;

    bot->Notice(theClient,
                bot->getResponse(theUser, language::reg_by, string("%s is registered by:")).c_str(),
                st[1].c_str());
    ...
    // if( PGRES_TUPLES_OK == status )
    {
        for (unsigned int i = 0; i < bot->SQLDb->Tuples(); i++) {
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::last_seen_info,
                                         string("%s - last seen: %s ago"))
                            .c_str(),
    ...
    if (theChan->getFlag(sqlChannel::F_SPECIAL) && !adminAccess)
        return true;

    if (!theChan->getDescription().empty()) {
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::desc, string("Desc: %s")).c_str(),
    ...
    if ((adminAccess > 0) && (theChan->getFlag(sqlChannel::F_SUSPEND))) {
        ...
        queryString << "SELECT message,ts FROM channellog WHERE "
                    << "channelid = " << theChan->getID()
                    << " AND event = " << sqlChannel::EV_SUSPEND
                    << " AND ts <= " << bot->currentTime() << " ORDER BY ts DESC LIMIT 1" << ends;
            ...
            if (bot->SQLDb->Tuples() > 0) {
                ...
                if (suspendst.size() > 5) {
                    if ((suspendst[2].substr(0, 1) == "(") &&
                        ...
                        suspender = suspendst[2].substr(1, suspendst[2].size() - 2);
                    else
        ...
        if (suspendreason != "") {
            bot->Notice(theClient, "Channel suspended %s ago by %s, Reason: %s",
                        prettyDuration(suspendts).c_str(), suspender.c_str(),
                        suspendreason.c_str());
        }
    }
    ...
    if (!theChan->getComment().empty() && adminAccess) {
        if ((tmpUser) && bot->getAdminAccessLevel(tmpUser)) {
            bot->Notice(theClient, "Comments: %s", theChan->getComment().c_str());
        }
    }

    if (!theChan->getKeywords().empty()) {
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::keywords, string("Keywords: %s")).c_str(),
                    theChan->getKeywords().c_str());
    }

    if (!theChan->getURL().empty()) {
        bot->Notice(theClient, bot->getResponse(theUser, language::url, string("URL: %s")).c_str(),
    ...
    if (!theChan->getWelcome().empty()) {
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::welcome_status, string("Welcome: %s")).c_str(),
