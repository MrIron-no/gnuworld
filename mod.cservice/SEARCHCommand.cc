...
bool SEARCHCommand::Exec(iClient* theClient, const string& Message) {
  ...
  for (unsigned int i = 0; i < bot->SQLDb->Tuples(); i++) {
    ...
    int chanMembers = -1;
    if (tmpChan)
      chanMembers = tmpChan->size();

    if (chanMembers >= chanLimit) {
      results++;
      bot->Notice(theClient, "\026%-14s \026 - %s (%i users)", bot->SQLDb->GetValue(i, 0).c_str(),
                  bot->SQLDb->GetValue(i, 1).c_str(), chanMembers);
    }

    if (results >= MAX_SEARCH_RESULTS) {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::exc_search,
                                   string("There are more than %i entries matching [%s]"))
                      .c_str(),
                  MAX_SEARCH_RESULTS, matchString.c_str());
      bot->Notice(theClient, bot->getResponse(theUser, language::restrict_search,
                                              string("Please restrict your search mask"))
                                 .c_str());
      break;
    }
  } // for()
  ...
  if (0 == results) {
    if (!chanLimit) {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::no_search_match,
                                   string("No matching entries for [%s]"))
                      .c_str(),
                  matchString.c_str());
    } else {
