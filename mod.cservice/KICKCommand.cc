  ...
  for (; counter < st2.size(); counter++) {
    ...
    if ((validUserMask(kickTarget)) && (level >= level::masskick)) {
      ...
      if (target->getMode(iClient::MODE_SERVICES)) {
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::wouldnt_appreciate,
                                     string("I don't think %s would appreciate that."))
                        .c_str(),
    ...
    if (level < 500)
      if (allmatched && validUserMask(kickTarget) && (theChan->getFlag(sqlChannel::F_NOTAKE))) {
        string theMessage =
            TokenStringsParams("Take over attempt by %s (%s) on channel %s with kickmask %s",
                               theClient->getNickName().c_str(), theUser->getUserName().c_str(),
                               theChan->getName().c_str(), kickTarget.c_str());
