...
bool VERIFYCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (!target) {
    bot->Notice(theClient,
                bot->getResponse(tmpUser, language::dont_see_them,
  ...
  if (target->getMode(iClient::MODE_SERVICES)) {
    bot->Notice(theClient,
                bot->getResponse(tmpUser, language::is_service_bot,
  ...
  if (target->isOper()) {
    extra = bot->getResponse(tmpUser, language::is_also_an_ircop, " and an IRC operator");
  }
  ...
  if (!theUser) {
    if (target->isOper()) {
      bot->Notice(
          theClient,
          bot->getResponse(tmpUser, language::is_an_ircop, string("%s is an IRC operator")).c_str(),
          target->getNickUserHost().c_str());
    } else {
      bot->Notice(
          theClient,
          bot->getResponse(tmpUser, language::is_not_logged_in, string("%s is NOT logged in."))
              .c_str(),
          target->getNickUserHost().c_str());
    }
    return false;
  ...
  if ((0 == level) && (0 == cLevel)) {
    bot->Notice(theClient,
                bot->getResponse(tmpUser, language::logged_in_as, string("%s is logged in as %s%s"))
                    .c_str(),
  ...
  if (level >= level::admin::base) {
    if (theUser->getFlag(sqlUser::F_ALUMNI)) {
      bot->Notice(
          theClient,
          bot->getResponse(tmpUser, language::is_cservice_alumni,
  ...
  if ((level >= level::admin::base) && (level <= level::admin::helper)) {
    bot->Notice(
        theClient,
        bot->getResponse(tmpUser, language::is_cservice_rep,
  ...
  if ((level > level::admin::helper) && (level <= level::admin::admin)) {
    bot->Notice(
        theClient,
        bot->getResponse(tmpUser, language::is_cservice_admin,
  ...
  if ((level > level::admin::admin) && (level <= level::admin::coder)) {
    bot->Notice(
        theClient,
        bot->getResponse(tmpUser, language::is_cservice_dev,
  ...
  if ((cLevel >= level::coder::base) && (cLevel <= level::coder::contrib))
          ...
          bot->Notice(theClient,
                  bot->getResponse(tmpUser,
                          ...
                          string("%s is an Official Coder-Com Representative%s and logged in as
  %s")).c_str(), target->getNickUserHost().c_str(), extra.c_str(), theUser->getUserName().c_str());
          return true;
          }
  ...
  if ((cLevel > level::coder::base) && (cLevel <= level::coder::contrib))
          ...
          bot->Notice(theClient,
                  bot->getResponse(tmpUser,
                          ...
                          string("%s is an Official Coder-Com Contributer%s and logged in as
  %s")).c_str(), target->getNickUserHost().c_str(), extra.c_str(), theUser->getUserName().c_str());
          return true;
          }
  ...
  if ((cLevel > level::coder::contrib) && (cLevel <= level::coder::devel))
          ...
          bot->Notice(theClient,
                  bot->getResponse(tmpUser,
                          ...
                          string("%s is an Official Coder-Com Developer%s and logged in as
  %s")).c_str(), target->getNickUserHost().c_str(), extra.c_str(), theUser->getUserName().c_str());
          return true;
          }
  ...
  if (cLevel > level::coder::devel)
          ...
          bot->Notice(theClient,
                  bot->getResponse(tmpUser,
                          ...
                          string("%s is an Official Coder-Com Senior%s and logged in as
  %s")).c_str(), target->getNickUserHost().c_str(), extra.c_str(), theUser->getUserName().c_str());
          return true;
          }
  */
