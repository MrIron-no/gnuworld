...
bool STATUSCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (tmpChan) {
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::status_chan_info,
                                     string("Channel %s has %d users (%i operators)"))
                        .c_str(),
        ...
        if ((level >= level::status2) || (admLevel >= 1) || theClient->isOper()) {
            bot->Notice(
                theClient,
                bot->getResponse(theUser, language::status_mode, string("Mode is: %s")).c_str(),
    ...
    if (admLevel > 0) {
        ...
        if (theChan->getFlag(sqlChannel::F_NOPURGE))
            flagsSet += "NOPURGE ";
        if (theChan->getFlag(sqlChannel::F_SPECIAL))
            flagsSet += "SPECIAL ";
        if (theChan->getFlag(sqlChannel::F_NOREG))
            flagsSet += "NOREG ";
        if (theChan->getFlag(sqlChannel::F_NEVREG))
            flagsSet += "NEVERREG ";
        if (theChan->getFlag(sqlChannel::F_CAUTION))
            flagsSet += "CAUTION ";
        if (theChan->getFlag(sqlChannel::F_LOCKED))
            flagsSet += "LOCKED ";
        if (theChan->getFlag(sqlChannel::F_MIA))
            flagsSet += "MIA ";
        if (theChan->getFlag(sqlChannel::F_NOFORCE))
            flagsSet += "NOFORCE ";
    }
    ...
    if (theChan->getFlag(sqlChannel::F_TEMP))
        flagsSet += "TEMP ";
    if (theChan->getFlag(sqlChannel::F_VACATION))
        flagsSet += "VACATION ";
    if (theChan->getFlag(sqlChannel::F_ALWAYSOP))
        flagsSet += "ALWAYSOP ";
    ...
    if (((level >= level::set::oplog) || (admLevel)) && (theChan->getFlag(sqlChannel::F_OPLOG)))
        flagsSet += "OPLOG ";

    if (((level >= level::set::notake) || (admLevel)) && (theChan->getFlag(sqlChannel::F_NOTAKE))) {
        flagsSet += "NOTAKE (\002REV:\002";
        if (theChan->getNoTake() == 1)
            flagsSet += "NONE)";
        if (theChan->getNoTake() == 2)
            flagsSet += "BAN)";
        if (theChan->getNoTake() == 3)
            flagsSet += "SUSPEND)";
    }

    bot->Notice(theClient,
                bot->getResponse(theUser, language::status_flags, string("Flags set: %s")).c_str(),
                flagsSet.c_str());
    ...
    if (((level >= level::set::floodpro) || (admLevel)) &&
        ...
        floodperiods
            ...
            << " -- (\002MSG:\002" << theChan->getFloodMsg() << ", \002NOTICE:\002"
            << theChan->getFloodNotice() << ", \002CTCP:\002" << theChan->getFloodCTCP()
            << ", \002REP:\002" << theChan->getRepeatCount() << ", \002PERIOD:\002"
            << theChan->getFloodPeriod() << ")" << ends;
    ...
    if (theChan->getFlag(sqlChannel::F_FLOATLIM)) {
        ...
        floatLim << " FLOATLIM (\002MGN:\002" << theChan->getLimitOffset() << ", \002PRD:\002"
                 << theChan->getLimitPeriod() << ", \002GRC:\002" << theChan->getLimitGrace()
                 << ", \002MAX:\002" << theChan->getLimitMax() << ") " << ends;
    ...
    if (theChan->getFlag(sqlChannel::F_JOINLIM)) {
        ...
        joinLim << " JOINLIM (\002MAX:\002" << theChan->getLimitJoinMax() << ", \002SECS:\002"
                << theChan->getLimitJoinSecs() << ", \002PRD:\002" << theChan->getLimitJoinPeriod()
                << ", \002MODE:\002 " << theChan->getLimitJoinMode().c_str() << ")" << ends;
    ...
    if (theChan->getLimitJoinActive())
        bot->Notice(
            ...
            bot->getResponse(theUser, language::status_joinlim,
                             string("Active JOINMODE modes (+%s) will expire in %i seconds"))
                .c_str(),
    ...
    flagsSet.clear();
    if (theChan->getUserFlags() == 1)
        flagsSet += " USERFLAGS=OP ";
    else if (theChan->getUserFlags() == 2)
        flagsSet += " USERFLAGS=VOICE ";
    ...
    authQuery << "SELECT users.user_name,levels.access FROM "
              ...
              << "AND levels.channel_id = " << theChan->getID() << " ORDER BY levels.access DESC"
              << ends;
    ...
    for (unsigned int i = 0; i < bot->SQLDb->Tuples(); i++) {
        ...
        if (currentUser) {
            ...
            for (sqlUser::networkClientListType::iterator ptr =
                     (*currentUser)->networkClientList.begin();
                ...
                if (tmpChan)
                    showNick = (tmpChan->findUser(tmpClient) || admLevel);
    ...
    if (admLevel >= 1) {
        for (sqlChannel::forceMapType::const_iterator ptr = theChan->forceMap.begin();
            ...
            bot->Notice(theClient, "Force: %s (%i)", ptr->second.second.c_str(), ptr->second.first);
        }
    }
