...
enum class AuthType { LOGIN, XQUERY };
...
public:
  ...
  inline const string& asString() const { return string_value; }

  inline const unsigned int& asInt() const { return int_value; }
};
...
protected:
   ...
   */
  inline void clearConfigVariables() { return cserviceConfig->Clear(); }

  inline bool parseConfigFile() { return cserviceConfig->openConfigFile(); }
  /*
...
#undef CONFIG_VAR
  ...
  inline size_t getConfigVariables_size() { return cserviceConfig->size(); }

  inline size_t getConfigData_size() { return configTable.size(); }

  inline const statsMapType& getStatsMap() const { return statsMap; }
  ...
  void removeFP(const string& fingerprint) { fingerprintMap.erase(fingerprint); }
  ...
  const sqlLevelHashType& getLevelCache() const { return sqlLevelCache; }

  /* Insert an entry to the sqlLevelCache. */
  inline void insertLevelCache(sqlLevel* theLevel) {
    sqlLevelCache.emplace(std::make_pair(theLevel->getUserId(), theLevel->getChannelId()),
                          theLevel);
  }
  ...
  inline const string& getCurrentValidResponse() const { return validResponseString; }
   ...
   * channel */
  bool doSingleBan(sqlChannel* theChan, const string& banMask, unsigned short banLevel,
                   unsigned int banExpire, const string& theReason);

  /* Bans and kick just one client, with level, period, reason */
  bool doSingleBanAndKick(sqlChannel*, iClient*, unsigned short, unsigned int,
  ...
  bool doInternalBanAndKick(sqlChannel*, iClient*, unsigned short, unsigned int,
                            const string& theReason);

  /* Bans & kicks all client belonging to a specified banMask, with a specific level, period, reason
   */
  bool doInternalBanAndKick(sqlChannel*, const string&, unsigned short, unsigned int,
                            const string&);

  /* Suspends a specified user with a specific level, period, reason */
  bool doInternalSuspend(sqlChannel*, iClient*, unsigned short, unsigned int,
                         const string& theReason);
  ...
  bool KickBanAllWithFloodMessage(Channel*, const string&, unsigned short, unsigned int,
                                  const string&);
