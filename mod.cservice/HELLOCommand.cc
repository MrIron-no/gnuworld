...
bool HELLOCommand::Exec(iClient* theClient, const string& Message) {
  ...
  for (string::const_iterator ptr = theUserName.begin(); ptr != theUserName.end(); ++ptr) {
    /*
  ...
  if ((emailCheck.size() != 2) || (string::npos == emailCheck[1].find_first_of('.'))) {
    bot->Notice(theClient, "Invalid e-mail address.");
    return false;
  }
  ...
  bot->Notice(theClient, "I generated this password for you: \002%s\002", plainpass.c_str());
  bot->Notice(theClient, "Login using \002/msg %s@%s LOGIN %s %s\002", bot->getNickName().c_str(),
              bot->getUplinkName().c_str(), st[1].c_str(), plainpass.c_str());
  bot->Notice(theClient,
              "Then change your password using \002/msg "
  ...
  bot->helloIPList.insert(std::make_pair(xIP(theClient->getIP()).GetNumericIP(true),
