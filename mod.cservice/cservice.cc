...
#endif
{
    ...
    RegisterCommand(new ACCESSCommand(
        this, "ACCESS", "[channel] [username] [-min n] [-max n] [-op] [-voice] [-none] [-modif]",
        5));
    ...
    RegisterCommand(new MODINFOCommand(this, "MODINFO",
                                       "<#channel> [ACCESS <username> <level>] [AUTOMODE "
                                       "<username> <NONE|OP|VOICE>] [INVITE <ON|OFF>]",
                                       6));
    ...
    RegisterCommand(new SUSPENDCommand(this, "SUSPEND",
                                       "<#channel> <username> <duration> [level] [reason]", 5));
    ...
    RegisterCommand(new SCANHOSTCommand(
        this, "SCANHOST", "<mask> [-sort <user|cmd|host|ip|time>] [-max n] [-order <asc|desc>]",
        10));
    ...
    if (SQLDb->Exec("SELECT date_part('epoch', CURRENT_TIMESTAMP)::int;", true)) {
        ...
        elog << "*** [CMaster::ImplementServer]:  Current DB server time: " << currentTime()
             << endl;
    } else {
...
void cservice::BurstChannels() {
    ...
    while (ptr != sqlChannelCache.end()) {
        ...
        if (theChan->getFlag(sqlChannel::F_AUTOJOIN)) {
            ...
            /* If FLOATLIM is activated, we don't set +l. The correct limit will be set shortly
             * after burst. */
            if (theChan->getFlag(sqlChannel::F_FLOATLIM))
                stripModes(tempModes, "l");

            MyUplink->JoinChannel(this, theChan->getName(), tempModes, theChan->getChannelTS(),
                                  true);
...
void cservice::OnPrivateMessage(iClient* theClient, const string& Message, bool secure) {
    ...
    if (commHandler == commandMap.end()) {
        ...
        setFloodPoints(theClient,
                       getFloodPoints(theClient) + commHandler->second->getFloodPoints());
        ipFloodMap[xIP(theClient->getIP()).GetNumericIP(true)] +=
            commHandler->second->getFloodPoints();

        sqlUser* theUser = isAuthed(theClient, false);

        /* Check IP restriction (if admin level) - this is in case you get added as admin AFTER
         * logging in */
...
void cservice::OnCTCP(iClient* theClient, const string& CTCP, const string& Message, bool) {
    ...
    if (Command == "PING" || Command == "ECHO") {
        ...
        xClient::DoCTCP(theClient, CTCP,
                        "Undernet P10 Channel Services II [" __DATE__ " " __TIME__
                        "] Release 2.0.0B1");
...
void cservice::loadHelpTable() {
    ...
    elog << "*** [CMaster::loadHelpTable]: Loaded " << helpTable.size() << " help messages."
         << endl;
}
    ...
    // if( PGRES_TUPLES_OK == status )
    {
        for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
            ...
            translationTable.insert(
                translationTableType::value_type(thePair, SQLDb->GetValue(i, 2)));
        }
    }
...
int cservice::rehashMOTD() {
    ...
    if (ptr != translationTable.end()) {
        ...
        if (SQLDb->Exec(selectQuery, true)) {
            translationTable.insert(
                translationTableType::value_type(thePair, SQLDb->GetValue(0, 0)));
        ...
        for (const auto& [chanID, banID] : expireVector) {
            ...
            if (ptr != theChan->banList.end()) {
                ...
                if (tmpChan && tmpChan->findBan(theBan->getBanMask())) {
                    banVector.push_back(
                        xServer::banVectorType::value_type(false, theBan->getBanMask()));
                }
...
vector<sqlUser*> cservice::getChannelManager(int channelId) {
    ...
    for (sqlLevelHashType::iterator itr = sqlLevelCache.begin(); itr != sqlLevelCache.end();
         ++itr) {
...
bool cservice::sqlRegisterChannel(iClient* theClient, sqlUser* mngrUsr, const string& chanName) {
    ...
    theQuery.str("");
    theQuery << "DELETE FROM pending_chanfix_scores WHERE channel_id = " << newChan->getID()
             << ends;
    ...
    logAdminMessage("%s (%s) has registered %s to %s", theClient->getNickName().c_str(),
                    theUser->getUserName().c_str(), chanName.c_str(),
                    mngrUsr->getUserName().c_str());
     ...
     */
    if (theClient != getInstance())
        writeChannelLog(newChan, theClient, sqlChannel::EV_REGISTER,
                        "to " + mngrUsr->getUserName());
    ...
    if (theClient == getInstance()) {
        ...
        newManager->setLastModifBy("(" + theUser->getUserName() + ") " +
...
void cservice::checkValidUsersAndChannelsState() {
    ...
    if (!chanList.empty()) {
        ...
        for (size_t i = 0; i < chanList.size(); ++i) {
            ...
            // logDebugMessage("Checking channel %s's Manager validity
            // ...",chanList[i].second.c_str());
    ...
    while (ptr != pendingChannelList.end()) {
        ...
        if (actualMinSupporters < MinSupporters) {
            ...
            if (elapsedDays < currentTime())
               ...
               // joincount
        {
            sqlPendingChannel::trafficListType::iterator ptr2 =
                pendingChan->uniqueSupporterList.begin();
            while (ptr2 != pendingChan->uniqueSupporterList.end()) {
                if (ptr2->second->join_count < MinSupportersJoin) {
                    ...
                    if (elapsedDays < currentTime())
                        logTheJudgeMessage("Insufficient supporter joincount of supporter user %s "
                                           "(%i/%i) on pending channel %s",
                                           suppUserName.c_str(), actualMinSupportersJoin,
                                           MinSupportersJoin, ptr->first.c_str());
                }
                ++ptr2;
            } // end while
        } // end else
        ...
        if (elapsedDays < currentTime()) {
            ...
            if ((!minSupportersPass) || (!minSupportersJoinPass))
                rejectReason = "Insufficient supporter activity. *All* supporters need to be "
                               "active users of the channel";
...
void cservice::checkObjections() {
    ...
    int actualChan = 0;
    stringstream theQuery;
    theQuery << "SELECT channels.name,channels.id,users.user_name FROM "
                "channels,pending,users,objections "
             << "WHERE channels.id = pending.channel_id "
    ...
    if (!SQLDb->Exec(theQuery, true)) {
        ...
        for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
            ...
            if (chanId != actualChan) // more than one objection per channel lead to multiple db
                                      // hit, so we need to limit to one result
            {
                actualChan = chanId;
    ...
    if (!objectList.empty())
        for (unsigned int i = 0; i < objectList.size(); i++) {
            ...
            NoteAllAuthedClients(mgrUsr,
                                 "Your channel application of %s is now at state Ready to review",
                                 objectList[i].first.second.c_str());
        }
    objectList.clear();
    return;
    ...
    if (!acceptList.empty())
        for (unsigned int i = 0; i < acceptList.size(); i++) {
            ...
            if (DecideOnCompleted || (reviewed == false && RequireReview)) {
                ...
                NoteAllAuthedClients(mgrUsr,
                                     "Your channel application of %s is now in 'Ready to review'",
                                     acceptList[i].first.second.c_str());
...
void cservice::OnEvent(const eventType& theEvent, void* data1, void* data2, void* data3,
    ...
    case EVT_XREPLY: {
        ...
        const char* Routing = reinterpret_cast<char*>(data2);
        const char* Message = reinterpret_cast<char*>(data3);
        // elog << "CSERVICE.CC XREPLY: " << theServer->getName() << " " << Routing << " " <<
        // Message << endl; As it is possible to run multiple GNUWorld clients on one server, first
...
bool cservice::doInternalBanAndKick(sqlChannel* theChan, iClient* theClient,
    ...
    if (!clientsToKick.empty()) {
        for (vector<iClient*>::iterator itr = clientsToKick.begin(); itr != clientsToKick.end();
             ++itr)
...
bool cservice::KickBanAllWithFloodMessage(Channel* theChan, const string& Message,
    ...
    for (; itr != MaskList.end(); itr++) {
         ...
         * otherwise any remaining items won't be banned on ChannelMessage because they don't reach
         * the RepeatCount to trigger this function
         */
    ...
    for (suppItr = suppIdList.begin(); suppItr != suppIdList.end(); ++suppItr) {
        ...
        if (suppItr->second.size() == 1) {
            ...
            message << "You are listed as a supporter for registration application for "
                    << currItr->chanName
                    << ". To progress the channel registration application you required to lodge "
                       "your support of the registrant "
                    << "(" << currItr->mngrUserName << "). "
                    << "Please type: \002/msg " << this->getNickName() << " support "
                    << currItr->chanName << " <yes|no>\002" << endl;

            if ((FirstNoticing) && (!currItr->noticed)) {
                noticeAllAuthedClients(suppUser, message.str().c_str());
                LOG_MSG(DEBUG, "setSupporterNoticedStatus for chanId={} userId={user_id}",
                        currItr->chanId)
                    .with("user", suppUser)
                    .logStructured();
            ...
            if (((FirstNoticing) && (!currItr->noticed)) ||
                ...
                noticeAllAuthedClients(suppUser,
                                       "You are listed as a supporter of the channel application "
                                       "for the following channels: %s",
                                       chanEnum.str().c_str());
                message << "To progress the channel registration application you required to lodge "
                           "your support of the registrant. "
    ...
    for (Channel::userIterator userItr = theChan->userList_begin();
        ...
        if (isSupporter) {
            ...
            if (Supptr == pendingChan->supporterList.end()) {
                ...
                // << " suppUser = " << loggedUser->getUserName() << " inserting with joincount 1 "
                // << endl;
        ...
        if (totalUsers > 50) {
            ...
            LOG_MSG(INFO,
                    "Weird large new channel application of {chan_name}, usercount={}, clones={}",
        ...
        if (SQLDb->ConnectionBad()) {
            ...
            if (connectRetries == connectRetry) {
                logAdminMessage("Unable to contact database after %d attempts, shutting down.",
                                connectRetry);
                LOG(FATAL, "Unable to contact database after {} attempts, shutting down.",
                    connectRetry);
...
void cservice::preloadLevelsCache() {
    ...
    elog << "*** [CMaster::preloadLevelCache]: Done. Loaded " << goodCount
         << " level records out of " << SQLDb->Tuples() << "." << endl;
}
...
void cservice::NoteAllAuthedClients(sqlUser* theUser, const char* Message, ...) {
    if (Connected && MyUplink && Message && Message[0] != 0) {
        ...
        static const char* queryHeader =
            "INSERT INTO notices (user_id,message,last_updated) VALUES (";
        queryString.str("");
...
void cservice::doCoderStats(iClient* theClient) {
    ...
    Notice(theClient, "--- Total users in DB with TLS: %i (fingerprints in cache %i)",
           userDBFPTotal, fingerprintMap.size());
...
/* Load prometheus settings if enabled. */
    ...
    if (prometheusEnable) {
        if (rehash) {
            ...
            prometheusPort =
                cserviceConfig->TryRequire<unsigned int>("prometheus_port", prometheusPort);
        } else {
    ...
    /* Nothing configures logging here any more: where this module's records go,
     * at what level, and whether a statement of its database handle is among
     * them, is what logging.conf says about the logger "cservice" */
}
...
cservice::AuthResult cservice::authenticateUser(AuthStruct& auth) {
     ...
     * regular login.
     *
     * If the client is authenticating with EXTERNAL, the auth fails if the fingerprint does not
     * match the username.
     *
     */
    ...
    if (totpAuthEnabled && theUser->getFlag(sqlUser::F_TOTP_ENABLED)) {
        /* If the user has a certificate matching the username or we are using SCRAM, the password
         * is the TOTP token. */
        ...
        /* If the user does not have a certificate matching the username and we are not using SCRAM,
           the second part of the password is the TOTP token. */
    ...
    /*
     * 9: Check password.
     * Password will not be checked for EXTERNAL or SCRAM or when a matching certificate is
     * provided.
     */
...
/* 10: Check TOTP token. */
    ...
    if (totp_enabled) {
        char* key;
        size_t len;
        int res = oath_base32_decode(theUser->getTotpKey().c_str(), theUser->getTotpKey().size(),
                                     &key, &len);
        if (res != OATH_OK)
            return AUTH_ERROR;
...
bool cservice::processAuthentication(AuthStruct auth, std::string* Message) {
    ...
    case cservice::AUTH_UNKNOWN_USER:
        authResponse = TokenStringsParams(
            getResponse(NULL, language::not_registered, string("AUTHENTICATION FAILED as %s"))
                .c_str(),
            auth.username.c_str());
        setFailedLogin = true;
        break;
    ...
    case cservice::AUTH_INVALID_PASS:
        ...
        if ((max_failed_logins > 0) && (auth.theUser->getFailedLogins() > max_failed_logins) &&
            ...
            logPrivAdminMessage("%d failed logins for %s (last attempt by %s%s, checksum %d).",
                                ...
                                auth.theUser->getUserName().c_str(), clientMask.c_str(),
                                auth.type == AuthType::XQUERY ? " (LoC)" : "", checksum);
        }
        setFailedLogin = true;
        break;
    ...
    case cservice::AUTH_FAILED_IPR:
        authResponse =
            TokenStringsParams("AUTHENTICATION FAILED as %s (IPR)", auth.username.c_str());
    ...
    case cservice::AUTH_ML_EXCEEDED:
        ...
        if (auth.theClient) {
            string clientList;
            for (sqlUser::networkClientListType::iterator ptr =
                     auth.theUser->networkClientList.begin();
    ...
    case cservice::AUTH_FAIL_EXCEEDED:
        authResponse =
            TokenStringsParams(getResponse(NULL, language::max_failed_logins,
                                           string("AUTHENTICATION FAILED as %s (Exceeded maximum "
                                                  "login failures for this session)"))
                                   .c_str(),
                               auth.username.c_str());
        setFailedLogin = true;
        break;
    ...
    /**
     * If theClient is nullptr, it means that the authentication is done by SASL/XQUERY and auth
     * will occurr when the client is introduced to the network.
     */
    else {
        *Message = authResponse;
    }
...
bool cservice::doXQSASL(iServer* theServer, const string& Routing, const string& Message) {
    ...
    if (it != saslRequests.end()) {
        ...
        if (it->mechanism == SaslMechanism::PLAIN) {
            ...
            if (p1 == std::string::npos || p2 == std::string::npos || p1 == 0) {
                elog << "[PLAIN] Invalid PLAIN format in decoded credentials: " << decodedString
                     << endl;
                incStat("SASL." + saslMechanismToString(it->mechanism) + ".ERROR");
                doXResponse(theServer, Routing, "Invalid credentials", true);
                saslRequests.erase(it);
            ...
            if (it->credentials.empty()) {
                incStat("SASL." + saslMechanismToString(it->mechanism) + ".FAILED");
                doXResponse(theServer, Routing,
                            "Username required for SASL EXTERNAL authentication", true);
                saslRequests.erase(it);
                return false;
            }
            ...
            if (it->state == SaslState::INITIAL) {
                auto bufferOpt = b64decode(it->credentials, nullptr, true);
                if (!bufferOpt) {
                    elog << "[SCRAM] Failed to decode client first message: " << it->credentials
                         << endl;
                    incStat("SASL." + saslMechanismToString(it->mechanism) + ".ERROR");
                    doXResponse(theServer, Routing, "Invalid credentials", true);
                    saslRequests.erase(it);
                ...
                if (it->username.empty() || it->client_nonce.empty()) {
                    elog << "[SCRAM] Missing username or client nonce in authentication: "
                         << decodedString << endl;
                    incStat("SASL." + saslMechanismToString(it->mechanism) + ".ERROR");
                    doXResponse(theServer, Routing, "Invalid credentials", true);
                    saslRequests.erase(it);
                ...
                << " "; elog << std::dec << "\n"; elog << "ServerKey (hex): "; for (auto c :
                it->scram.serverKey) elog << std::hex << (int)c << " "; elog << std::dec << "\n";*/
                ...
                auto bufferOpt = b64decode(it->credentials, nullptr, true);
                if (!bufferOpt) {
                    elog << "[SCRAM] Failed to decode client final message: " << it->credentials
                         << endl;
                    incStat("SASL." + saslMechanismToString(it->mechanism) + ".ERROR");
                    doXResponse(theServer, Routing, "Invalid credentials", true);
                    saslRequests.erase(it);
                ...
                if (p_pos == std::string::npos) {
                    elog << "[SCRAM] Missing ,p= in client final message: " << decodedString
                         << endl;
                    incStat("SASL." + saslMechanismToString(it->mechanism) + ".ERROR");
                    doXResponse(theServer, Routing, "Invalid credentials", true);
                    saslRequests.erase(it);
                ...
                if (cbind_input.empty() || nonce.empty() || proof.empty()) {
                    elog << "[SCRAM] Missing c=, r= or p= in client final message: "
                         << decodedString << endl;
                    incStat("SASL." + saslMechanismToString(it->mechanism) + ".ERROR");
                    doXResponse(theServer, Routing, "Invalid credentials", true);
                    saslRequests.erase(it);
                ...
                std::string server_signature =
                    compute_server_signature(it->scram.serverKey, auth_message);
                std::string server_final = "v=" + server_signature;
                std::string server_final_b64 =
                    b64encode(reinterpret_cast<const unsigned char*>(server_final.data()),
                              server_final.size());
    ...
    // It is the initial message consisting of the mechanism.
        ...
        if (tmpClient) {
            ...
            fingerprint =
                tmpClient->getTlsFingerprint().empty() ? "_" : tmpClient->getTlsFingerprint();
        ...
        for (const std::string& id : st2) {
            ...
            if (st2[0][0] == '+')
                it = std::find_if(
                    ...
                    [&user](
                        const std::pair<std::string, std::pair<std::string, std::string>>& pair) {
...
bool cservice::doXROplist(iServer* /*theServer*/, const string& Routing, const string& Message) {
    ...
    LOG(TRACE,
        "OPLIST scorechan = {} opCount(rank) = {} score = {} account = {} firstOpped = {} "
        "lastOpped = {}",
        scoreChan, opCount, score, account, firstOpped, lastOpped);

    /*	Now we do the actual work to insert scores to SQL DB
            Rules:
                    1). Only do work for channels with a pending application? (we only send XQUERY
       for these) 2). Only INSERT if (channel_id,user_id) pair does not exist, otherwise we UPDATE
    */
    stringstream queryString;
    ...
    if (SQLDb->Exec(queryString, true)) {
        if (SQLDb->Tuples() < 1) {
            ...
            if (!tmpUser) {
                LOG(ERROR, "tmpUser with account = {} could not be found (expired or purged)",
                    account);
                return false;
            }
            ...
            queryString << "SELECT channel_id,first FROM pending_chanfix_scores WHERE user_id='"
                        << userID << "' AND channel_id=(SELECT id FROM channels WHERE lower(name)='"
            ...
            if (SQLDb->Exec(queryString, true)) {
                ...
                if (SQLDb->Tuples() < 1) {
                    ...
                    updateQuery << "INSERT INTO pending_chanfix_scores "
                                << "(channel_id,user_id,rank,score,account,first_opped,last_opped, "
                                   "first) VALUES ('"
                                << chanID << "', '" << userID << "', '" << opCount << "', '"
                                << score << "', '" << account << "', '" << firstOpped << "', '"
                                << lastOpped << "', '"
                                << "Y"
                                << "')" << ends;
                } else {
                    ...
                    if (first == 'N') {
                        updateQuery << "UPDATE pending_chanfix_scores SET "
                                    ...
                                    << " last_updated=date_part('epoch', CURRENT_TIMESTAMP)::int, "
                                    << "first='N'"
                                    << " WHERE user_id='" << userID << "' AND channel_id='"
                                    << chanID << "' AND first='N'" << ends;
                    } else {
                        updateQuery << "INSERT INTO pending_chanfix_scores "
                                    << "(channel_id,user_id,rank,score,account,first_opped,last_"
                                       "opped, first) VALUES ('"
                                    << chanID << "', '" << userID << "', '" << opCount << "', '"
                                    << score << "', '" << account << "', '" << firstOpped << "', '"
                                    << lastOpped << "', '"
...
bool cservice::doCommonAuth(iClient* theClient, string username) {
    ...
    /* check to see if we have a last seen time (bug workaround) - if not, make one */
    stringstream queryString;
    queryString << "SELECT last_seen FROM users_lastseen WHERE user_id=" << theUser->getID()
                << ends;
    ...
    // But I think the sqlUser data must be assigned to the customData structure in any case!
    // --Seven
    ...
    if (getConfigVar("ALERT_FAILED_LOGINS")->asInt() == 1 && theUser->getFailedLogins() > 0) {
        ...
        Notice(theClient,
               getResponse(
                   theUser, language::auth_failed_logins,
    ...
    if (multiAuth) {
        ...
        Notice(theClient,
               getResponse(
                   theUser, language::suspend_if_not_you,
    ...
    if (SQLDb->Tuples() == 1) {
        ...
        Notice(theClient,
               ...
               channelName.c_str(), mngrUserName.c_str(), getNickName().c_str(),
               channelName.c_str(), getNickName().c_str(), channelName.c_str());
...
void cservice::outputChannelAccesses(iClient* theClient, sqlUser* theUser, sqlUser* tmpUser,
    ...
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
        ...
        if ((channelList.size() + chanName.size() + chanAccess.size() + 5) >= 450) {
            Notice(theClient,
                   getResponse(tmpUser, language::channels, string("Channels: %s")).c_str(),
                   channelList.c_str());
