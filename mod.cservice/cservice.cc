...
extern "C" {
xClient* _gnuwinit(const string& args) { return new cservice(args); }
}
...
void cservice::OnAttach() {
   ...
   */
  if (SQLDb->Exec("DELETE FROM webnotices WHERE created_ts < (date_part('epoch', "
                  "CURRENT_TIMESTAMP)::int - 600)")) {
...
#endif
{
  ...
  RegisterCommand(new ACCESSCommand(
      this, "ACCESS", "[channel] [username] [-min n] [-max n] [-op] [-voice] [-none] [-modif]", 5));
  ...
  RegisterCommand(new MODINFOCommand(this, "MODINFO",
                                     "<#channel> [ACCESS <username> <level>] [AUTOMODE <username> "
                                     "<NONE|OP|VOICE>] [INVITE <ON|OFF>]",
                                     6));
  RegisterCommand(new SETCommand(this, "SET",
                                 "[#channel] <variable> <value> or, SET <invisible> <ON|OFF> or, "
                                 "SET LANG <language> or, SET MAXLOGINS <max-logins>.",
                                 6));
  ...
  RegisterCommand(
      new SUSPENDCommand(this, "SUSPEND", "<#channel> <username> <duration> [level] [reason]", 5));
  RegisterCommand(new UNSUSPENDCommand(this, "UNSUSPEND", "<#channel> <username>", 5));
  RegisterCommand(new BANCommand(
      this, "BAN", "<#channel> <nicks | *!*user@*.host> [duration] [level] [reason]", 5));
  ...
  RegisterCommand(
      new WHITELISTCommand(this, "WHITELIST", "<ADD|REM|VIEW> <IP> [duration] [reason]", 10));
  RegisterCommand(new SCANHOSTCommand(
      this, "SCANHOST", "<mask> [-sort <user|cmd|host|ip|time>] [-max n] [-order <asc|desc>]", 10));
  ...
  RegisterCommand(
      new REHASHCommand(this, "REHASH", "[translations|help|variables|config|motd]", 5));
...
#ifdef ALLOW_HELLO
  RegisterCommand(new HELLOCommand(this, "HELLO", "<username> <email> <email>", 10));
#endif // ALLOW_HELLO
  ...
  elog << "*** [CMaster]: Attempting to make PostgreSQL connection to: " << confSqlHost
       << "; Database Name: " << confSqlDb << endl;
  ...
  if (SQLDb->ConnectionBad()) {
    elog << "*** [CMaster]: Unable to connect to SQL server." << endl
         << "*** [CMaster]: PostgreSQL error message: " << SQLDb->ErrorMessage() << endl;
  ...
  if (SQLDb->Exec("SELECT date_part('epoch', CURRENT_TIMESTAMP)::int;", true)) {
    ...
    elog << "*** [CMaster::ImplementServer]:  Current DB server time: " << currentTime() << endl;
  } else {
...
void cservice::BurstChannels() {
  ...
  while (ptr != sqlChannelCache.end()) {
    ...
    if (theChan->getName() == "*") { // Do not do anything for the admins channel
      ++ptr;
      continue;
    }
    ...
    if (theChan->getFlag(sqlChannel::F_AUTOJOIN)) {
      ...
      /* If FLOATLIM is activated, we don't set +l. The correct limit will be set shortly after
       * burst. */
      if (theChan->getFlag(sqlChannel::F_FLOATLIM))
        stripModes(tempModes, "l");

      MyUplink->JoinChannel(this, theChan->getName(), tempModes, theChan->getChannelTS(), true);
...
void cservice::OnConnect() {
  ...
  if (!netSaslTimeout || std::stoul(netSaslTimeout->first) != saslTimeout) {
    MyUplink->Write("%s CF %d sasl.timeout :%d", getCharYY().c_str(), time(nullptr), saslTimeout);
  }
  ...
  if ((getLastRecieved(theClient) + flood_duration) <= ::time(NULL)) {
    ...
    if (getFloodPoints(theClient) > input_flood) {
      ...
      silenceList.insert(silenceListType::value_type(
          silenceMask, std::make_pair(expireTime, theClient->getCharYYXXX())));
      ...
      if (getConfigVar("FLOOD_MESSAGES")->asInt() == 1) {
        logAdminMessage("MSG-FLOOD from %s (%s)", theClient->getNickUserHost().c_str(),
                        floodComment.c_str());
      }
      return true;
    } // if()
    ...
    if (ipFloodMap[xIP(theClient->getIP()).GetNumericIP(true)] > input_flood * 5) {
      ...
      if (getConfigVar("FLOOD_MESSAGES")->asInt() == 1) {
        logAdminMessage("IP-FLOOD from %s (%s)", theClient->getNickUserHost().c_str(),
                        floodComment.c_str());
      }
      return true;
    }
...
void cservice::OnPrivateMessage(iClient* theClient, const string& Message, bool secure) {
  /*
  ...
  if (!secure && ((Command == "LOGIN") || (Command == "NEWPASS") || (Command == "SUSPENDME"))) {
    Notice(theClient, "To use %s, you must /msg %s@%s", Command.c_str(), nickName.c_str(),
           getUplinkName().c_str());
    return;
  }
   ...
   *
                  ...
                  if (theUser && getAdminAccessLevel(theUser))
                          {
                          adminLog << ::time(NULL) << " " << theClient->getRealNickUserHost() << " "
   << st.assemble() << endl;
                          }
   */
  ...
  if (commHandler == commandMap.end()) {
    ...
    /* Check IP restriction (if admin level) - this is in case you get added as admin AFTER logging
     * in */
    ...
    if (commHandler->second->Exec(theClient, Message)) {
      ...
      if (theUser && getAdminAccessLevel(theUser) && (Command != "LOGIN") &&
          (Command != "NEWPASS") && (Command != "SUSPENDME")) {
        stringstream theLog;
        theLog << "INSERT INTO adminlog (user_id,cmd,args,timestamp,issue_by)"
               << " VALUES(" << theUser->getID() << ","
      ...
      if (commandLog) {
        ...
        jsonParams += "\"client_nick\":\"" + escapeJsonString(theClient->getNickName()) +
                      "\",\"client_userhost\":\"" + escapeJsonString(theClient->getRealUserHost()) +
                      "\"";
...
void cservice::OnChannelCTCP(iClient* Sender, Channel* theChan, const string& CTCPCommand,
  ...
  if (!sqlChan)
    return;
  if (!sqlChan->getInChan())
    return;
  if (Sender->getMode(iClient::MODE_SERVICES))
    return;
...
void cservice::OnCTCP(iClient* theClient, const string& CTCP, const string& Message, bool) {
  /**
  ...
  if (Command == "PING" || Command == "ECHO") {
    ...
    xClient::DoCTCP(theClient, CTCP, "Tried to be a man again - there was a slip - now I'm an IT");
  } else if (Command == "VERSION") {
    xClient::DoCTCP(theClient, CTCP,
                    "Undernet P10 Channel Services II [" __DATE__ " " __TIME__ "] Release 2.0.0B1");
...
sqlUser* cservice::getUserRecord(const string& id) {
  ...
  if (ptr != sqlUserCache.end()) {
    // Found something!
    LOG_MSG(TRACE, "Cache hit for {user_id}").with("user_id", id).logStructured();
...
bool cservice::updateIPRlast_used(sqlUser* theUser, const string& ipr_ipvalue) {
  ...
  theQuery << "UPDATE ip_restrict SET last_used = date_part('epoch', CURRENT_TIMESTAMP)::int"
           << " WHERE user_id = " << theUser->getID() << " AND value = '" << ipr_ipvalue << "'"
           << ends;
...
#ifdef LOG_SQL
  elog << "cservice::hasFP::sqlQuery> " << theQuery.str() << endl;
#endif

  if (!SQLDb->Exec(theQuery, true)) {
    elog << "cservice::hasFP> SQL Error: " << SQLDb->ErrorMessage() << endl;
    return 0;
  }
...
bool cservice::checkIPR(const string& ip, sqlUser* theUser, unsigned int& ipr_ts) {
  ...
  stringstream theQuery;
  theQuery << "SELECT value,expiry,added FROM ip_restrict WHERE (user_id = " << theUser->getID()
           << ") AND ((expiry IS NOT NULL) AND (expiry <> 0)) AND (type = 1)" << ends;
  ...
  if (SQLDb->Tuples() > 0) {
    ...
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
      ...
      if (ipr_expiry < (unsigned)currentTime())
        continue;
      ipr_res = false;
  ...
  theQuery.str("");
  theQuery << "SELECT value,added FROM ip_restrict WHERE (user_id = " << theUser->getID()
...
bool cservice::needIPRcheck(sqlUser* theUser) {
  ...
  if (theUser->getFlag(sqlUser::F_TOTP_REQ_IPR)) // && (theUser->getFlag(sqlUser::F_TOTP_ENABLED))
    return true;
  if ((hasIPR(theUser)) && (theUser->getFlag(sqlUser::F_TOTP_ENABLED)) &&
      (theUser->getFlag(sqlUser::F_TOTP_REQ_IPR)))
    return true;
  if (hasIPR(theUser))
    return true;
 ...
 */
short int cservice::getEffectiveAccessLevel(sqlUser* theUser, sqlChannel* theChan, bool notify) {
  ...
  if (theChan->getFlag(sqlChannel::F_SUSPEND)) {
    ...
    if (theUser->isAuthed() && notify) {
      noticeAllAuthedClients(theUser,
                             "The channel %s has been suspended by a cservice administrator.",
 ...
 */
short int cservice::getAccessLevel(sqlUser* theUser, sqlChannel* theChan) {
  sqlLevel* theLevel = getLevelRecord(theUser, theChan);
  if (theLevel) {
    return theLevel->getAccess();
  }
  ...
  if (SQLDb->Exec("SELECT language_id,topic,contents FROM help", true))
    ...
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++)
      helpTable.insert(helpTableType::value_type(
          std::make_pair(atoi(SQLDb->GetValue(i, 0).c_str()), SQLDb->GetValue(i, 1)),
          SQLDb->GetValue(i, 2)));

  elog << "*** [CMaster::loadHelpTable]: Loaded " << helpTable.size() << " help messages." << endl;
}
 ...
 * language.
 */
const string cservice::getResponse(sqlUser* theUser, int response_id, string msg) {
...
void cservice::loadTranslationTable() {
  if (SQLDb->Exec("SELECT id,code,name FROM languages", true))
    // if (PGRES_TUPLES_OK == status)
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++)
      languageTable.insert(languageTableType::value_type(
          SQLDb->GetValue(i, 1),
          std::make_pair(atoi(SQLDb->GetValue(i, 0).c_str()), SQLDb->GetValue(i, 2))));

  elog << "*** [CMaster::loadTranslationTable]: Loaded " << languageTable.size() << " languages."
       << endl;

  if (SQLDb->Exec("SELECT language_id,response_id,text FROM translations", true))
  // if( PGRES_TUPLES_OK == status )
  {
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
      /*
  ...
  elog << "*** [CMaster::loadTranslationTable]: Loaded " << translationTable.size()
       << " translations." << endl;
}
...
int cservice::rehashMOTD() {
  ...
  if (ptr != translationTable.end()) {
    ...
    selectQuery << "SELECT text,last_updated FROM "
                << "translations WHERE language_id = " << lang_id
                << "AND response_id = " << language::motd << ends;
...
bool cservice::isOnChannel(const string& /* chanName */) const { return true; }
...
void cservice::expireSuspends() {
  ...
  expireQuery << "SELECT user_id,channel_id FROM levels "
              << "WHERE suspend_expires <= " << expiredTime << " AND suspend_expires <> 0" << ends;
  ...
  for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
    expireVector.push_back(
        expireVectorType::value_type(SQLDb->GetValue(i, 0), SQLDb->GetValue(i, 1)));
  }
  ...
  updateQuery << "UPDATE levels SET suspend_expires = 0, suspend_level = 0, suspend_by = '', "
                 "suspend_reason = ''"
...
void cservice::expireBans() {
  ...
  expireQuery << "SELECT channel_id,id FROM bans "
              << "WHERE expires > 0 AND expires <= " << expiredTime << " ORDER BY channel_id"
              << ends;
  ...
  for (size_t i = 0; i < SQLDb->Tuples(); i++) {
    expireVector.emplace_back(atoi(SQLDb->GetValue(i, 0)), atoi(SQLDb->GetValue(i, 1)));
    ...
    for (const auto& [chanID, banID] : expireVector) {
      ...
      if (ptr != theChan->banList.end()) {
        ...
        if (tmpChan && tmpChan->findBan(theBan->getBanMask())) {
          banVector.push_back(xServer::banVectorType::value_type(false, theBan->getBanMask()));
        }
...
void cservice::cacheExpireUsers() {
  ...
  while (ptr != sqlUserCache.end()) {
    ...
    if (tmpUser->isAuthed() && ((tmpUser->getInstantiatedTS() + 86400) < ::time(NULL))) {
      ...
      queryString << "SELECT last_seen FROM users_lastseen WHERE user_id=" << tmpUser->getID()
                  << ends;
      ...
      if (SQLDb->Exec(queryString, true)) {
        if (SQLDb->Tuples() < 1) {
          ...
          updateQuery << "INSERT INTO users_lastseen (user_id,"
                      << "last_seen,last_updated) VALUES(" << tmpUser->getID()
                      << ",date_part('epoch', CURRENT_TIMESTAMP)::int,date_part('epoch', "
     ...
     */
    if (((tmpUser->getLastUsed() + 3600) < currentTime()) && !tmpUser->isAuthed()) {
      LOG(TRACE, "Purging user-id: {} from cache. Last used: {}", tmpUser->getID(),
          tmpUser->getLastUsed());
      purgeCount++;
      removeKey = ptr->first;
      delete (ptr->second);
  ...
  LOG(INFO, "User cache cleanup complete; Removed {} user records in {} ms.", purgeCount,
      elapsedMs(startTime));
  LOG(INFO, "I also updated {} last_seen records for people logged in for >24 hours.", updateCount);
}
...
void cservice::cacheExpireLevels() {
  ...
  while (ptr != sqlChannelCache.end()) {
    ...
    if (((unsigned int)(currentTime() - theChan->getLastUsed()) >= partIdleChan) &&
        theChan->getInChan() && !theChan->getFlag(sqlChannel::F_SPECIAL)) {
      /*
...
void cservice::performReops() {
  for (xNetwork::channelIterator itr = Network->channels_begin(); itr != Network->channels_end();
       itr++) {
...
bool cservice::deleteUserFromTable(unsigned int userId, const string& table) {
  ...
  if (table == "pending")
    userIdStr = "manager_id";
  if (table == "users")
    userIdStr = "id";
  if (table == "pending_mgrchange")
    userIdStr = "new_manager_id";
  stringstream queryString;
  queryString << "DELETE FROM " << table << " WHERE " << userIdStr << " = " << userId << endl;
...
void cservice::updateChannels() {
  ...
  if (!newChanList.empty()) {
    for (vector<newChanData*>::iterator itr = newChanList.begin(); itr != newChanList.end();
         itr++) {
      sqlUser* mngrUser = getUserRecord((*itr)->mngr_userId);
      if (!sqlRegisterChannel(getInstance(), mngrUser, (*itr)->chanName))
        LOG_MSG(ERROR, "FAILED to sqlRegisterChannel for {chan_name} with manager {}",
                (*itr)->mngr_userId)
...
void cservice::updateUserLevels(sqlUser* theUser) {
  ...
  theQuery << "SELECT " << sql::level_fields << " FROM levels WHERE user_id = " << theUser->getID()
           << ends;
...
void cservice::updateLevels(int channelId) {
  ...
  if (channelId > 0) {
    theQuery << "levels WHERE channel_id = " << channelId << ends;
  } else {
    theQuery << "levels WHERE last_updated >= " << lastLevelRefresh << ends;
  }
...
#ifdef LOG_SQL
  elog << "*** [CMaster::updateFingerprints]: sqlQuery: " << theQuery << endl;
#endif

  if (!SQLDb->Exec(theQuery, true)) {
    elog << "*** [CMaster::updateFingerprints]: SQL error: " << SQLDb->ErrorMessage() << endl;
    return;
  }
...
//-------------------------
  ...
  if (timer_id == pendingNotif_timerID) {
    ...
    LOG(INFO,
        "Loaded Pending Channels notification list, I have just notified {} channels that they are "
        "under registration.",
        noticeCount);
...
void cservice::AddToValidResponseString(const string& resp) {
  if (validResponseString == "")
    validResponseString = resp;
  else
    validResponseString += "," + resp;
  return;
}
...
bool cservice::isValidUser(const string& userName) {
   ...
   * if user noreg type is 0, then it is NOREG, if type=5 then it is LOCKED, if type = 6 then
   * user_name is referring to the locked verification answer.
   */
  stringstream theQuery;
  theQuery
  ...
  if (!SQLDb->Exec(theQuery, true)) {
    ...
    for (ValidUserDataListType::const_iterator itr = ValidUserDataList.begin();
         itr != ValidUserDataList.end(); ++itr) {
      if (itr->type == 6) {
        if (itr->UserName[0] == '!') {
        ...
        } else { /* This should be the matchcase metod - TODO: need to find a solution */
...
// We use this when we need to know if a channel in the meantime got registered in the database (eg.
// by the webpage)
...
bool cservice::RejectChannel(unsigned int chanId, const string& reason) {
  ...
  theQuery << "UPDATE pending SET status = '9',"
           ...
           << " decision = 'by The Judge: " << reason
           << "', reviewed = 'Y', reviewed_by_id = " << ReviewerId
...
bool cservice::sqlRegisterChannel(iClient* theClient, sqlUser* mngrUsr, const string& chanName) {
  ...
  theQuery.str("");
  theQuery << "DELETE FROM pending_chanfix_scores WHERE channel_id = " << newChan->getID() << ends;
  ...
  if (theClient != getInstance()) {
    Notice(theClient,
           getResponse(theUser, language::regged_chan, string("Registered channel %s")).c_str(),
           newChan->getName().c_str());
  }

  /*
   *  Finally, commit a channellog entry, but just if theClient is not this client (X) because in
   * that case it is already done by the webpage
   */
  if (theClient != getInstance())
  ...
  if (!newManager->insertRecord()) {
    Notice(theClient,
           "Couldn't automatically add the level 500 Manager, check it doesn't already exist.");
    delete (newManager);
    return (false);
  }
  ...
  if (!welcomeNewChanMessage.empty())
    Notice(newChan->getName(),
           TokenStringsParams(welcomeNewChanMessage.c_str(), newChan->getName().c_str()).c_str());

  // Set a welcome topic of the new channel, only if the actual topic is empty and the channel has
  // not been registered with the register command
#ifdef TOPIC_TRACK
  if (tmpChan && tmpChan->getTopic().empty() && !welcomeNewChanTopic.empty() &&
      theClient == getInstance())
...
void cservice::checkValidUsersAndChannelsState() {
  ...
  stringstream theQuery;
  theQuery
      << "SELECT channel_id,channels.name,manager_id,users.user_name FROM channels,pending,users "
  ...
  if (!chanList.empty()) {
    ...
    for (size_t i = 0; i < chanList.size(); ++i) {
      ...
      if (!isValidChannel(chanList.at(i).second)) {
        ...
        NoteAllAuthedClients(mgrUsr,
                             "Your channel application of %s has been rejected with reason: %s",
                             chanList[i].second.c_str(), rejectReason.c_str());
  ...
  theQuery << "SELECT user_id, user_name,channel_id,channels.name FROM supporters,users,channels "
           << "WHERE channels.id = supporters.channel_id AND users.id = supporters.user_id" << ends;
  if (!SQLDb->Exec(theQuery, true)) {
    ...
    for (size_t i = 0; i < suppList.size(); i++) {
      ...
      // logDebugMessage("Checking validity of suporter %s supporting channel
      // %s",suppUserName.c_str(),chanName.c_str());
      if (!isValidUser(suppUserName)) {
        string rejectReason =
            "Invalid supporter: " + suppUserName + " (" + validResponseString + ")";
        validResponseString.clear();
        logTheJudgeMessage(rejectReason.c_str());
        ...
        if (currentChanId != chanId) {
          ...
          NoteAllAuthedClients(mgrUsr,
                               "Your channel application of %s has been rejected with reason: %s",
                               chanName.c_str(), rejectReason.c_str());
        }
  ...
  else if (!chanList.empty())
    LOG(WARN, "WARNING: Not found any supporter!");
  chanList.clear();
  managerList.clear();
  return;
}
...
void cservice::checkNewIncomings() {
  ...
  theQuery << "SELECT channels.name,channels.id,users.user_name FROM channels,pending,users WHERE "
              "channels.id = pending.channel_id "
  ...
  if (!SQLDb->Exec(theQuery, true)) {
    ...
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
      ...
      logTheJudgeMessage("Rejected channel application %s with reason: %s", chanName.c_str(),
                         reason.c_str());
    }
    if (!rejectList.empty()) {
      for (unsigned int i = 0; i < rejectList.size(); i++) {
        ...
        NoteAllAuthedClients(mgrUsr,
                             "Your channel application of %s has been rejected with reason: %s",
                             rejectList[i].first.second.c_str(), reason.c_str());
      }
    }
    rejectList.clear();
...
void cservice::checkTrafficPass() {
  ...
  while (ptr != pendingChannelList.end()) {
    ...
    // Check if the channel was visited at least at ONCE by the required MinSupporters count
    // supporters
    ...
    if (actualMinSupporters < MinSupporters) {
      ...
      if (elapsedDays < currentTime())
        logTheJudgeMessage(
            "Insufficient number of supporters that visited the pending channel %s (%i/%i)",
            ptr->first.c_str(), actualMinSupporters, MinSupporters);
    } else // <- if yes, we check if one of the supporters has an insufficient MinSupportersJoin
           // joincount
    {
      ...
      while (ptr2 != pendingChan->uniqueSupporterList.end()) {
        if (ptr2->second->join_count < MinSupportersJoin) {
          ...
          if (elapsedDays < currentTime())
            logTheJudgeMessage("Insufficient supporter joincount of supporter user %s (%i/%i) on "
                               "pending channel %s",
                               suppUserName.c_str(), actualMinSupportersJoin, MinSupportersJoin,
                               ptr->first.c_str());
        }
        ++ptr2;
      } // end while
    ...
    if (pendingChan->unique_join_count < UniqueJoins) {
      ...
      if (elapsedDays < currentTime())
        logTheJudgeMessage(
            "Insufficient number of IP's (%i/%i) that visited the pending channel %s",
            pendingChan->unique_join_count, UniqueJoins, ptr->first.c_str());
    }
    if (pendingChan->join_count < Joins) {
      JoinsPass = false;
      if (elapsedDays < currentTime())
        logTheJudgeMessage(
            "Insufficient number of joincounts (%i/%i) that visited the pending channel %s",
            pendingChan->join_count, Joins, ptr->first.c_str());
    }
    ...
    if ((JoinsPass) && (uniqueJoinsPass) && (minSupportersPass) && (minSupportersJoinPass)) {
      ...
      if (!SQLDb->Exec(theQuery, true)) {
        ...
        logTheJudgeMessage(
            "Channel %s has passed traffic checking, successfully moved to Notification stage",
            ptr->first.c_str());
      pendingChan->commit();
      ptr->second = NULL;
      delete (pendingChan);
    ...
    if (elapsedDays < currentTime()) {
      ...
      if ((!minSupportersPass) || (!minSupportersJoinPass))
        rejectReason = "Insufficient supporter activity. *All* supporters need to be active users "
                       "of the channel";
      if (!rejectReason.empty()) {
        RejectChannel(pendingChan->channel_id, rejectReason);
        logTheJudgeMessage("Rejecting channel %s with reason: %s", ptr->first.c_str(),
                           rejectReason.c_str());
        pendingChan->commit();
        ptr->second = NULL;
        delete (pendingChan);
...
void cservice::checkObjections() {
  ...
  stringstream theQuery;
  theQuery
      << "SELECT channels.name,channels.id,users.user_name FROM channels,pending,users,objections "
      << "WHERE channels.id = pending.channel_id "
      << "AND pending.status = 2 AND (pending.check_start_ts + " << notifTime
      << ") < date_part('epoch', CURRENT_TIMESTAMP)::int "
  ...
  if (!SQLDb->Exec(theQuery, true)) {
    ...
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
      ...
      if (chanId != actualChan) // more than one objection per channel lead to multiple db hit, so
                                // we need to limit to one result
      {
        actualChan = chanId;
  ...
  if (!objectList.empty())
    for (unsigned int i = 0; i < objectList.size(); i++) {
      ...
      logTheJudgeMessage("Channel %s has been moved to Ready to review",
                         objectList[i].first.second.c_str());
      sqlUser* mgrUsr = getUserRecord(objectList[i].second.c_str());
      NoteAllAuthedClients(mgrUsr, "Your channel application of %s is now at state Ready to review",
                           objectList[i].first.second.c_str());
    }
  objectList.clear();
  return;
...
void cservice::checkAccepts() {
  ...
  unsigned int notifTime = NotifyDays * JudgeDaySeconds;
  stringstream theQuery;
  theQuery << "SELECT channels.name,channels.id,users.user_name,pending.reviewed FROM "
              "channels,pending,users "
           << "WHERE channels.id = pending.channel_id "
           << "AND pending.status = 2 AND (pending.check_start_ts + " << notifTime
           << ") < date_part('epoch', CURRENT_TIMESTAMP)::int "
           << " AND users.id = manager_id" << ends;
  ...
  if (!SQLDb->Exec(theQuery, true)) {
    ...
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
      ...
      acceptList.push_back(
          std::make_pair(std::make_pair(chanId, chanName), std::make_pair(mngrUser, acc)));
      // logTheJudgeMessage(chanName.c_str());
    }
  }
  if (!acceptList.empty())
    for (unsigned int i = 0; i < acceptList.size(); i++) {
      ...
      if (DecideOnCompleted || (reviewed == false && RequireReview)) {
        ...
        logTheJudgeMessage("Channel application %s completed, moved to 'Ready to review'",
                           acceptList[i].first.second.c_str());
        NoteAllAuthedClients(mgrUsr, "Your channel application of %s is now in 'Ready to review'",
                             acceptList[i].first.second.c_str());
...
void cservice::checkReviews() {
  ...
  theQuery << "SELECT channels.name,channels.id,users.user_name,pending.reviewed FROM "
           ...
           << "AND users.id = manager_id "
           << "AND NOT EXISTS (SELECT pending.channel_id FROM pending,objections WHERE "
              "pending.channel_id = objections.channel_id)"
           << ends;
...
void cservice::cleanUpReviews() {
  ...
  theQuery << "SELECT channels.name,channels.id,users.user_name FROM channels,pending,users "
           << "WHERE channels.id = pending.channel_id "
           << "AND pending.status = 8 AND (pending.check_start_ts + " << reviewTime
           << ") < date_part('epoch', CURRENT_TIMESTAMP)::int "
           << " AND users.id = manager_id" << ends;
  ...
  if (!reviewList.empty())
    for (unsigned int i = 0; i < reviewList.size(); i++) {
      if (wipeChannel(reviewList[i].first.first))
        logTheJudgeMessage("Expired and wiped Ready to Review channel application %s",
                           reviewList[i].first.second.c_str());
      else
        logTheJudgeMessage("(The Judge) Failed to wipeChannel(ready to Review) %s",
                           reviewList[i].first.second.c_str());
    }
  reviewList.clear();
}

// After a time, we cleanup the database from old application datas: pending channels, supporters,
// etc
...
void cservice::OnChannelModeV(Channel* theChan, ChannelUser* theChanUser,
  ...
  for (xServer::voiceVectorType::const_iterator ptr = theTargets.begin(); ptr != theTargets.end();
       ++ptr) {
...
void cservice::OnChannelModeO(Channel* theChan, ChannelUser* theChanUser,
  ...
  for (xServer::opVectorType::const_iterator ptr = theTargets.begin(); ptr != theTargets.end();
       ++ptr) {
    ChannelUser* tmpUser = ptr->second;
    bool polarity = ptr->first;
    ...
    if (polarity) {
      ...
      if (authUser && authUser->getFlag(sqlUser::F_GLOBAL_SUSPEND)) {
        if (theChanUser)
          Notice(theChanUser->getClient(),
                 "The user %s (%s) has been suspended by a CService Administrator.",
      ...
      if (theBan && (theBan->getLevel() <= 75)) {
        if (!tmpUser->getClient()->getMode(iClient::MODE_SERVICES)) {
          ...
          Notice(tmpUser->getClient(), "You are not allowed to be opped on %s",
                 reggedChan->getName().c_str());

          /* Tell the person doing the op'ing this is bad */
          if (theChanUser) {
            Notice(theChanUser->getClient(), "%s isn't allowed to be opped on %s",
                   tmpUser->getClient()->getNickName().c_str(), reggedChan->getName().c_str());
          }
        }
      }
      ...
      if (tmpUser->getClient() == me) {
        logAdminMessage("I've been deopped on %s!", reggedChan->getName().c_str());
  ...
  if (!deopList.empty()) {
    if ((theChanUser) && (reggedChan->getFlag(sqlChannel::F_NOOP))) {
      Notice(theChanUser->getClient(), "The NOOP flag is set on %s", reggedChan->getName().c_str());
    }
  ...
  if ((theChanUser) && (deopCounter >= reggedChan->getMassDeopPro()) &&
      ...
      !theChanUser->getClient()->getMode(iClient::MODE_SERVICES)) {
...
void cservice::OnEvent(const eventType& theEvent, void* data1, void* data2, void* data3,
  ...
  case EVT_XQUERY: {
    ...
    // elog << "CSERVICE.CC XQUERY: " << theServer->getName() << " " << Routing << " " << Message <<
    // endl;
    // As it is possible to run multiple GNUWorld clients on one server, first parameter should be a
    // nickname. If it ain't us, ignore the message, the message is probably meant for another
    // client here.
    StringTokenizer st(Message);
    if (st.size() < 2) {
  ...
  case EVT_XREPLY: {
    ...
    const char* Routing = reinterpret_cast<char*>(data2);
    const char* Message = reinterpret_cast<char*>(data3);
    // elog << "CSERVICE.CC XREPLY: " << theServer->getName() << " " << Routing << " " << Message <<
    // endl; As it is possible to run multiple GNUWorld clients on one server, first parameter
    // should be a nickname. If it ain't us, ignore the message, the message is probably meant for
    // another client here.
    StringTokenizer st(Message);
    if (st.size() < 2) {
...
void cservice::deVoiceAllOnChan(Channel* theChan) {
  ...
  for (Channel::const_userIterator ptr = theChan->userList_begin(); ptr != theChan->userList_end();
       ++ptr) {
    ...
    if (ptr->second->getMode(ChannelUser::MODE_O)) {
      ...
      if (!authUser) {
        ...
        /* Authed but no access? Tough. :) */
                 ...
                 !(getEffectiveAccessLevel(authUser, reggedChan, false) >= level::op)) {
        /* Don't deop +k things */
        if (!ptr->second->getClient()->getMode(iClient::MODE_SERVICES)) {
          deopList.push_back(ptr->second->getClient());
        }
...
void cservice::deopSuspendedOnChan(Channel* theChan, sqlUser* theUser) {
  ...
  if (!theUser->networkClientList.empty())
    for (sqlUser::networkClientListType::iterator cliPtr = theUser->networkClientList.begin();
      ...
      if ((tmpUser) &&
          (tmpUser->getMode(ChannelUser::MODE_O) && (!tmpClient->getMode(iClient::MODE_SERVICES))))
...
void cservice::doAllBansOnChan(Channel* tmpChan) {
  ...
  for (vector<ChannelUser*>::iterator tItr = tmpList.begin(); tItr != tmpList.end(); tItr++) {
...
void cservice::doTheRightThing(Channel* tmpChan) {
  ...
  if (tmpChan && !tmpBotUser->getMode(ChannelUser::MODE_O)) {
    ...
    if (reggedChan->getInChan()) {
      ...
      LOG_MSG(INFO, "Performed reop for channel {chan_name}").with("chan", tmpChan).logStructured();
    }
  }
...
void cservice::OnChannelEvent(const channelEventType& whichEvent, Channel* theChan, void* data1,
      ...
      if (!burstJoin) {
        ...
        if (ptr->second->unique_join_count < 50) {
          if (Tptr == ptr->second->trafficList.end()) {
            ...
            LOG_MSG(
                INFO,
                "Created a new IP traffic record for IP#{ip} ({client_userhost}) on {chan_name}")
                .with("ip", NumericIP)
  ...
  case EVT_PART: {
    ...
    // elog << "cservice::EVT_PART> " << theClient << " Part " << theChan->getName() << " (" <<
    // partMsg << ")" << endl;
...
bool cservice::checkBansOnJoin(Channel* netChan, sqlChannel* theChan, iClient* theClient) {
  ...
  if (theBan && (theBan->getLevel() >= 75)) {
    stringstream s;
    s << getCharYYXXX() << " M " << theChan->getName() << " +b " << theBan->getBanMask() << ends;
    ...
    if (!theClient->getMode(iClient::MODE_SERVICES)) {
      Kick(netChan, theClient, string("(" + theBan->getSetBy() + ") " + theBan->getReason()));
    }
...
void cservice::OnWhois(iClient* sourceClient, iClient* targetClient) {
  /*
   *  Return info about 'targetClient' to 'sourceClient'
   */

  stringstream s;
  s << getCharYY() << " 311 " << sourceClient->getCharYYXXX() << " " << targetClient->getNickName()
    << " " << targetClient->getUserName() << " " << targetClient->getInsecureHost()
    << " * :" << targetClient->getDescription() << ends;
  Write(s);
  ...
  s.str("");
  s << getCharYY() << " 318 " << sourceClient->getCharYYXXX() << " " << targetClient->getNickName()
...
bool cservice::Kick(Channel* theChan, const string& mask, const std::string& reason,
  ...
  for (Channel::userIterator chanUsers = theChan->userList_begin();
    ...
    if ((tmpUser->getClient()->isModeX() && tmpUser->getClient()->isModeR()) &&
        (mask.find(tmpUser->getClient()->getHiddenHostSuffix()) == string::npos))
      continue;
...
void cservice::undoJoinLimits(sqlChannel* reggedChan) {
  ...
  stringstream s;
  s << getCharYYXXX() << " M " << reggedChan->getName() << " -" << reggedChan->getLimitJoinModeSet()
...
void cservice::doJoinLimit(sqlChannel* reggedChan, Channel* theChan) {
  ...
  if (reggedChan->getLimitJoinTime() == 0 ||
      (reggedChan->getLimitJoinTime() + reggedChan->getLimitJoinSecs()) <
          currentTime()) { // We have passed the session, start a new one
  ...
  if (reggedChan->getLimitJoinCount() >=
      (reggedChan->getLimitJoinMax())) { // Check if we're above threshold
    ...
    // Filter out already set channel modes from the mode to set, to avoid removing them when the
    // mode is lifted
...
void cservice::doFloatingLimit(sqlChannel* reggedChan, Channel* theChan) {
  ...
  if (currentDif <= (int)reggedChan->getLimitGrace())
    return;
  ...
  if (reggedChan->getLimitMax() && (newLimit >= reggedChan->getLimitMax()))
    return;
...
// This function is used to ban a mask, to who no matching client is currently existing on the
// channel
bool cservice::doSingleBan(sqlChannel* theChan, const string& banMask, unsigned short banLevel,
                           unsigned int banExpire, const string& theReason) {
  /*
...
bool cservice::doSingleBanAndKick(sqlChannel* theChan, iClient* theClient, unsigned short banLevel,
                                  unsigned int banExpire, const string& theReason) {
  ...
  /* Create a new Ban record */
  sqlBan* newBan =
      new (std::nothrow) sqlBan(this, theChan->getID(), banTarget, getNickName(), currentTime(),
                                banLevel, banExpire + currentTime(), theReason);
  assert(newBan != 0);
 ...
 */
bool cservice::doInternalBanAndKick(sqlChannel* theChan, iClient* theClient,
                                    const string& theReason) {
  ...
  for (Channel::userIterator chanUsers = netChan->userList_begin();
       chanUsers != netChan->userList_end(); ++chanUsers) {
    ChannelUser* tmpUser = chanUsers->second;
...
bool cservice::doInternalBanAndKick(sqlChannel* theChan, iClient* theClient,
                                    unsigned short banLevel, unsigned int banExpire,
                                    const string& theReason) {
  Channel* netChan = Network->findChannel(theChan->getName());
  /* Even if the channel is currently empty, presumably the sqlChannel exists in the db */
  if (!netChan) {
  ...
  for (Channel::userIterator chanUsers = netChan->userList_begin();
       chanUsers != netChan->userList_end(); ++chanUsers) {
    ChannelUser* tmpUser = chanUsers->second;
    if (Channel::createBan(tmpUser->getClient()) ==
        Channel::createBan(theClient)) { // Since we above initially added at least the Sender
                                         // client to the list, we shouldn't add again
...
bool cservice::doInternalBanAndKick(sqlChannel* theChan, const string& banMask,
                                    unsigned short banLevel, unsigned int banExpire,
                                    const string& theReason) {
  Channel* netChan = Network->findChannel(theChan->getName());
  /* Even if the channel is currently empty, presumably the sqlChannel exists in the db */
  if (!netChan) {
    ...
    /* Create a new Ban record */
    sqlBan* newBan =
        new (std::nothrow) sqlBan(this, theChan->getID(), banMask, getNickName(), currentTime(),
                                  banLevel, banExpire + currentTime(), theReason);
    assert(newBan != 0);
  ...
  for (Channel::userIterator chanUsers = netChan->userList_begin();
    ...
    ChannelUser* tmpUser = chanUsers->second;
    if ((tmpUser->getClient()->isModeX() && tmpUser->getClient()->isModeR()) &&
        (banMask.find(tmpUser->getClient()->getHiddenHostSuffix()) == string::npos))
      continue;
...
bool cservice::doInternalSuspend(sqlChannel* theChan, iClient* theClient, unsigned short suspLevel,
                                 unsigned int suspExpire, const string& theReason) {
  sqlUser* theUser = isAuthed(theClient, false);
  if (theUser) {
    sqlLevel* accessRec = getLevelRecord(theUser, theChan);
...
bool cservice::doInternalGline(iClient* theClient, const time_t& thePeriod,
                               const string& theReason) {
  ...
  if (theGline)
    ...
    theGline = new (std::nothrow) csGline(this);
...
bool cservice::KickAllWithFloodMessage(Channel* theChan, const string& Message,
                                       const string& kickMsg, bool clearcount) {
  sqlChannel* sqlChan = getChannelRecord(theChan->getName());
  assert(sqlChan != 0);
...
bool cservice::KickBanAllWithFloodMessage(Channel* theChan, const string& Message,
                                          unsigned short banLevel, unsigned int banExpire,
                                          const string& theReason) {
  sqlChannel* sqlChan = getChannelRecord(theChan->getName());
  assert(sqlChan != 0);
  ...
  for (; itr != MaskList.end(); itr++) {
     ...
     * otherwise any remaining items won't be banned on ChannelMessage because they don't reach the
     * RepeatCount to trigger this function
     */
...
bool cservice::GlineAllWithFloodMessage(sqlChannel* sqlChan, const string& Message,
                                        const time_t& thePeriod, const string& theReason) {
  std::list<string> IPlist = sqlChan->getRepeatMessageCount(Message).second;
  std::list<string>::iterator itr = IPlist.begin();
  unsigned int index = 0;
  for (; itr != IPlist.end(); itr++) {
  ...
  if ((repeatCount >= sqlChan->getRepeatCount()) && IPcount > 1) {
    ...
    if (currFN > sqlChannel::FLOODPRO_NONE) {
      ...
      if (sqlChan->getFloodproLevel() > currFN) {
        string FNmessage = TokenStringsParams(
            "Increased FLOODPRO punishment level to \002%s\002 on channel \002%s\002",
            sqlChan->getFloodLevelName(sqlChan->getFloodproLevel()).c_str(),
            sqlChan->getName().c_str());
  ...
  while (ptr != sqlChannelCache.end()) {
    ...
    if (theChan->getFloodproLevel() > sqlChannel::FLOODPRO_NONE) {
      ...
      if ((theChan->getFloodproLevel() > theChan->getManualFloodproLevel()) &&
        ...
        string FNmessage = TokenStringsParams(
            "Relaxed FLOODPRO punishment level to \002%s\002 on channel \002%s\002",
            theChan->getFloodLevelName(theChan->getFloodproLevel()).c_str(),
            theChan->getName().c_str());
...
void cservice::writeChannelLog(sqlChannel* theChannel, iClient* theClient, unsigned short eventType,
  ...
  theLog << "INSERT INTO channellog (ts, channelID, event, message, "
         << "last_updated) VALUES "
         << "(" << currentTime() << ", " << theChannel->getID() << ", " << eventType << ", "
         << "'[" << nickName << "]: " << escapeSQLChars(theClient->getNickUserHost()) << " ("
         << userExtra << ") " << escapeSQLChars(theMessage) << "', " << currentTime() << ")"
         << ends;
 ...
 */
const string cservice::getLastChannelEvent(sqlChannel* theChannel, unsigned short eventType,
                                           time_t eventTime) {
  unsigned int ts;
  stringstream queryString;
  ...
  queryString << "SELECT ts, message FROM channellog WHERE "
              << "channelid = " << theChannel->getID() << " AND event = " << eventType
              << " AND ts <= " << ts << " ORDER BY ts DESC LIMIT 1" << ends;
...
bool cservice::Notice(const iClient* Target, const string& Message) {
  ...
  if (Connected && MyUplink) {
    ...
    for (m = Message.c_str(); *m != 0; m++) {
      if (*m == '\n' || *m == '\r') {
        *b = '\0';
        MyUplink->Write("%s O %s :%s\r\n", getCharYYXXX().c_str(), Target->getCharYYXXX().c_str(),
                        buffer);
        b = buffer;
      } else {
        if (b < buffer + 509)
...
void cservice::dbErrorMessage(iClient* theClient) {
  Notice(theClient, "An error occured while performing this action, "
...
void cservice::setSupporterNoticedStatus(int suppId, int chanId, bool noticed) {
  ...
  if (!noticed)
    noticedStr = "N";
  theQuery << "UPDATE supporters SET noticed = '" << noticedStr << "' WHERE channel_id = " << chanId
           << " AND user_id = " << suppId << ends;
...
void cservice::UpdatePendingOpLists() {
  ...
  theQuery << "SELECT channels.name"
           << " FROM pending,channels"
           << " WHERE channels.id = pending.channel_id"
           << " AND pending.status IN (0, 1, 2)" << ends;
 ...
 * - 2a. When within pending_duration (pending_timerID) is first time found a new incoming
 * application
...
void cservice::checkIncomings(bool FirstNoticing) {
  ...
  stringstream theQuery;
  theQuery << "SELECT channels.id, channels.name, supporters.user_id, users.user_name, "
              "supporters.noticed"
  ...
  for (suppItr = suppIdList.begin(); suppItr != suppIdList.end(); ++suppItr) {
    ...
    if (suppItr->second.size() == 1) {
      ...
      message << "You are listed as a supporter for registration application for "
              << currItr->chanName
              << ". To progress the channel registration application you required to lodge your "
                 "support of the registrant "
              << "(" << currItr->mngrUserName << "). "
              << "Please type: \002/msg " << this->getNickName() << " support " << currItr->chanName
              << " <yes|no>\002" << endl;
      ...
      currItr = suppItr->second.begin();
      if (((FirstNoticing) && (!currItr->noticed)) || ((!FirstNoticing) && (currItr->noticed))) {
        noticeAllAuthedClients(suppUser,
                               "You are listed as a supporter of the channel application for the "
                               "following channels: %s",
                               chanEnum.str().c_str());
        message << "To progress the channel registration application you required to lodge your "
                   "support of the registrant. "
                   "For each channel please type '\002/msg "
                << getNickName().c_str()
                << " support <#chan> <YES|NO>\002' "
...
void cservice::initialiseSupport(const string& chanName,
                                 sqlPendingChannel::supporterListType suppList) {
  ...
  if (!theChan) {
     ...
     */
    LOG_MSG(WARN,
            "Warning: New empty channel application of {chan_name} (no users found on channel)")
  ...
  for (Channel::userIterator userItr = theChan->userList_begin();
    ...
    if (isSupporter) {
      sqlPendingChannel::supporterListType::iterator Supptr =
          pendingChan->supporterList.find(loggedUser->getID());
      if (Supptr == pendingChan->supporterList.end()) {
        // elog << "cservice::initializeInitialIPs> New supporter for channel " << chanName << "
        // suppUser = " << loggedUser->getUserName() << " inserting with joincount 1 " << endl;
        pendingChan->supporterList.insert(
            sqlPendingChannel::supporterListType::value_type(loggedUser->getID(), 1));
      } else {
        // elog << "cservice::initializeInitialIPs> Already existing supporter for channel " <<
        // chanName << " suppUser = " << loggedUser->getUserName() << " joinCount = " <<
        // Supptr->second << " reset joincount to 1" << endl;
        Supptr->second = 1;
      }
      LOG_MSG(INFO,
    ...
    if (Tptr == pendingChan->trafficList.end()) {
      ...
      trafRecord->join_count = 1;
      pendingChan->trafficList.insert(
          sqlPendingChannel::trafficListType::value_type(insertIP, trafRecord));
      trafRecord->insertRecord();
    } else {
      trafRecord = Tptr->second;
      trafRecord->join_count = 1;
    ...
    if (totalUsers > 50) {
      ...
      LOG_MSG(INFO, "Weird large new channel application of {chan_name}, usercount={}, clones={}",
              totalUsers, clonesCount)
          .with("chan", theChan)
          .logStructured();
      logAdminMessage("Weird large new channel application of %s, usercount=%i, clones=%i",
                      theChan->getName().c_str(), totalUsers, clonesCount);
    }
  }
...
void cservice::loadPendingChannelList() {
  ...
  stringstream theQuery;
  theQuery << "SELECT channels.name, pending.channel_id, user_id, pending.join_count, "
              "supporters.join_count, pending.unique_join_count, pending.first_init, "
              "pending.check_start_ts"
           << " FROM pending,supporters,channels"
           << " WHERE pending.channel_id = supporters.channel_id"
           << " AND channels.id = pending.channel_id"
           << " AND pending.status = 1;" << ends;
  ...
  // if( PGRES_TUPLES_OK == status )
  {
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
      ...
      if (ptr != pendingChannelList.end()) {
        ...
        newPending->channel_id = atoi(SQLDb->GetValue(i, 1).c_str());
        newPending->join_count = atoi(SQLDb->GetValue(i, 3).c_str());
        newPending->unique_join_count = atoi(SQLDb->GetValue(i, 5).c_str());
      ...
      newPending->supporterList.insert(sqlPendingChannel::supporterListType::value_type(
          atoi(SQLDb->GetValue(i, 2).c_str()), atoi(SQLDb->GetValue(i, 4).c_str())));
    }
  }
...
void cservice::checkDbConnectionStatus() {
  ...
  {
    logAdminMessage(
        "\002WARNING:\002 Backend database connection has been lost, attempting to reconnect.");
    ...
    if (SQLDb->ConnectionBad()) {
      ...
      if (connectRetries == connectRetry) {
        logAdminMessage("Unable to contact database after %d attempts, shutting down.",
                        connectRetry);
...
void cservice::preloadChannelCache() {
  ...
  elog << "*** [CMaster::preloadChannelCache]: Done. Loaded " << SQLDb->Tuples()
       << " registered channel records." << endl;
}
...
void cservice::preloadBanCache() {
  ...
  stringstream theQuery;
  theQuery << "SELECT " << sql::ban_fields << " FROM bans;" << ends;

  elog << "*** [CMaster::preloadBanCache]: Precaching Bans table: " << endl;
  ...
  // if( PGRES_TUPLES_OK == status )
  {
    for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
      ...
      theChan->banList.insert(std::make_pair(newBan->getID(), newBan));
      //		theChan->banList[newBan->getID()] = newBan;

    } // for()
  } // if()

  elog << "*** [CMaster::preloadBanCache]: Done. Loaded " << SQLDb->Tuples() << " bans." << endl;
}
...
void cservice::preloadLevelsCache() {
  ...
  stringstream theQuery;
  theQuery << "SELECT " << sql::level_fields << " FROM levels" << ends;

  elog << "*** [CMaster::preloadLevelCache]: Precaching Level table: " << endl;
  ...
  elog << "*** [CMaster::preloadLevelCache]: Done. Loaded " << goodCount << " level records out of "
       << SQLDb->Tuples() << "." << endl;
}
...
void cservice::preloadUserCache() {
  ...
  elog << "*** [CMaster::preloadUserCache]: Done. Loaded " << SQLDb->Tuples() << " user accounts."
       << endl;
}

void cservice::preloadFingerprintCache() {
  stringstream theQuery;
  theQuery << "SELECT fingerprint,user_id FROM users_fingerprints" << endl;

  elog << "*** [CMaster::preloadFingerprintCache]: Loading TLS fingerprints : " << endl;
  ...
  elog << "*** [CMaster::preloadFingerprintCache]: Done. Loaded " << SQLDb->Tuples()
       << " fingerprints." << endl;
}
...
void cservice::NoteAllAuthedClients(sqlUser* theUser, const char* Message, ...) {
  if (Connected && MyUplink && Message && Message[0] != 0) {
    ...
    queryString.str("");
    queryString << queryHeader << theUser->getID() << ", '" << escapeSQLChars(noteMessage) << "', "
                << "date_part('epoch', CURRENT_TIMESTAMP)::int);" << ends;
...
void cservice::doCoderStats(iClient* theClient) {
  ...
  int userDBTotal = 0;
  stringstream theQuery;
  theQuery << "SELECT COUNT(*) FROM users" << ends;
  ...
  theQuery.str("");
  theQuery << "SELECT COUNT(*) FROM users WHERE flags & " << sqlUser::F_TOTP_ENABLED << " > 0"
           << ends;
  ...
  theQuery.str("");
  // theQuery	<< "SELECT COUNT(*) FROM users,ip_restrict (SELECT DISTINCT user_id FROM
  // ip_restrict) AS temp WHERE flags & "
  theQuery << "SELECT COUNT(*) FROM users,ip_restrict WHERE flags & " << sqlUser::F_TOTP_ENABLED
...
#ifdef LOG_SQL
  elog << "cservice::doCoderStats::sqlQuery> " << theQuery.str().c_str() << endl;
#endif

  if (!SQLDb->Exec(theQuery, true)) {
    elog << "cservice::doCoderStats> SQL Error: " << SQLDb->ErrorMessage() << endl;
    return;
  }
  ...
  Notice(theClient,
         "[User Record Stats] \002Cached Entries:\002 %i    \002DB Requests:\002 %i    \002Cache "
         "Hits:\002 %i    \002Efficiency:\002 %.2f%%",
         sqlUserCache.size(), userHits, userCacheHits, userEf);
  ...
  while (ptr != sqlUserCache.end()) {
    ...
    if (tmpUser->isAuthed()) {
      ...
      } else if (tmpUser->getFlag(sqlUser::F_TOTP_ENABLED))
        totpCount++;
    }
    ++ptr;
  }
  ...
  while (ptr2 != Network->clients_end()) {
    ...
    if (tmpClient->isModeX())
      plusXCount++;
    if (tmpClient->isModeW())
      plusWCount++;
    if (tmpClient->isModeD())
      plusDCount++;
    if (tmpClient->isModeR())
      authCount++;
    ++ptr2;
  }
  ...
  Notice(theClient, "--- Total users in DB: %i (in cache: %i (%.2f%% of total))", userDBTotal,
         (int)sqlUserCache.size(), cacheTotal);
  float cacheTOTPTotal = ((float)totpCount / (float)sqlUserCache.size()) * 100;
  Notice(theClient, "--- Total users in DB with TOTP: %i (in cache: %i (%.2f%% of total))",
         userDBTOTPTotal, totpCount, cacheTOTPTotal);
  float cacheIPRTotal = ((float)iprCount / (float)sqlUserCache.size()) * 100;
  Notice(theClient, "--- Total users in DB with IPR: %i (in cache: %i (%.2f%% of total))",
         userDBIPRTotal, iprCount, cacheIPRTotal);
  float cacheIPRTOTPTotal = ((float)ipr_totp_Count / (float)sqlUserCache.size()) * 100;
  Notice(theClient, "--- Total users in DB with TOTP&IPR: %i (in cache: %i (%.2f%% of total))",
         userDBTOTPIPRTotal, ipr_totp_Count, cacheIPRTOTPTotal);
  Notice(theClient, "--- Total users in DB with TLS: %i (fingerprints in cache %i)", userDBFPTotal,
         fingerprintMap.size());

  float authTotal = ((float)authCount / (float)Network->clientList_size()) * 100;
  Notice(theClient, "--- Total Auth'd  : %i (%.2f%% of total)", authCount, authTotal);

  float plusXTotal = ((float)plusXCount / (float)Network->clientList_size()) * 100;
  Notice(theClient, "--- Total umode +x: %i (%.2f%% of total)", plusXCount, plusXTotal);

  float plusWTotal = ((float)plusWCount / (float)Network->clientList_size()) * 100;
  Notice(theClient, "--- Total umode +w: %i (%.2f%% of total)", plusWCount, plusWTotal);

  float plusDTotal = ((float)plusDCount / (float)Network->clientList_size()) * 100;
  Notice(theClient, "--- Total umode +d: %i (%.2f%% of total)", plusDCount, plusDTotal);

  float totpTotal = ((float)totpCount / (float)authCount) * 100;
  Notice(theClient, "--- Total TOTP: %i (%.2f%% of total)", totpCount, totpTotal);

  float iprTotal = ((float)iprCount / (float)authCount) * 100;
  Notice(theClient, "--- Total IPR: %i (%.2f%% of total)", iprCount, iprTotal);

  float totp_ipr_Total = ((float)ipr_totp_Count / (float)authCount) * 100;
  Notice(theClient, "--- Total TOTP&IPR: %i (%.2f%% of total)", ipr_totp_Count, totp_ipr_Total);

  float joinTotal = ((float)joinCount / (float)Network->channelList_size()) * 100;
  Notice(theClient, "I am in %i channels out of %i on the network. (%.2f%%)", joinCount,
         Network->channelList_size(), joinTotal);
  ...
  Notice(theClient, "\002Memory usage:\002 %s KB", prettyNumber(getMemoryUsage()).c_str());
  ...
  for (cservice::configHashType::iterator ptr = configTable.begin(); ptr != configTable.end();
       ++ptr) {
...
void cservice::loadConfigData() {
  stringstream theQuery;
  theQuery << "SELECT var_name,contents FROM variables;" << ends;
...
#define CONFIG_VAR(type, name, key)                                                                \
  configBindings.push_back({key, [this](bool rehash) {                                             \
                              if (rehash)                                                          \
...
cservice::AuthResult cservice::authenticateUser(AuthStruct& auth) {
   ...
   * regular login.
   *
   * If the client is authenticating with EXTERNAL, the auth fails if the fingerprint does not match
   * the username.
   *
   */
  ...
  if (totpAuthEnabled && theUser->getFlag(sqlUser::F_TOTP_ENABLED)) {
    /* If the user has a certificate matching the username or we are using SCRAM, the password is
     * the TOTP token. */
    ...
    /* If the user does not have a certificate matching the username and we are not using SCRAM, the
       second part of the password is the TOTP token. */
  ...
  if (!certAuth && auth.sasl != SaslMechanism::SCRAM_SHA_256 &&
      auth.sasl != SaslMechanism::EXTERNAL && !isPasswordRight(theUser, st.assemble(0, pass_end))) {
...
#ifdef HAVE_LIBSSL
  if (!certAuth && auth.sasl != SaslMechanism::SCRAM_SHA_256 &&
      auth.sasl != SaslMechanism::EXTERNAL && theUser->getScramRecord().empty()) {
    std::string err;
...
/* 10: Check TOTP token. */
  ...
  if (totp_enabled) {
    ...
    size_t len;
    int res =
        oath_base32_decode(theUser->getTotpKey().c_str(), theUser->getTotpKey().size(), &key, &len);
    if (res != OATH_OK)
      return AUTH_ERROR;
...
bool cservice::processAuthentication(AuthStruct auth, std::string* Message) {
  ...
  case cservice::TOO_EARLY_TOLOGIN:
    authResponse = TokenStringsParams("AUTHENTICATION FAILED as %s (Unable to login during "
                                      "reconnection, please try again in %i seconds)",
                                      auth.username.c_str(), (loginTime - currentTime()));
    break;
  case cservice::AUTH_FAILED:
    authResponse = TokenStringsParams(
        getResponse(NULL, language::auth_failed, string("AUTHENTICATION FAILED as %s")).c_str(),
        auth.username.c_str());
    setFailedLogin = true;
    break;
  case cservice::AUTH_UNKNOWN_USER:
    authResponse = TokenStringsParams(
        getResponse(NULL, language::not_registered, string("AUTHENTICATION FAILED as %s")).c_str(),
        auth.username.c_str());
    setFailedLogin = true;
    break;
  case cservice::AUTH_SUSPENDED_USER:
    authResponse =
        TokenStringsParams("AUTHENTICATION FAILED as %s (Suspended)", auth.username.c_str());
    setFailedLogin = true;
    break;
  ...
  case cservice::AUTH_INVALID_PASS:
    ...
    if ((max_failed_logins > 0) && (auth.theUser->getFailedLogins() > max_failed_logins) &&
      ...
      logPrivAdminMessage("%d failed logins for %s (last attempt by %s%s, checksum %d).",
                          auth.theUser->getFailedLogins(), auth.theUser->getUserName().c_str(),
                          clientMask.c_str(), auth.type == AuthType::XQUERY ? " (LoC)" : "",
                          checksum);
    }
    setFailedLogin = true;
    break;
  ...
  case cservice::AUTH_ERROR:
    authResponse = TokenStringsParams(
        "AUTHENTICATION FAILED as %s due to an error. Please contact a CService representative",
        auth.username.c_str());
    break;
  case cservice::AUTH_INVALID_TOKEN:
    authResponse = TokenStringsParams(
        getResponse(NULL, language::auth_failed_token, string("AUTHENTICATION FAILED as %s"))
            .c_str(),
        auth.username.c_str());
  ...
  case cservice::AUTH_FAILED_IPR:
    authResponse = TokenStringsParams("AUTHENTICATION FAILED as %s (IPR)", auth.username.c_str());

    /* notify the relay channel */
    logAdminMessage("%s (%s) failed IPR check.", clientMask.c_str(), auth.username.c_str());
  ...
  case cservice::AUTH_CERTONLY:
    authResponse = TokenStringsParams(
        "AUTHENTICATION FAILED as %s (Password login disabled and fingerprint mismatch)",
        auth.username.c_str());
    setFailedLogin = true;
    break;
  case cservice::AUTH_ML_EXCEEDED:
    authResponse =
        TokenStringsParams("AUTHENTICATION FAILED as %s (Maximum concurrent logins exceeded)",
  ...
  case cservice::AUTH_FAIL_EXCEEDED:
    authResponse = TokenStringsParams(
        getResponse(
            NULL, language::max_failed_logins,
            string(
                "AUTHENTICATION FAILED as %s (Exceeded maximum login failures for this session)"))
            .c_str(),
        auth.username.c_str());
    setFailedLogin = true;
    break;
  ...
  default:
    ...
    authResponse =
        TokenStringsParams("AUTHENTICATION FAILED as %s (due to an error)", auth.username.c_str());
    break;
  }
...
#define X(NAME, NAME_STR)                                                                          \
  if (up == NAME_STR) {                                                                            \
    out = cservice::SaslMechanism::NAME;                                                           \
    return true;                                                                                   \
  }
...
#define X(NAME, NAME_STR)                                                                          \
  if (mech == cservice::SaslMechanism::NAME)                                                       \
    return NAME_STR;
  CSERVICE_SASL_MECH_LIST
#undef X
  return "";
...
bool cservice::doXQSASL(iServer* theServer, const string& Routing, const string& Message) {
  ...
  it = std::find_if(saslRequests.begin(), saslRequests.end(), [&](const SaslRequest& req) {
  ...
  if (it != saslRequests.end()) {
    ...
    if (it->mechanism == SaslMechanism::PLAIN) {
      ...
      if (it->fingerprint.empty()) {
        ...
        doXResponse(theServer, Routing,
                    "Client certificate required for SASL EXTERNAL authentication", true);
        saslRequests.erase(it);
        return false;
      }
      ...
      if (it->state == SaslState::INITIAL) {
        ...
        if (it->username.empty() || it->client_nonce.empty()) {
          elog << "[SCRAM] Missing username or client nonce in authentication: " << decodedString
               << endl;
          incStat("SASL." + saslMechanismToString(it->mechanism) + ".ERROR");
          doXResponse(theServer, Routing, "Invalid credentials", true);
          saslRequests.erase(it);
        ...
        elog << "Salt (base64): " << b64encode(it->scram.salt.data(), it->scram.salt.size()) <<
        endl; elog << "StoredKey (base64): " << b64encode(it->scram.storedKey.data(),
        it->scram.storedKey.size()) << endl; elog << "ServerKey (base64): " <<
        b64encode(it->scram.serverKey.data(), it->scram.serverKey.size()) << endl; elog <<
        "StoredKey (hex): "; for (auto c : it->scram.storedKey) elog << std::hex << (int)c << " ";
        elog << std::dec << "\n";
        elog << "ServerKey (hex): ";
        for (auto c : it->scram.serverKey) elog << std::hex << (int)c << " ";
        elog << std::dec << "\n";*/

        it->server_first = "r=" + combinedNonce +
                           ",s=" + b64encode(it->scram.salt.data(), it->scram.salt.size()) +
                           ",i=" + std::to_string(it->scram.iterations);
        std::string serverFirst_b64 =
            b64encode(reinterpret_cast<const unsigned char*>(it->server_first.data()),
                      it->server_first.size());
        ...
        std::string auth_message =
            it->client_first + "," + it->server_first + "," + it->client_final;
        ...
        std::string server_final_b64 = b64encode(
            reinterpret_cast<const unsigned char*>(server_final.data()), server_final.size());
    ...
    AuthStruct auth = {AuthType::XQUERY,
    ...
    if (it->mechanism == SaslMechanism::SCRAM_SHA_256)
      ...
      elog << "Authenticating with username " << it->username << " and password "
           << mask(it->password) << endl;
...
bool cservice::doXQLogin(iServer* theServer, const string& Routing, const string& Message) {
  ...
  //  AB XQ Az iauth:15_d :LOGIN2 <ip-addr> <hostname> <ident> <username> <accountname password
  //  [totptoken]> elog << "cservice::doXQLogin: Routing: " << Routing << " Message: " << Message <<
  //  "\n";
  StringTokenizer st(Message);
  string username;
  string password;
  ...
  if (auth_res == AUTH_SUCCEEDED) {
    ...
    elog << "cservice::doXQLogin: "
         << "Succesful auth for " << username << endl;
    return true;
  }
...
bool cservice::doXQIsCheck(iServer* theServer, const string& Routing, const string& Command,
                           const string& Message) {
  ...
  if (Command == "ISUSER") {
    ...
    for (size_t i = 0; i < SQLDb->Tuples(); i++)
      retUsers[SQLDb->GetValue(i, 0)] = std::make_pair(
          SQLDb->GetValue(i, 1), std::to_string(makeAccountFlags(SQLDb->GetValue(i, 2))));
    ...
    for (const std::string& id : st2) {
      ...
      if (it != retUsers.end()) {
        /* Flush buffer. */
        if (trueString.length() + it->first.length() + it->second.first.length() +
                it->second.second.length() + 2 >
            450) {
...
bool cservice::doXQOplist(const string& chanName) {
  LOG_MSG(TRACE, "Executing XQ for {chan}").with("chan", chanName).logStructured();
...
bool cservice::doXROplist(iServer* /*theServer*/, const string& Routing, const string& Message) {
  ...
  LOG(TRACE,
      "OPLIST scorechan = {} opCount(rank) = {} score = {} account = {} firstOpped = {} lastOpped "
      "= {}",
      scoreChan, opCount, score, account, firstOpped, lastOpped);

  /*	Now we do the actual work to insert scores to SQL DB
          Rules:
                  1). Only do work for channels with a pending application? (we only send XQUERY for
     these) 2). Only INSERT if (channel_id,user_id) pair does not exist, otherwise we UPDATE
  */
  stringstream queryString;
  queryString << "SELECT name,id FROM channels,pending WHERE lower(name)='"
              << string_lower(scoreChan) << "'"
              << " AND channels.id = pending.channel_id" << ends;
  ...
  if (SQLDb->Exec(queryString, true)) {
    if (SQLDb->Tuples() < 1) {
      ...
      stringstream queryString;
      queryString << "SELECT channel_id,first FROM pending_chanfix_scores WHERE user_id='" << userID
                  << "' AND channel_id=(SELECT id FROM channels WHERE lower(name)='"
                  << string_lower(scoreChan) << "')" << ends;
      ...
      if (SQLDb->Exec(queryString, true)) {
        ...
        if (SQLDb->Tuples() < 1) {
          ...
          updateQuery
              << "INSERT INTO pending_chanfix_scores "
              << "(channel_id,user_id,rank,score,account,first_opped,last_opped, first) VALUES ('"
              << chanID << "', '" << userID << "', '" << opCount << "', '" << score << "', '"
              << account << "', '" << firstOpped << "', '" << lastOpped << "', '"
              << "Y"
              << "')" << ends;
        } else {
          ...
          if (first == 'N') {
            updateQuery << "UPDATE pending_chanfix_scores SET "
                        << "rank='" << opCount << "', "
                        << " score='" << score << "', "
                        << " first_opped='" << firstOpped << "',"
                        << " last_opped='" << lastOpped << "',"
                        << " last_updated=date_part('epoch', CURRENT_TIMESTAMP)::int, "
                        << "first='N'"
                        << " WHERE user_id='" << userID << "' AND channel_id='" << chanID
                        << "' AND first='N'" << ends;
          } else {
            updateQuery
                << "INSERT INTO pending_chanfix_scores "
...
bool cservice::doCommonAuth(iClient* theClient, string username) {
  ...
  if (multiAuth) {
    noticeAllAuthedClients(theUser,
                           ...
                           getNickName().c_str(), getUplinkName().c_str());
  }

  /* check to see if we have a last seen time (bug workaround) - if not, make one */
  stringstream queryString;
  queryString << "SELECT last_seen FROM users_lastseen WHERE user_id=" << theUser->getID() << ends;
  ...
  if (SQLDb->Exec(queryString, true)) {
    if (SQLDb->Tuples() < 1) {
      ...
      stringstream updateQuery;
      updateQuery << "INSERT INTO users_lastseen (user_id,"
                  << "last_seen,last_updated) VALUES(" << theUser->getID()
                  << ",date_part('epoch', CURRENT_TIMESTAMP)::int,date_part('epoch', "
                     "CURRENT_TIMESTAMP)::int)"
                  << ends;
  ...
  /* update their details */
  theUser->setLastSeen(currentTime(), theClient->getRealNickUserHost(),
  ...
  networkData* newData = static_cast<networkData*>(theClient->getCustomData(this));
  if (NULL == newData) {
    Notice(theClient, "Internal error.");
    LOG_MSG(ERROR, "newData is NULL for: {client_nick}").with("client", theClient).logStructured();
    return false;
  }
  ...
  Notice(theClient, getResponse(theUser, language::auth_success).c_str(),
         theUser->getUserName().c_str());

  /* set failed logins counter back to zero after alerting the user (if required) */
  if (getConfigVar("ALERT_FAILED_LOGINS")->asInt() == 1 && theUser->getFailedLogins() > 0) {
    /* ok, alert the user */
    Notice(
        theClient,
        getResponse(theUser, language::auth_failed_logins,
                    string("There were %d failed login attempts since your last successful login."))
            .c_str(),
  ...
  if (multiAuth) {
    Notice(theClient,
           getResponse(theUser, language::following_clients_auth,
                       "The following clients are also authenticated as %s:")
               .c_str(),
           theUser->getUserName().c_str());
    ...
    Notice(
        theClient,
        getResponse(theUser, language::suspend_if_not_you,
                    "if any of these clients are not you, your account may have been compromised. "
                    "If you wish to suspend all your access as a precautionary measure, "
                    "type '\002/msg %s@%s suspendme <password>'\002 and contact a "
                    "CService representative to resolve the problem.")
            .c_str(),
        getNickName().c_str(), getUplinkName().c_str());
    Notice(theClient, getResponse(theUser, language::account_unusable_suspendme,
                                  "\002** Note: You will NOT be able to use your account after "
                                  "you issue this command **\002")
                          .c_str());
  }
  ...
  if (tmpLevel > 0) {
    ...
    if (tmpLevel < 900) {
      ...
      logAdminMessage("%s (%s) has authenticated %sat level %d", theClient->getNickName().c_str(),
                      theUser->getUserName().c_str(),
                      theUser->getFlag(sqlUser::F_TOTP_ENABLED) ? "with TOTP " : "", tmpLevel);
    } else {
  ...
  if (theUser->getFlag(sqlUser::F_GLOBAL_SUSPEND)) {
    Notice(theClient, "..however your account has been suspended by a CService administrator."
  ...
  for (autoOpVectorType::const_iterator resultPtr = autoOpVector.begin();
    ...
    if (!(resultPtr->flags & sqlLevel::F_AUTOOP) && !(resultPtr->flags & sqlLevel::F_AUTOVOICE) &&
      ...
      if (resultPtr->flags & sqlLevel::F_AUTOINVITE) {
        ...
        if (level >= level::invite) {
          ...
          if (tmpBan && tmpBan->getLevel() >= 75) {
            Notice(theClient, "Can't invite you to channel %s, you are banned",
                   theChan->getName().c_str());
            continue;
          }
  ...
  if (SQLDb->Tuples() == 1) {
    ...
    Notice(theClient,
           "You have been named as a supporter in a "
           ...
           channelName.c_str(), mngrUserName.c_str(), getNickName().c_str(), channelName.c_str(),
           getNickName().c_str(), channelName.c_str());
    ...
    noticeStr << ends;
    Notice(theClient,
           "You have been named as a supporter for the following "
           "new channel applications: %s",
           noticeStr.str().c_str());
    Notice(theClient,
           "You may visit the "
           "website to register your support or to make an "
 ...
 */
  ...
  if (!theUser->getFlag(sqlUser::F_NONOTES)) {
    ...
    noteQuery << "SELECT message_id FROM notes "
              << "WHERE user_id = " << theUser->getID() << ends;
    ...
    if (count) {
      Notice(theClient,
             "You have %i note(s). To read "
             "them type /msg %s notes read all",
             count, getNickName().c_str());
    }
  }
...
bool cservice::doXResponse(iServer* theServer, const string& Routing, const string& Message,
                           bool kill) {
  string doKill;
  if (kill)
    doKill = "NO";
    ...
    doKill = "OK";
  LOG(TRACE, "XQ-RESPONSE: {} XR {} {} :{} {}", getCharYY().c_str(), theServer->getCharYY().c_str(),
      Routing.c_str(), doKill.c_str(), Message.c_str());
 ...
 */
void cservice::outputChannelAccesses(iClient* theClient, sqlUser* theUser, sqlUser* tmpUser,
                                     unsigned int minlevel) {
  stringstream channelsQuery;
  string channelList;

  channelsQuery << "SELECT channels.name,levels.access,levels.flags FROM levels,channels "
                << "WHERE levels.channel_id = channels.id AND channels.registered_ts <> 0 AND "
                   "levels.user_id = "
                << theUser->getID() << " AND levels.access >= " << minlevel
                << " ORDER BY levels.access DESC" << ends;
  ...
  for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
    ...
    if ((channelList.size() + chanName.size() + chanAccess.size() + 5) >= 450) {
      Notice(theClient, getResponse(tmpUser, language::channels, string("Channels: %s")).c_str(),
             channelList.c_str());
...
void Command::Usage(iClient* theClient) { bot->Notice(theClient, string("SYNTAX: ") + getInfo()); }
...
bool cservice::loadGlines() {
  ...
  for (unsigned int i = 0; i < SQLDb->Tuples(); i++) {
    ...
    tempGline->setAddedOn(static_cast<time_t>(atoi(SQLDb->GetValue(i, 3).c_str())));
...
bool cservice::expireGlines() {
  ...
  for (glineIterator ptr = glineList.begin(); ptr != glineList.end(); ++ptr) {
    ...
    if ((tempGline->getExpires() <= ::time(0)) &&
        ((tempGline->getHost().substr(0, 1) != "#") || (tempGline->getExpires() != 0)))
...
void cservice::addGlineToUplink(csGline* theGline) {
  int Expires;
  if ((theGline->getHost().substr(0, 1) == "#") && (theGline->getExpires() == 0)) {
...
/* This method translates sqlUser flags into accountFlags taking the flags (as a string) as an
 * argument. */
