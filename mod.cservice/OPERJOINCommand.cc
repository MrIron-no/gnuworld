...
bool OPERJOINCommand::Exec([[maybe_unused]] iClient* theClient,
  ...
  bot->Wallops("An IRC Operator is asking me to join channel %s", theChan->getName().c_str());

  bot->logAdminMessage("%s is asking me to join channel %s", theClient->getNickUserHost().c_str(),
