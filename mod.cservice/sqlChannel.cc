...
bool sqlChannel::loadData(const string& channelName) {
  ...
  queryString << "SELECT "
              ...
              << " FROM channels WHERE lower(name) = '" << escapeSQLChars(string_lower(channelName))
              << "'" << ends;
...
bool sqlChannel::insertRecord() {
  static const char* queryHeader = "INSERT INTO channels (name, flags, registered_ts, channel_ts, "
                                   "channel_mode, last_updated, no_take) VALUES (";
