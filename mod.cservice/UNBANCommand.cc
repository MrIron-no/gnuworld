  ...
  for (; counter < st2.size(); counter++) {
      ...
      if (comparison) {
        ...
        if (theBan->getLevel() > level) {
          bot->Notice(
              theClient,
              bot->getResponse(
                     theUser, language::cant_rem_ban,
                     string("You have insufficient access to remove the ban %s from %s's database"))
                  .c_str(),
    ...
    for (; banIterator != oldBans.end(); ++banIterator) {
      ...
      if (sqlBanIterator == theChan->banList.end()) {
        LOG_MSG(
            WARN,
            "Unable to locate sqlBan (id: {ban_id}, mask: {ban_mask}) in sqlChannel {chan_name}")
            .with("ban", theBan)
            .with("chan", theChan)
            .logStructured();
        continue;
