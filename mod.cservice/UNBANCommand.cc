...
bool UNBANCommand::Exec(iClient* theClient, const string& Message) {
    ...
    for (; counter < st2.size(); counter++) {
        ...
        while (ptr != theChan->banList.end()) {
            ...
            if (comparison) {
                ...
                if (theBan->getLevel() > level) {
                    bot->Notice(theClient,
                                bot->getResponse(theUser, language::cant_rem_ban,
                                                 string("You have insufficient access to remove "
                                                        "the ban %s from %s's database"))
                                    .c_str(),
        ...
        for (; banIterator != oldBans.end(); ++banIterator) {
            ...
            if (sqlBanIterator == theChan->banList.end()) {
                LOG_MSG(WARN, "Unable to locate sqlBan (id: {ban_id}, mask: {ban_mask}) in "
                              "sqlChannel {chan_name}")
                    .with("ban", theBan)
                    .with("chan", theChan)
                    .logStructured();
                continue;
        ...
        while (cPtr != theChannel->banList_end()) {
            ...
            if (comparison) {
                banVector.push_back(xServer::banVectorType::value_type(false, *cPtr));
            }
    ...
    if (!respUnban.empty())
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::bans_removed,
                             string("Removed %i channel bans and %i internal bans that matched %s"))
                .c_str(),
    ...
    if (!respUnban.empty() && theChan->getFlag(sqlChannel::F_OPLOG))
        bot->NoticeChannelOps(
            theChan->getName(),
            "%s (%s) removed %i channel bans and %i internal bans that matched %s",
            theClient->getNickName().c_str(), theUser->getUserName().c_str(), banVector.size(),
            banCount, respUnban.c_str());
