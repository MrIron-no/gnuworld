...
bool CERTCommand::Exec([[maybe_unused]] iClient* theClient,
  ...
  if (Command == "LIST") {
    ...
    theQuery
        << "SELECT fingerprint, added_by, added_ts, note FROM users_fingerprints WHERE user_id = "
  ...
  if (Command == "ADD") {
    ...
    /* Add to SQL. */
    std::stringstream theQuery;
    theQuery << "INSERT INTO users_fingerprints (user_id, fingerprint, added_ts, added_by, note) "
                "VALUES ("
