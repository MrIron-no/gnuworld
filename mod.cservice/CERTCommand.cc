...
bool CERTCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (Command == "LIST") {
        bot->Notice(theClient, "+------------------------------------------------------------------"
                               "-------------------------------+-----------------------------------"
                               "-------+--------------+--------------------+");
        bot->Notice(theClient, "| %-95s | %-40s | %-12s | %-18s |", "TLS Fingerprint", "Added By",
                    "Last Updated", "Note");
        bot->Notice(theClient, "+------------------------------------------------------------------"
                               "-------------------------------+-----------------------------------"
                               "-------+--------------+--------------------+");
        ...
        theQuery << "SELECT fingerprint, added_by, added_ts, note FROM users_fingerprints WHERE "
                    "user_id = "
        ...
        if (bot->SQLDb->Tuples() > 0) {
            ...
            bot->Notice(theClient, "+--------------------------------------------------------------"
                                   "-----------------------------------+---------------------------"
                                   "---------------+--------------+--------------------+");
            bot->Notice(theClient, "Done listing %d fingerprint%s.", bot->SQLDb->Tuples(),
                        bot->SQLDb->Tuples() > 1 ? "s" : "");
        } else {
    ...
    if (Command == "ADD") {
        ...
        if (bot->hasFP(theUser) > bot->getConfmaxFingerprints()) {
            bot->Notice(theClient, bot->getResponse(theUser, language::max_fingerprints).c_str(),
                        bot->getConfmaxFingerprints());
            return true;
        }
        ...
        /* We have one param. */
            ...
            /* If it is not a fingerprint, we treat it as a note if (and only if) the user has a
             * fingerprint. */
            ...
            /* If the param is not a fingerprint and the user does not have a fingerprint, fail. */
                ...
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::invalid_fingerprint).c_str());
                return true;
            }
        }
        /* We have either two params, or a note consisting  of several words. */
            ...
            /* If it is not a fingerprint, we treat it as a note if (and only if) the user has a
               fingerprint. */
                ...
                note = st.assemble(2);
            /* If the first word is not a fingerprint and the user does not have a fingerprint,
             * fail. */
            else {
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::invalid_fingerprint).c_str());
        ...
        if (!result.second) {
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::fingerprint_already_exists).c_str());
            return true;
        }
    ...
    if (Command == "REM") {
        ...
        if (bot->hasFP(theUser) == 1 && theUser->getFlag(sqlUser::F_CERTONLY)) {
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::fingerprint_norem_certonly).c_str());
            return true;
        }

        if (!bot->checkFP(fingerPrint, theClient->getAccountID())) {
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::fingerprint_not_found).c_str());
            return true;
        }

        /* Remove from SQL. */
        std::stringstream theQuery;
        theQuery << "DELETE FROM users_fingerprints WHERE fingerprint ='" << fingerPrint << "'"
                 << std::endl;
