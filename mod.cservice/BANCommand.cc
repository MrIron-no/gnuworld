...
bool BANCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (st[1][0] != '#') {
    bot->Notice(theClient, bot->getResponse(theUser, language::inval_chan_name).c_str());
    return false;
  }
  ...
  if (!theChan->getInChan()) {
    bot->Notice(theClient, bot->getResponse(theUser, language::i_am_not_on_chan).c_str());
    return false;
  }
  ...
  if (banLevel < 1 || banLevel > level || 500 < banLevel) {
    bot->Notice(theClient, bot->getResponse(theUser, language::ban_level_range).c_str(),
                (500 < level) ? 500 : level);
    return true;
  }

  if ((banTime < 0) || ((banTime < 300) && (banTime > 0)) ||
      banTime > (int)bot->getConfigVar("MAX_BAN_DURATION")->asInt()) {
    int maxbanhours = ((int)bot->getConfigVar("MAX_BAN_DURATION")->asInt() / 86400);
    bot->Notice(theClient, bot->getResponse(theUser, language::ban_duration).c_str(), maxbanhours);
    return true;
  }
  ...
  if ((int)banReason.size() > max_ban_reason_length) {
    bot->Notice(theClient, bot->getResponse(theUser, language::ban_reason_size).c_str(),
  ...
  if (max_bans > 0) {
    ...
    if (ban_count >= max_bans) {
      /* banlist is full */
      bot->Notice(theClient, "Sorry, The channel banlist is full (%i bans)", ban_count);
      return true;
    }
  }
  ...
  if ((int)st2.size() > max_multibans) {
    bot->Notice(theClient, "Sorry, you can only add %i bans with one BAN command.", max_multibans);
    return true;
  }
  ...
  for (; counter < st2.size(); counter++) {
    ...
    while (ptr != theChan->banList.end()) {
      ...
      // More specific ban?
        ...
        bot->Notice(theClient, bot->getResponse(theUser, language::ban_covered).c_str(),
    ...
    for (; banIterator != oldBans.end(); ++banIterator) {
      ...
      sqlChannel::sqlBanMapType::iterator sqlBanIterator = theChan->banList.find(theBan->getID());
      if (sqlBanIterator == theChan->banList.end()) {
        LOG_MSG(WARN,
                "Unable to find ban (id: {ban_id}, name: {ban_name}) in sqlChannel {chan_name}")
            .with("ban", theBan)
            .with("chan", theChan)
            .logStructured();
      } else {
    ...
    sqlBan* newBan = new (std::nothrow) sqlBan(
        bot, theChan->getID(), banTarget, theUser->getUserName(), bot->currentTime(), banLevel);
    assert(newBan != 0);
    ...
    if (foundClient)
      banVector.push_back(xServer::banVectorType::value_type(true, banTarget));
  ...
  if (level < 500 &&
    ...
    string theMessage =
        TokenStringsParams("Take over attempt by %s (%s) on channel %s with banmask %s",
                           theClient->getNickName().c_str(), theUser->getUserName().c_str(),
                           theChan->getName().c_str(), st[2].c_str());
  ...
  if (!respNotOnChan.empty())
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::dont_see_them, string("I don't see %s anywhere"))
            .c_str(),
        respNotOnChan.c_str());
  ...
  if (!respExists.empty())
    bot->Notice(theClient, bot->getResponse(theUser, language::ban_exists).c_str(),
                respExists.c_str());
  ...
  if (!respBans.empty())
    bot->Notice(
        ...
        respBans.c_str(), theChannel->getName().c_str(), banLevel);
