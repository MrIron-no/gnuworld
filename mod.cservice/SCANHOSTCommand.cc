...
bool SCANHOSTCommand::Exec(iClient* theClient, const string& Message) {
  ...
  scanhostQuery
      << "SELECT user_id, user_name, command, ip, hostmask, timestamp FROM user_sec_history WHERE "
      << "(hostmask ILIKE '" << escapeSQLChars(searchSQL(host)) << "'"
      << ") OR (ip ILIKE '" << escapeSQLChars(searchSQL(host)) << "') ORDER BY " << sortby
      << (desc == true ? " DESC" : "") << limto << ends;
  ...
  if (bot->SQLDb->Tuples() > 15) {
    bot->Notice(
        theClient,
        "More than 15 matches were found without the use of -max n, please visit the website.");
    return false;
  }
  ...
  for (scanResultsType::const_iterator Itr = scanResults.begin(); Itr != scanResults.end(); ++Itr) {
    ...
    if (tmpUser) {
      ...
      if ((tmpadminLevel > 0 || tmpUser->getFlag(sqlUser::F_OPER)) && level < 800) {
        ...
        /* display entry */
        stringstream s;
        s << "user: " << username << " (" << Itr->second.user_id << ")"
          << " -- cmd: " << Itr->second.command << " -- host: " << Itr->second.hostmask
          << " -- ip: " << Itr->second.ip << " -- when: " << Itr->second.timestamp << ends;
