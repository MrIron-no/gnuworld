...
bool DEVOICECommand::Exec(iClient* theClient, const string& Message) {
    ...
    while (counter < st2.size()) {
        ...
        if (cont)
            tmpChanUser = tmpChan->findUser(target);
        ...
        if (cont) {
            ...
            if (ptr == duplicateMap.end())                              // Not a duplicate.
            {
                ...
                if (target !=
                    theClient) // Don't send a notice to the person who issued the command.
                {
                    sqlUser* tmpTargetUser = bot->isAuthed(target, false);
                    if (tmpTargetUser) {
                        bot->Notice(
                            target,
                            bot->getResponse(tmpTargetUser, language::youre_devoiced_by).c_str(),
                            theClient->getNickName().c_str(), theUser->getUserName().c_str(),
                            theChan->getName().c_str());
                    } else {
                        bot->Notice(target,
                                    bot->getResponse(NULL, language::youre_devoiced_by).c_str(),
    ...
    if (!devoiceList.empty()) {
        ...
        if (theChan->getFlag(sqlChannel::F_OPLOG)) {
            ...
            bot->NoticeChannelOps(theChan->getName(), "%s (%s) devoiced: %s",
                                  theClient->getNickName().c_str(), theUser->getUserName().c_str(),
                                  devoiceStr.c_str());
        }
    }
