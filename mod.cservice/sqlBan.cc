...
bool sqlBan::commit() {
  ...
  queryString << queryHeader << "SET channel_id = " << channel_id << ", "
...
bool sqlBan::insertRecord() {
  ...
  static const char* queryHeader =
      "INSERT INTO bans (channel_id,banmask,set_by,set_ts,level,expires,reason,last_updated) "
      "VALUES (";

  stringstream queryString;
  queryString << queryHeader << channel_id << ", '" << escapeSQLChars(banmask) << "', '"
              << escapeSQLChars(set_by) << "', " << set_ts << ", " << level << ", " << expires
