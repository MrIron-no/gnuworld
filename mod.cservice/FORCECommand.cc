...
bool FORCECommand::Exec(iClient* theClient, const string& Message) {
  ...
  if ((theChan->getFlag(sqlChannel::F_NOFORCE)) && (admLevel < level::immune::noforce)) {
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::chan_noforce, string("The NOFORCE flag is set on %s"))
            .c_str(),
        st[1].c_str());
    return false;
  }
  ...
  theChan->forceMap.insert(sqlChannel::forceMapType::value_type(
      theUser->getID(), std::make_pair(admLevel, theUser->getUserName())));

  bot->logAdminMessage("%s (%s) is getting access on %s", theClient->getNickName().c_str(),
                       theUser->getUserName().c_str(), theChan->getName().c_str());
  bot->Notice(theClient,
              bot->getResponse(theUser, language::temp_inc_access,
                               string("Temporarily increased your access on channel %s to %i"))
                  .c_str(),
