...
public:
  ...
  inline const time_t& getInstantiatedTS() const { return instantiated_ts; }

  inline const time_t& getSignupTS() const { return signup_ts; }

  inline const std::string& getEmail() const { return email; }

  inline const std::string& getVerifData() const { return verifdata; }

  inline bool isAuthed() { return (networkClientList.size() != 0); }

  inline void addAuthedClient(iClient* theClient) { networkClientList.push_back(theClient); };
  ...
  inline const unsigned int& getMaxLogins() const { return maxlogins; }

  inline const time_t& getLastNote() const { return last_note; }

  inline const unsigned int& getNotesSent() const { return notes_sent; }

  inline const unsigned int& getFailedLogins() const { return failed_logins; }

  inline const unsigned int& getLastFailedLoginTS() const { return failed_login_ts; }
  ...
  inline void setPassword(const std::string& _password) { password = _password; }

  inline void setLastSeen(const time_t& _last_seen, const std::string& _last_hostmask,
  ...
  inline void setSignUpTS(const time_t& _signup_ts) { signup_ts = _signup_ts; }

  inline void setEmail(const std::string& _email) { email = _email; }

  inline void setVerifData(const std::string& _verifdata) { verifdata = _verifdata; }

  inline void setMaxLogins(const unsigned int& _maxlogins) { maxlogins = _maxlogins; }

  inline void setLastNote(const time_t& _last_note) { last_note = _last_note; }

  inline void setNotesSent(const unsigned int& _notes_sent) { notes_sent = _notes_sent; }
  ...
  inline void incFailedLogins() { failed_logins++; }

  inline void setFailedLogins(const unsigned int& _failed_logins) {
    failed_logins = _failed_logins;
  }

  inline void setLastFailedLoginTS(const unsigned int& _ts) { failed_login_ts = _ts; }
