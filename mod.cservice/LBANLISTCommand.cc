...
bool LBANLISTCommand::Exec(iClient* theClient, const string& Message) {
  ...
  bot->Notice(theClient,
              bot->getResponse(theUser, language::lbanlist_for,
                               string("\002*** Ban List for channel %s ***\002"))
                  .c_str(),
  ...
  for (std::map<int, sqlBan*>::const_iterator ptr = theChan->banList.begin();
       ptr != theChan->banList.end(); ++ptr) {
    ...
    if ((ban_expires >= bot->currentTime()) || (ban_expires == 0)) {
      if (!match(fixAddress(st[2]), theBan->getBanMask())) {
        ...
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::lban_entry, string("%s %s Level: %i")).c_str(),
            theChan->getName().c_str(), theBan->getBanMask().c_str(), theBan->getLevel());
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::lban_added_by, string("ADDED BY: %s (%s)")).c_str(),
            theBan->getSetBy().c_str(), theBan->getReason().c_str());
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::lban_since, string("SINCE: %s")).c_str(),
                    ctime(&theBan->getSetTS()));
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::lban_exp, string("EXP: %s")).c_str(),
  ...
  if ((results > MAX_LBAN_RESULTS) && !showAll) {
    bot->Notice(theClient,
                bot->getResponse(theUser, language::more_than_max,
                                 string("There are more than %d matching entries."))
                    .c_str(),
                MAX_LBAN_RESULTS);
    bot->Notice(theClient, bot->getResponse(theUser, language::restrict_query,
                                            string("Please restrict your query.")));
  } else if (results > 0) {
    bot->Notice(theClient,
                bot->getResponse(theUser, language::lban_end, string("\002*** END ***\002")));
  } else {
    bot->Notice(theClient, bot->getResponse(theUser, language::no_match, string("No match!")));
  }
