...
bool PURGECommand::Exec(iClient* theClient, const string& Message) {
  ...
  if ((!theChan) || (st[1] == "*")) {
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::chan_not_reg, string("%s isn't registered with me"))
            .c_str(),
        st[1].c_str());
    return false;
  }
  ...
  managerQuery << "SELECT users.user_name,users.email "
               ...
               << "AND levels.channel_id = " << theChan->getID() << " LIMIT 1" << ends;
      ...
      if (!opList.empty()) {
        ...
        if (tmpBotUser) {
          if (!tmpBotUser->getMode(ChannelUser::MODE_O)) {
            ...
            stringstream s;
            s << bot->getCharYY() << " M " << theChan->getName() << " +o " << bot->getCharYYXXX()
              << ends;
            bot->Write(s);
  ...
  bot->logAdminMessage("%s (%s) has purged %s (%s)", theClient->getNickName().c_str(),
                       theUser->getUserName().c_str(), theChan->getName().c_str(), reason.c_str());

  bot->Notice(theClient,
              bot->getResponse(theUser, language::purged_chan, string("Purged channel %s")).c_str(),
              st[1].c_str());

  bot->writeChannelLog(theChan, theClient, sqlChannel::EV_PURGE,
                       "has purged " + theChan->getName() + " (" + reason + "), " + "Manager was " +
