...
static const char* queryHeader =
    "SELECT "
    "channels.name,users.user_name,levels.access,levels.flags,users_lastseen.last_seen,levels."
    "suspend_expires,levels.last_modif,levels.last_modif_by,levels.suspend_level,levels.suspend_"
    "reason FROM levels,channels,users,users_lastseen ";
static const char* queryCondition = "WHERE levels.channel_id=channels.id AND "
                                    "levels.user_id=users.id AND users.id=users_lastseen.user_id ";
...
bool ACCESSCommand::Exec(iClient* theClient, const string& Message) {
  ...
  for (StringTokenizer::const_iterator ptr = st.begin(); ptr != st.end(); ++ptr) {
    ...
    case 1: /* Min */
    {
      ...
      if ((minAmount > 1000) /* || (minAmount < 0) */) {
        bot->Notice(theClient, bot->getResponse(theUser, language::inval_min_lvl).c_str());
        return false;
      }
      currentType = 0;
      break;
    ...
    case 2: /* Max */
    {
      ...
      if ((maxAmount > 1000) /* || (maxAmount < 0) */) {
        bot->Notice(theClient, bot->getResponse(theUser, language::inval_max_lvl).c_str());
        return false;
      }
  ...
  theQuery << queryHeader << queryCondition << extraCond.str().c_str()
           << "AND levels.channel_id = " << theChan->getID() << " " << queryFooter << ends;
  ...
  for (unsigned int i = 0; i < bot->SQLDb->Tuples(); i++) {
    ...
    if (match(matchString, bot->SQLDb->GetValue(i, 1)) == 0) {
      ...
      if (aVoice == true || aOp == true || aNone == true) {
        if (aNone == true) {
          if (!aVoice && (flag & sqlLevel::F_AUTOVOICE))
            continue;
          if (!aOp && (flag & sqlLevel::F_AUTOOP))
            continue;
        } else {
          if (!(flag & sqlLevel::F_AUTOVOICE) && !(flag & sqlLevel::F_AUTOOP))
            continue;
          if (!aVoice && (flag & sqlLevel::F_AUTOVOICE))
            continue;
          if (!aOp && (flag & sqlLevel::F_AUTOOP))
      ...
      bot->Notice(theClient, bot->getResponse(theUser, language::user_access_is).c_str(),
                  bot->SQLDb->GetValue(i, 1).c_str(), bot->SQLDb->GetValue(i, 2).c_str(),
                  bot->userStatusFlags(bot->SQLDb->GetValue(i, 1)).c_str());

      bot->Notice(theClient, bot->getResponse(theUser, language::channel_automode_is).c_str(),
                  bot->SQLDb->GetValue(i, 0).c_str(), autoMode.c_str());
      ...
      if (suspend_expires > bot->currentTime()) {
        bot->Notice(theClient, bot->getResponse(theUser, language::suspend_expires_in).c_str(),
                    prettyDuration(suspend_expires_f).c_str(), suspendLevel);
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::susp_reason, string("Reason: %s")).c_str(),
                    suspReason.c_str());
      } else if ((suspReason != "") && (suspend_expires == 0))
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::unsusp_reason, string("UNSUSPENDED - %s")).c_str(),
            suspReason.c_str());
      bot->Notice(theClient, bot->getResponse(theUser, language::last_seen).c_str(),
                  prettyDuration(duration).c_str());

      if (modif) {
        bot->Notice(theClient, bot->getResponse(theUser, language::last_mod).c_str(),
  ...
  if ((results >= MAX_ACCESS_RESULTS) && !showAll) {
    bot->Notice(theClient, bot->getResponse(theUser, language::more_than_max).c_str(),
                MAX_ACCESS_RESULTS);
    bot->Notice(theClient, bot->getResponse(theUser, language::restrict_query).c_str());
  } else if (results > 0) {
    bot->Notice(theClient, bot->getResponse(theUser, language::end_access_list).c_str());
  } else {
