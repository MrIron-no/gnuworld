...
bool CLEARMODECommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (!theChan->getInChan()) {
    bot->Notice(theClient,
                bot->getResponse(theUser, language::i_am_not_on_chan, "I'm not in that channel!"));
    return false;
  }
  ...
  bot->ClearMode(tmpChan, string("mstnipklrDcCuMZ"), false);

  bot->Notice(
      theClient,
      bot->getResponse(theUser, language::modeclear_done, string("%s: Cleared channel modes."))
          .c_str(),
      theChan->getName().c_str());

  // Send action opnotice to channel if OPLOG is enabled
  if (theChan->getFlag(sqlChannel::F_OPLOG)) {
    bot->NoticeChannelOps(theChan->getName(), "%s (%s) cleared channel modes.",
