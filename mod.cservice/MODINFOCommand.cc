...
bool MODINFOCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (command == "INVITE") { // Handle modinfo INVITE
    ...
    if (st[1] == "*") { // Admin channel?
      bot->Notice(theClient,
                  "It is a mistake to think you can solve any major problems just with potatoes.");
      return false;
    }
    if (string_upper(st[3]) == "ON") { // Time to enable INVITE option
      sqlLevel* aLevel = bot->getLevelRecord(theUser, theChan);
      aLevel->setFlag(sqlLevel::F_AUTOINVITE);
      aLevel->commit();

      bot->Notice(theClient,
                  bot->getResponse(theUser, language::automode_invite,
                                   string("Set INVITE on login %s for channel %s"))
                      .c_str(),
  ...
  if (!targetUser) {
    bot->Notice(theClient,
                bot->getResponse(theUser, language::not_registered,
                                 string("Sorry, I don't know who %s is."))
                    .c_str(),
                st[3].c_str());
    return false;
  }
  ...
  if (targetLevel == 0) {
    bot->Notice(theClient,
                bot->getResponse(theUser, language::doesnt_have_access,
                                 string("%s doesn't appear to have access in %s."))
                    .c_str(),
  ...
  if (command == "ACCESS") {
    ...
    if (level <= targetLevel) {
      ...
      if (!bot->isForced(theChan, theUser)) {
        bot->Notice(theClient,
                    bot->getResponse(
                        theUser, language::mod_access_higher,
    ...
    if ((newAccess <= 0) || (newAccess > 999)) {
      bot->Notice(theClient, bot->getResponse(theUser, language::inval_access,
    ...
    if (level <= newAccess) {
      bot->Notice(theClient, bot->getResponse(
                                 theUser, language::cant_give_higher,
    ...
    if ((theChan->getUserFlags() == 2) && (newAccess >= level::voice) &&
        (!aLevel->getFlag(sqlLevel::F_AUTOVOICE)))
      aLevel->setFlag(sqlLevel::F_AUTOVOICE);
    if ((theChan->getUserFlags() == 1) && (newAccess >= level::op) &&
        (!aLevel->getFlag(sqlLevel::F_AUTOOP)))
      aLevel->setFlag(sqlLevel::F_AUTOOP);

    aLevel->commit();

    bot->Notice(theClient,
                bot->getResponse(theUser, language::mod_access_to,
                                 string("Modified %s's access level on channel %s to %i"))
                    .c_str(),
                targetUser->getUserName().c_str(), theChan->getName().c_str(), newAccess);
    if (targetUser != theUser)
      bot->NoteAllAuthedClients(
          targetUser,
          bot->getResponse(targetUser, language::acc_modif,
                           string("Your access on %s has been modified from %i to %i"))
              .c_str(),
          theChan->getName().c_str(), targetLevel, newAccess);
    // Announce the manager about the new access change
    if (level < 500) {
      string theMessage = TokenStringsParams(
          "%s modified %s's access on channel %s from %i to %i", theUser->getUserName().c_str(),
          targetUser->getUserName().c_str(), theChan->getName().c_str(), targetLevel, newAccess);
  ...
  if (command == "AUTOMODE") {
    ...
    if (string_upper(st[4]) == "OP") {
      ...
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::automode_op,
                                   string("Set AUTOMODE to OP for %s on channel %s"))
                      .c_str(),
                  targetUser->getUserName().c_str(), theChan->getName().c_str());
    ...
    if (string_upper(st[4]) == "VOICE") {
      ...
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::automode_voice,
                                   string("Set AUTOMODE to VOICE for %s on channel %s"))
                      .c_str(),
                  targetUser->getUserName().c_str(), theChan->getName().c_str());
      return false;
    }
    ...
    if (string_upper(st[4]) == "NONE") {
      ...
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::automode_none,
                                   string("Set AUTOMODE to NONE for %s on channel %s"))
                      .c_str(),
                  targetUser->getUserName().c_str(), theChan->getName().c_str());
      return false;
    }
