...
bool sqlUser::loadData(int userID) {
  ...
  queryString << "SELECT " << sql::user_fields << " FROM users WHERE id = " << userID << ends;
...
bool sqlUser::loadData(const string& userName) {
  ...
  stringstream queryString;
  queryString << "SELECT " << sql::user_fields << " FROM users WHERE lower(user_name) = '"
...
bool sqlUser::commit(std::string last_updated_by) {
  ...
  queryString << queryHeader << "SET flags = " << flags << ", "
              ...
              << "scram_record = '" << escapeSQLChars(scram_record) << "' " << queryCondition << id
              << ends;
...
bool sqlUser::commitLastSeen() {
  ...
  stringstream queryString;
  queryString << queryHeader << "SET last_seen = " << last_seen << ", "
              << "last_hostmask = '" << escapeSQLChars(last_hostmask) << "', "
              << "last_ip = '" << escapeSQLChars(last_ip) << "', "
              << "last_updated = date_part('epoch', CURRENT_TIMESTAMP)::int " << queryCondition
              << id << ends;
...
time_t sqlUser::getLastSeen() {
  ...
  queryString << "SELECT last_seen"
              << " FROM users_lastseen WHERE user_id = " << id << ends;
...
void sqlUser::writeEvent(unsigned short eventType, sqlUser* theUser, const string& theMessage) {
  ...
  theLog << "INSERT INTO userlog (ts, user_id, event, message, "
         ...
         << ", " << id << ", " << eventType << ", "
         << "'" << escapeSQLChars(theMessage) << " (By " << userExtra << ")"
...
const string sqlUser::getLastEvent(unsigned short eventType, unsigned int& eventTime) {
  ...
  queryString << "SELECT message,ts"
              << " FROM userlog WHERE user_id = " << id << " AND event = " << eventType
              << " ORDER BY ts DESC LIMIT 1" << ends;
...
bool sqlUser::Insert() {
  ...
  stringstream queryString;
  queryString << queryHeader << escapeSQLChars(user_name) << "'," << "'" << password << "'," << 1
              << "," << 0 << ",'" << escapeSQLChars(last_updated_by) << "',"
              << "date_part('epoch', CURRENT_TIMESTAMP)::int,"
