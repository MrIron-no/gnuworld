...
bool SETCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (st[1][0] != '#') // Didn't find a hash?
    {
        ...
        if (option == "INVISIBLE") {
            if (value == "ON") {
                ...
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::invis_on,
            ...
            if (value == "OFF") {
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::invis_off,
        ...
        if (option == "NOADDUSER") {
            if (value == "ON") {
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::noadduser_on,
            ...
            if (value == "OFF") {
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::noadduser_off,
        ...
        if (option == "AUTOHIDE") {
            if (value == "ON") {
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::autohide_on,
            ...
            if (value == "OFF") {
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::autohide_off,
        ...
        if (option == "CERTONLY") {
            if (value == "ON") {
                if (!bot->hasFP(theUser)) {
                    bot->Notice(
                        theClient,
                        bot->getResponse(
                            theUser, language::no_fingerprints_registered,
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::certonly_on,
            ...
            if (value == "OFF") {
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::certonly_off,
        ...
        if (option == "LANG") {
            ...
            if (ptr != bot->languageTable.end()) {
                ...
                theUser->commit(theClient);
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::lang_set_to,
                                             string("Language is set to %s."))
                                .c_str(),
                            lang.c_str());
                return true;
            }
        ...
        /* Require management of TOTP via website
                ...
                if (option == "TOTP")
                {
                        ...
                        if((admin > 0) || (theUser->getFlag(sqlUser::F_OPER)) ||
        (theClient->isOper())) { if(value == "ON") { if(theUser->getFlag(sqlUser::F_TOTP_ENABLED)) {
                                                bot->Notice(theClient,"TOTP is already enabled for
        your account"); return true;
                                        }
        ...
        secret key is: %s",key); bot->Notice(theClient,"Your key in hex string: %s",hex_key);
                                        bot->Notice(theClient,"For QR representation of your key,
        visit : https://cservice.undernet.org/genqr.php?code=%s&name=UnderNet",key);
        ...
        already enabled for your account"); return true;
                                        ...
                                        if(st.size() == 3) {
                                                bot->Notice(theClient,"Usage: /msg %s set totp
        CONFIRM <token>",bot->getNickName().c_str()); return true;
                                        }
                                        if(theUser->getTotpKey() == "") {
                                                bot->Notice(theClient,"Before confirming TOTP it
        must be enabled using /msg %s set TOTP on",bot->getNickName().c_str()); return true;
                                        }
                                        char* key;
                                        size_t len;
                                        int res  =
        oath_base32_decode(theUser->getTotpKey().c_str(),theUser->getTotpKey().size(),&key,&len);
                                        if(res != OATH_OK) {
                                                bot->Notice(theClient,"TOTP key validation failed
        due to an error, please contact CService represetitive"); elog << "ERROR while decoding
        base32 (" << st[st.size()-1].c_str() << ") " << oath_strerror(res) << "\n"; return false;
                                        }
                                        res=oath_totp_validate(key,len,time(NULL),30,0,1,st[3].c_str());
                                        free(key);
                                        if(res < 0 ) {
                                                bot->Notice(theClient,"TOTP validation failed,
        invalid key, please make sure youre device is configured properly with the correct key");
                                                return false;
                                        }
                                        if(st.size() ==4) {
                                                bot->Notice(theClient,"WARNING:  This will enable
        time-based OTP (one time passwords).  Once enabled, in order to login you will require a
        device to generate the OTP token which has the stored secret key.  If you are sure, type:
        /msg %s set totp CONFIRM <token> -force",bot->getNickName().c_str()); return true;
                                        }
        ...
        authentication, please contact a cservice representitive"); return true;
                                                ...
                                                bot->Notice(theClient,"TOTP Authentication is
        ENABLED"); return true; } else { bot->Notice(theClient,"Invalid option %s",st[4].c_str());
                                                return true;
                                        }
                                }
                        }
            ...
            for (sqlChannel::forceMapType::const_iterator ptr = theChan->forceMap.begin();
                ...
                if (ForceLevel < level::immune::noforce) {
                    ...
                    bot->noticeAllAuthedClients(
                        AdminUser,
                        bot->getResponse(
                               AdminUser, language::rem_temp_access,
    ...
    if (option == "TAKEREVENGE") {
        ...
        bot->Notice(theClient, "TAKEREVENGE for %s is %s", theChan->getName().c_str(),
                    value.c_str());
        return true;
    }
#endif
    ...
    if (option == "FLOODPRO") {
        ...
        if ((value == "ON") || (value == "KICK")) {
            ...
            bot->Notice(theClient, "Value of %s must be ON (KICK), BAN, DEFAULT, or OFF",
                        option.c_str());
            return true;
        }
    ...
    if (option == "NOTICEFLOOD") {
        ...
        theChan->commit();
        bot->Notice(theClient, "NOTICEFLOOD for %s is set to %d", theChan->getName().c_str(),
                    numValue);
        return true;
    }
    if (option == "CTCPFLOOD") {
        ...
        theChan->commit();
        bot->Notice(theClient, "CTCPFLOOD for %s is set to %d", theChan->getName().c_str(),
                    numValue);
        return true;
    }
    if ((option == "FLOODPERIOD") || (option == "FLOODPRD")) {
        ...
        theChan->commit();
        bot->Notice(theClient, "FLOODPERIOD for %s is set to %d", theChan->getName().c_str(),
                    numValue);
        if (numValue == 0) {
            theChan->removeFlag(sqlChannel::F_FLOODPRO);
            theChan->setFloodproLevel(sqlChannel::FLOODPRO_NONE);
            ...
            bot->Notice(
                theClient,
                bot->getResponse(theUser, language::set_cmd_status, string("%s for %s is %s"))
                    .c_str(),
    ...
    if ((option == "REPEATFLOOD") || (option == "REPFLOOD")) {
        ...
        theChan->commit();
        bot->Notice(theClient, "REPEATFLOOD for %s is %s", theChan->getName().c_str(),
                    value.c_str());
        return true;
    }
#endif
            ...
            if ((setting < 0) || (setting > 2)) {
                bot->Notice(
                    ...
                    bot->getResponse(
                        ...
                        string(
                            "Invalid USERFLAGS setting. Correct values are NONE, OP or VOICE.")));
                return false;
            }
    ...
    if (option == "DESCRIPTION" || option == "DESC") {
        ...
        if (desc == "") {
            ...
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::desc_status,
                                         string("DESCRIPTION for %s is: %s"))
                            .c_str(),
    ...
    if (option == "URL") {
        ...
        if (url == "") {
            ...
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::url_status, string("URL for %s is: %s"))
                            .c_str(),
    ...
    if (option == "WELCOME") {
        ...
        if ((string_upper(welcome) == "OFF") || (welcome == "")) {
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::welcome_cleared,
                                         string("WELCOME for %s is cleared."))
                            .c_str(),
                        theChan->getName().c_str());
            theChan->setWelcome("");
            theChan->commit();
    ...
    if (option == "COMMENT") {
        ...
        if (comment.empty()) {
            ...
            bot->Notice(theClient, "COMMENT for %s is: %s", theChan->getName().c_str(),
                        comment.c_str());
        }
            ...
            if (limit_joins < (int)bot->getConflimitJoinMaxLowest() ||
                ...
                bot->Notice(theClient,
                            bot->getResponse(theUser, language::set_joinmax_joinrange,
                                ...
                                .c_str(),
                            (int)bot->getConflimitJoinMaxLowest(),
                            (int)bot->getConflimitJoinMaxHighest());
                return true;
            }
    ...
    if (option == "JOINMODE") {
        ...
        if (invalidParam || numParam != (st.size() - 4)) {
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::set_joinmode_params,
                                         string("Invalid number of JOINMODE parameters: %i modes "
                                                "and %i parameters provided"))
                            .c_str(),
                        numParam, st.size() - 4);
            return true;
        }
