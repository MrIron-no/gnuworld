...
bool SETCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (st[1][0] != '#') // Didn't find a hash?
  {
    ...
    if (option == "INVISIBLE") {
      if (value == "ON") {
        ...
        bot->Notice(theClient, bot->getResponse(theUser, language::invis_on,
      ...
      if (value == "OFF") {
        ...
        theUser->commit(theClient);
        bot->Notice(theClient, bot->getResponse(theUser, language::invis_off,
    ...
    if (option == "NOADDUSER") {
      if (value == "ON") {
        ...
        theUser->commit(theClient);
        bot->Notice(theClient, bot->getResponse(theUser, language::noadduser_on,
      ...
      if (value == "OFF") {
        ...
        theUser->commit(theClient);
        bot->Notice(theClient, bot->getResponse(theUser, language::noadduser_off,
    ...
    if (option == "AUTOHIDE") {
      if (value == "ON") {
        ...
        theUser->commit(theClient);
        bot->Notice(theClient, bot->getResponse(theUser, language::autohide_on,
      ...
      if (value == "OFF") {
        ...
        theUser->commit(theClient);
        bot->Notice(theClient, bot->getResponse(theUser, language::autohide_off,
    ...
    if (option == "CERTONLY") {
      if (value == "ON") {
        if (!bot->hasFP(theUser)) {
          bot->Notice(
              theClient,
              bot->getResponse(theUser, language::no_fingerprints_registered,
                               string("You currently don't have any fingerprints added to your "
                                      "account. For more information, use '/msg X help cert'")));
          return true;
        }
        theUser->setFlag(sqlUser::F_CERTONLY);
        theUser->commit(theClient);
        bot->Notice(theClient, bot->getResponse(theUser, language::certonly_on,
      ...
      if (value == "OFF") {
        ...
        theUser->commit(theClient);
        bot->Notice(theClient, bot->getResponse(theUser, language::certonly_off,
    ...
    if (option == "DISABLEAUTH") {
      if (value == "ON") {
        ...
        if (admLevel > 0) {
          ...
          bot->logAdminMessage("%s (%s) set DISABLEAUTH on!", theClient->getNickName().c_str(),
                               theUser->getUserName().c_str());
        } else {
          /* not an admin, return unknown command */
          bot->Notice(theClient, bot->getResponse(theUser, language::invalid_option,
      ...
      if (value == "OFF") {
        ...
        if (theUser->getFlag(sqlUser::F_NOADMIN)) {
          ...
          bot->logAdminMessage("%s (%s) set DISABLEAUTH off!", theClient->getNickName().c_str(),
                               theUser->getUserName().c_str());
        } else {
    ...
    if (option == "NOPURGE") {
      ...
      if (!admLevel) {
        /* not an admin, return unknown command */
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::invalid_option, string("Invalid option.")));
        return true;
      }
      ...
      if (value == "ON") {
        ...
        bot->Notice(theClient, "NOPURGE setting for %s is now ON",
      ...
      if (value == "OFF") {
        ...
        targetUser->commit(theClient);
        bot->Notice(theClient, "NOPURGE setting for %s is now OFF",
                    targetUser->getUserName().c_str());
        return true;
      }
      bot->Notice(theClient,
    ...
    if (option == "LANG") {
      ...
      if (ptr != bot->languageTable.end()) {
        ...
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::lang_set_to, string("Language is set to %s."))
                .c_str(),
            lang.c_str());
        return true;
      }
    ...
    { if(value == "ON") { if(theUser->getFlag(sqlUser::F_TOTP_ENABLED)) {
                                            bot->Notice(theClient,"TOTP is already enabled for your
    account"); return true;
                                    }
                                    ...
                                    if(res != OATH_OK) {
                                            bot->Notice(theClient,"Failed to enable TOTP
    authentication, please contact a cservice representitive"); return true;
                                    }
                                    theUser->setTotpKey(string(key));
                                    if(!theUser->commit(theClient)) {
                                            bot->Notice(theClient,"Failed to enable TOTP
    authentication, please contact a cservice representitive"); free(key); return true;
                                    }

                                    bot->Notice(theClient,"TOTP key set.  Your base 32 encoded
    secret key is: %s",key); bot->Notice(theClient,"Your key in hex string: %s",hex_key);
                                    bot->Notice(theClient,"For QR representation of your key, visit
    : https://cservice.undernet.org/genqr.php?code=%s&name=UnderNet",key);
                                    bot->Notice(theClient,"Please note, this key will never be
    presented to you again.  NEVER GIVE YOUR KEY TO ANYONE!"); bot->Notice(theClient,"To confirm
    TOTP activation please configure your device with the above key and type  /msg %s set TOTP
    confirm <token>",bot->getNickName().c_str()); free(key); return true; } else if(value ==
    "CONFIRM") { if(theUser->getFlag(sqlUser::F_TOTP_ENABLED)) { bot->Notice(theClient,"TOTP is
    already enabled for your account"); return true;
                                    }
                                    if(st.size() == 3) {
                                            bot->Notice(theClient,"Usage: /msg %s set totp CONFIRM
    <token>",bot->getNickName().c_str()); return true;
                                    }
                                    if(theUser->getTotpKey() == "") {
                                            bot->Notice(theClient,"Before confirming TOTP it must be
    enabled using /msg %s set TOTP on",bot->getNickName().c_str()); return true;
                                    }
                                    char* key;
                                    size_t len;
                                    int res  =
    oath_base32_decode(theUser->getTotpKey().c_str(),theUser->getTotpKey().size(),&key,&len); if(res
    != OATH_OK) { bot->Notice(theClient,"TOTP key validation failed due to an error, please contact
    CService represetitive"); elog << "ERROR while decoding base32 (" << st[st.size()-1].c_str() <<
    ") " << oath_strerror(res) << "\n"; return false;
                                    }
                                    res=oath_totp_validate(key,len,time(NULL),30,0,1,st[3].c_str());
                                    free(key);
                                    if(res < 0 ) {
                                            bot->Notice(theClient,"TOTP validation failed, invalid
    key, please make sure youre device is configured properly with the correct key"); return false;
                                    }
                                    if(st.size() ==4) {
                                            bot->Notice(theClient,"WARNING:  This will enable
    time-based OTP (one time passwords).  Once enabled, in order to login you will require a device
    to generate the OTP token which has the stored secret key.  If you are sure, type: /msg %s set
    totp CONFIRM <token> -force",bot->getNickName().c_str()); return true;
                                    }
  ...
  if (option == "LOCKED") {
    ...
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::set_cmd_status, string("%s for %s is %s")).c_str(),
        option.c_str(), theChan->getName().c_str(),
  ...
  if (theChan->getFlag(sqlChannel::F_LOCKED)) {
    ...
    if (admLevel < level::set::locked) {
      bot->Notice(theClient,
                  "The channel settings for %s have been locked by a cservice"
  ...
  if (option == "CAUTION") {
    ...
    if (value == "ON")
      ...
      theChan->removeFlag(sqlChannel::F_CAUTION);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "NOREG") {
    ...
    if (value == "ON")
      theChan->setFlag(sqlChannel::F_NOREG);
    else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_NOREG);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "SPECIAL") {
    ...
    if (value == "ON")
      theChan->setFlag(sqlChannel::F_SPECIAL);
    else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_SPECIAL);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "NEVERREG") {
    ...
    if (value == "ON")
      theChan->setFlag(sqlChannel::F_NEVREG);
    else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_NEVREG);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "NOPURGE") {
    ...
    if (value == "ON")
      theChan->setFlag(sqlChannel::F_NOPURGE);
    else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_NOPURGE);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "SUSPEND") {
    ...
    if (theChan->getFlag(sqlChannel::F_SUSPEND)) {
      ...
      bot->logAdminMessage("%s (%s) has suspended %s", theClient->getNickName().c_str(),
                           theUser->getUserName().c_str(), theChan->getName().c_str());
      if (tmpChan)
        bot->deopAllOnChan(tmpChan); // Deop everyone. :)
    } else {
      bot->writeChannelLog(theChan, theClient, sqlChannel::EV_UNSUSPEND, logmsg);
      /* inform admin channel */
      bot->logAdminMessage("%s (%s) has unsuspended %s", theClient->getNickName().c_str(),
                           theUser->getUserName().c_str(), theChan->getName().c_str());
    }
    bot->Notice(
        theClient,
  ...
  if (option == "TEMPMAN") {
    ...
    if (value == "ON")
      theChan->setFlag(sqlChannel::F_TEMP);
    else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_TEMP);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "VACATION") {
    ...
    if (value == "ON")
      theChan->setFlag(sqlChannel::F_VACATION);
    else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_VACATION);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "NOFORCE") {
    ...
    if (value == "ON")
      ...
      theChan->removeFlag(sqlChannel::F_NOFORCE);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
    ...
    if (value == "ON") {
      for (sqlChannel::forceMapType::const_iterator ptr = theChan->forceMap.begin();
        ...
        if (ForceLevel < level::immune::noforce) {
          bot->noticeAllAuthedClients(
              AdminUser,
              bot->getResponse(AdminUser, language::set_cmd_status, string("%s for %s is %s"))
                  .c_str(),
              option.c_str(), theChan->getName().c_str(), "ON");

          theChan->forceMap.erase(AdminUser->getID());

          bot->noticeAllAuthedClients(
              AdminUser,
              bot->getResponse(AdminUser, language::rem_temp_access,
                               string("Removed your temporary access of %i from channel %s"))
                  .c_str(),
  ...
  if (option == "OPLOG") {
    ...
    if (value == "ON")
      theChan->setFlag(sqlChannel::F_OPLOG);
    else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_OPLOG);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "NOOP") {
    ...
    if (value == "ON") {
      ...
      theChan->removeFlag(sqlChannel::F_NOOP);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "NOVOICE") {
    ...
    if (value == "ON") {
      ...
      if (tmpChan)
        bot->deVoiceAllOnChan(tmpChan); // DeVoice everyone. :)
    } else if (value == "OFF")
      theChan->removeFlag(sqlChannel::F_NOVOICE);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "STRICTOP") {
    ...
    if (value == "ON") {
      ...
      theChan->removeFlag(sqlChannel::F_STRICTOP);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "NOTAKE") {
    ...
    if (value == "ON") {
      ...
      theChan->removeFlag(sqlChannel::F_NOTAKE);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "TAKEREVENGE") {
    ...
    if (value != "") {
      ...
      if ((setting < 1) || (setting > 3)) {
        bot->Notice(theClient,
                    "Invalid TAKEREVENGE setting. Correct values are NONE, BAN or SUSPEND.");
        return false;
      }
    ...
    bot->Notice(theClient, "TAKEREVENGE for %s is %s", theChan->getName().c_str(), value.c_str());
    return true;
  }
#endif
#ifdef USE_FLOODPRO
  if (option == "FLOODPRO") {
    ...
    if ((value == "ON") || (value == "KICK")) {
      ...
      theChan->setFloodproLevel(sqlChannel::FLOODPRO_KICK);
      theChan->setManualFloodproLevel(sqlChannel::FLOODPRO_KICK);
      bot->Notice(theClient, "Default FLOODPRO values have been set for channel %s",
                  theChan->getName().c_str());
    } else {
    ...
    theChan->commit();
    bot->Notice(theClient, "%s punishment level on %s is %s", option.c_str(),
                theChan->getName().c_str(),
                sqlChannel::getFloodLevelName(theChan->getFloodproLevel()).c_str());
  ...
  if ((option == "MSGFLOOD") || (option == "MESSAGEFLOOD") || (option == "PRIVMSGFLOOD")) {
    ...
    theChan->commit();
    bot->Notice(theClient, "MESSAGEFLOOD for %s is set to %d", theChan->getName().c_str(),
                numValue);
    return true;
  }
  if (option == "NOTICEFLOOD") {
    ...
    theChan->commit();
    bot->Notice(theClient, "NOTICEFLOOD for %s is set to %d", theChan->getName().c_str(), numValue);
    return true;
  }
  if (option == "CTCPFLOOD") {
    if (level < level::set::floodpro) {
    ...
    theChan->commit();
    bot->Notice(theClient, "CTCPFLOOD for %s is set to %d", theChan->getName().c_str(), numValue);
    return true;
  }
  if ((option == "FLOODPERIOD") || (option == "FLOODPRD")) {
    ...
    theChan->commit();
    bot->Notice(theClient, "FLOODPERIOD for %s is set to %d", theChan->getName().c_str(), numValue);
    if (numValue == 0) {
      theChan->removeFlag(sqlChannel::F_FLOODPRO);
      theChan->setFloodproLevel(sqlChannel::FLOODPRO_NONE);
      theChan->commit();
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::set_cmd_status, string("%s for %s is %s")).c_str(),
          "FLOODPRO", theChan->getName().c_str(),
  ...
  if ((option == "REPEATFLOOD") || (option == "REPFLOOD")) {
    ...
    theChan->commit();
    bot->Notice(theClient, "REPEATFLOOD for %s is %s", theChan->getName().c_str(), value.c_str());
    return true;
  }
#endif
  if (option == "AUTOTOPIC") {
    ...
    if (value == "ON") {
      ...
      theChan->removeFlag(sqlChannel::F_AUTOTOPIC);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  if (option == "USERFLAGS") {
    ...
    if (value != "") {
      ...
      if ((setting < 0) || (setting > 2)) {
        bot->Notice(
            theClient,
            bot->getResponse(
                theUser, language::userflags_syntax,
    ...
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::userflags_status, string("USERFLAGS for %s is %s"))
            .c_str(),
        theChan->getName().c_str(), value.c_str());
    return true;
  }
  ...
  if (option == "MASSDEOPPRO") {
    ...
    if (!IsNumeric(value)) {
      bot->Notice(theClient, bot->getResponse(theUser, language::massdeoppro_syntax,
    ...
    theChan->commit();
    bot->Notice(theClient,
                bot->getResponse(theUser, language::massdeoppro_status,
                                 string("MASSDEOPPRO for %s is set to %d"))
                    .c_str(),
  ...
  if (option == "DESCRIPTION" || option == "DESC") {
    ...
    if (strlen(desc.c_str()) > 380) {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::desc_max_len,
                                   string("The DESCRIPTION can be a maximum of %i chars!"))
                      .c_str(),
                  380);
      return true;
    }
    ...
    if (desc == "") {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::desc_cleared,
                                   string("DESCRIPTION for %s is cleared."))
                      .c_str(),
                  theChan->getName().c_str());
    } else {
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::desc_status, string("DESCRIPTION for %s is: %s"))
              .c_str(),
          theChan->getName().c_str(), desc.c_str());
    }
  ...
  if (option == "URL") {
    ...
    {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::url_max_len,
                                   string("The URL can be a maximum of %i chars!"))
                      .c_str(),
                  128);
      return true;
    }
    ...
    if (url == "") {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::url_cleared, string("URL for %s is cleared."))
                      .c_str(),
                  theChan->getName().c_str());
    } else {
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::url_status, string("URL for %s is: %s")).c_str(),
          theChan->getName().c_str(), url.c_str());
    }
  ...
  if (option == "KEYWORDS") {
    ...
    {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::keywords_max_len,
                                   string("The string of keywords cannot exceed %i chars!"))
                      .c_str(),
                  300);
      return true;
    }
    ...
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::keywords_status, string("KEYWORDS for %s are: %s"))
            .c_str(),
  ...
  if (option == "WELCOME") {
    ...
    if (strlen(welcome.c_str()) > 300) {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::welcome_max_len,
    ...
    if ((string_upper(welcome) == "OFF") || (welcome == "")) {
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::welcome_cleared, string("WELCOME for %s is cleared."))
              .c_str(),
          theChan->getName().c_str());
      theChan->setWelcome("");
      theChan->commit();
      ...
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::welcome_status, string("WELCOME for %s is: %s"))
              .c_str(),
  ...
  if (option == "MODE") {
    ...
    if (!tmpChan) {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::no_such_chan,
                                   string("Can't locate channel %s on the network!"))
                      .c_str(),
                  st[1].c_str());
      return false;
    }
    ...
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::set_cmd_status, string("%s for %s is %s")).c_str(),
        option.c_str(), theChan->getName().c_str(), theChan->getChannelMode().c_str());
    return true;
  }
  ...
  if (option == "COMMENT") {
    ...
    if (comment.empty()) {
      bot->Notice(theClient, "COMMENT for %s is cleared.", theChan->getName().c_str());
    } else {
      bot->Notice(theClient, "COMMENT for %s is: %s", theChan->getName().c_str(), comment.c_str());
    }

    return true;
  }

  // ********* JOIN LIMITATION - prevents unidented and unauthed users from sneaking into the
  // channel in preparation for a flood **********
  if (option == "JOINLIM") {
  ...
  if (option == "JOINMAX") {
    ...
    if (ss >> limit_joins >> colon >> limit_secs && colon == ':') {
      if (limit_joins < (int)bot->getConflimitJoinMaxLowest() ||
          limit_joins > (int)bot->getConflimitJoinMaxHighest()) {
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::set_joinmax_joinrange,
                                     string("Value of joins has to be %i-%i"))
                        .c_str(),
                    (int)bot->getConflimitJoinMaxLowest(), (int)bot->getConflimitJoinMaxHighest());
        return true;
      }

      if (limit_secs < (int)bot->getConflimitJoinSecsLowest() ||
          limit_secs > (int)bot->getConflimitJoinSecsHighest()) {
        bot->Notice(theClient,
                    bot->getResponse(theUser, language::set_joinmax_secrange,
                                     string("Value of seconds has to be %i-%i"))
                        .c_str(),
                    (int)bot->getConflimitJoinSecsLowest(),
                    (int)bot->getConflimitJoinSecsHighest());
        return true;
      }
      ...
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_joinmax,
                                   string("JOINMAX for %s is now %i:%i (JOINS:SECS)"))
                      .c_str(),
      ...
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::syntax, string("SYNTAX: %s")).c_str(),
  ...
  if (option == "JOINMODE") {
      ...
      if (allowedModes.find(c) != std::string::npos) {
        ...
        if (c == 'b') {
          ...
          if (!validUserMask(banMask)) {
            bot->Notice(theClient,
                        bot->getResponse(theUser, language::ban_invalid,
                                         string("The banmask '%s' is not valid"))
                            .c_str(),
                        st[numParam + 3].c_str());
            return true;
          }
    ...
    if (invalidMode || resultModes.empty()) {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_joinmode_allowed,
                                   string("The allowed JOINMODE modes are: %s"))
                      .c_str(),
    ...
    if (invalidParam || numParam != (st.size() - 4)) {
      bot->Notice(
          theClient,
          bot->getResponse(
                 theUser, language::set_joinmode_params,
                 string(
                     "Invalid number of JOINMODE parameters: %i modes and %i parameters provided"))
              .c_str(),
          numParam, st.size() - 4);
      return true;
    }
    ...
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::set_joinmode, string("JOINMODE for %s is now: %s"))
            .c_str(),
  ...
  if (option == "JOINPERIOD") {
    ...
    if ((limit_period < 1) || (limit_period > bot->getConflimitJoinPeriodHighest())) {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_joinperiod_range,
                                   string("Value of JOINPERIOD has to be 1-%i"))
                      .c_str(),
    ...
    bot->Notice(theClient,
                bot->getResponse(theUser, language::set_joinperiod,
                                 string("JOINPERIOD for %s is now: %i seconds"))
                    .c_str(),
  ...
  if (option == "FLOATLIM") {
    ...
    if (value == "ON") {
      ...
      theChan->removeFlag(sqlChannel::F_FLOATLIM);
    else {
      bot->Notice(theClient,
                  bot->getResponse(theUser, language::set_cmd_syntax_on_off,
                                   string("value of %s must be ON or OFF"))
  ...
  bot->Notice(theClient, bot->getResponse(theUser, language::mode_invalid,
