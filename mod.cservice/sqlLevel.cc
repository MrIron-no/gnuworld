...
bool sqlLevel::loadData(unsigned int userID, unsigned int channelID) {
    ...
    LOG(TRACE, "Attempting to load level data for channel-id: {} and user-id: {}", channelID,
        userID);

    stringstream queryString;
    queryString << "SELECT " << sql::level_fields << " FROM levels WHERE channel_id = " << channelID
                << " AND user_id = " << userID << ends;
...
bool sqlLevel::insertRecord() {
    static const char* queryHeader = "INSERT INTO levels "
                                     "(channel_id,user_id,access,flags,added,added_by,last_modif,"
                                     "last_modif_by,last_updated) VALUES (";

    stringstream queryString;
    queryString << queryHeader << channel_id << ", " << user_id << ", " << access << ", " << flags
                << ", " << added << ", '" << escapeSQLChars(added_by) << "', " << last_modif
