...
bool OPCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (theChan->getFlag(sqlChannel::F_NOOP)) {
    bot->Notice(
        theClient,
        bot->getResponse(theUser, language::noop_set, string("The NOOP flag is set on %s")).c_str(),
        theChan->getName().c_str());
    return false;
  }
  ...
  for (; counter < st2.size(); ++counter) {
    ...
    if (theChan->getFlag(sqlChannel::F_STRICTOP)) {
      ...
      if (!authUser) {
        bot->Notice(
            theClient,
            bot->getResponse(theUser, language::strictop_not_authed,
        ...
        bot->Notice(
            ...
            bot->getResponse(
                   ...
                   string("The STRICTOP flag is set on %s (and %s has insufficient access)"))
                .c_str(),
    ...
    if (tmpBan && (tmpBan->getLevel() <= 75)) {
      ...
      bot->Notice(theClient, "%s isn't allowed to be opped on %s",
                  tmpChanUser->getClient()->getNickName().c_str(), theChan->getName().c_str());
      continue;
    }
