...
bool SUPPORTCommand::Exec(iClient* theClient, const string& Message) {
  ...
  theQuery << "SELECT channels.id FROM pending,channels"
           << " WHERE lower(channels.name) = '" << escapeSQLChars(string_lower(channelName)) << "'"
  ...
  if (bot->SQLDb->Tuples() <= 0) {
    bot->Notice(theClient,
                "The channel %s doesn't appear to have a pending application, please ensure you "
  ...
  supQuery << "SELECT support FROM supporters"
           << " WHERE channel_id = " << channel_id << " AND user_id = " << theUser->getID() << ends;
  ...
  if (bot->SQLDb->Tuples() <= 0) {
    bot->Notice(theClient,
                "You don't appear to be listed as a supporter in %s, please ensure you have "
                "spelled the channel name correctly.",
                channelName.c_str());
    return false;
  }
  ...
  updateQuery << "UPDATE supporters SET support = '" << supportChar << "'"
              << ", last_updated = date_part('epoch', CURRENT_TIMESTAMP)::int"
              << " WHERE channel_id = " << channel_id << " AND user_id = " << theUser->getID()
              << ends;
  ...
  bot->Notice(theClient, "Done. Set your support for %s to %s.", channelName.c_str(),
              support.c_str());

  LOG(INFO, "{} has set their support for {} to {}.", theUser->getUserName(), channelName,
      supportChar);
  ...
  if (supportChar == 'Y') {
    ...
    if (supporterCount < bot->getConfRequiredSupporters())
      return false;
    ...
    if (allSupporting) {
      ...
      updatePendingQuery << "UPDATE pending SET status = '1',"
                         ...
                         << " WHERE channel_id = " << channel_id << " AND status = '0'" << ends;

      bot->SQLDb->Exec(updatePendingQuery.str().c_str());
      LOG(INFO, "{} has just made it to traffic check phase with {} supporters.", channelName,
          supporterCount);
    }
  ...
  if (supportChar == 'N') {
    ...
    mgrQuery << "SELECT user_name,email FROM users,pending"
             ...
             << " AND pending.channel_id = " << channel_id << " AND pending.status = '9'" << ends;
    ...
    if (bot->SQLDb->Tuples() > 0) {
      ...
      static const char* cmdHeader =
          "INSERT INTO noreg "
          "(user_name,email,channel_name,type,expire_time,created_ts,set_by,reason) VALUES ";

      stringstream noregQuery;
      noregQuery << cmdHeader << "('', '','" << escapeSQLChars(channelName)
                 << "',"
                 // << "1, (date_part('epoch', CURRENT_TIMESTAMP)::int + (86400*3)),
                 // date_part('epoch', CURRENT_TIMESTAMP)::int, '* REGPROC', '-NON SUPPORT-'"
                 << "1, (date_part('epoch', CURRENT_TIMESTAMP)::int + (" << noregTime
                 << ")), date_part('epoch', CURRENT_TIMESTAMP)::int, '* REGPROC', '-NON SUPPORT-'"
                 << ")" << ends;
      ...
      usernoregQuery << cmdHeader << "('" << escapeSQLChars(managerName) << "', '"
      ...
      clogQuery << "INSERT INTO channellog (ts, channelid, event, message, last_updated) VALUES ("
                << "date_part('epoch', CURRENT_TIMESTAMP)::int, " << channel_id << ", "
                << "1, 'Non-support by " << escapeSQLChars(theUser->getUserName()) << "', "
