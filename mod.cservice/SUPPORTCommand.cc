...
bool SUPPORTCommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (bot->SQLDb->Tuples() <= 0) {
        bot->Notice(theClient,
                    "The channel %s doesn't appear to have a pending application, please ensure "
    ...
    if (supportChar == 'N') {
        ...
        mgrQuery << "SELECT user_name,email FROM users,pending"
                 ...
                 << " AND pending.channel_id = " << channel_id << " AND pending.status = '9'"
                 << ends;
        ...
        if (bot->SQLDb->Tuples() > 0) {
            ...
            noregQuery
                << cmdHeader << "('', '','" << escapeSQLChars(channelName)
                << "',"
            ...
            usernoregQuery
                << cmdHeader << "('" << escapeSQLChars(managerName) << "', '"
            ...
            clogQuery
                << "INSERT INTO channellog (ts, channelid, event, message, last_updated) VALUES ("
