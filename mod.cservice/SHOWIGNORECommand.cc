...
bool SHOWIGNORECommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (bot->silenceList.empty()) {
    bot->Notice(theClient, bot->getResponse(theUser, language::ignore_list_empty,
                                            string("Ignore list is empty")));
    return true;
  }

  bot->Notice(theClient,
              bot->getResponse(theUser, language::ignore_list_start, string("Ignore list:")));

  // TODO: Same as above
  for (cservice::silenceListType::const_iterator ptr = bot->silenceList.begin();
       ptr != bot->silenceList.end(); ++ptr) {
    bot->Notice(theClient,
                bot->getResponse(theUser, language::rpl_ignorelist, "%s for %i minutes").c_str(),
                ptr->first.c_str(), (((ptr->second.first - bot->currentTime()) / 60) % 60));
  }

  bot->Notice(theClient, bot->getResponse(theUser, language::ignore_list_end,
