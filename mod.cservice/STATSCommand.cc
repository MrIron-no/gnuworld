...
bool STATSCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (bot->getAdminAccessLevel(theUser) < level::stats)
    return false;
  ...
  if (st.size() >= 2) {
    ...
    if (st[1][0] == '#') {
      ...
      if (!tmpChan) {
        bot->Notice(theClient, "That channel doesn't exist on the network :(");
        return true;
      }

      cservice::pendingChannelListType::iterator ptr =
          bot->pendingChannelList.find(tmpChan->getName());
      ...
      if (ptr != bot->pendingChannelList.end()) {
        ...
        bot->Notice(theClient, "# of entries in supporter list: %i",
                    tmpPending->supporterList.size());
        ...
        bot->Notice(theClient, "# of entries in IP traffic checker list: %i",
                    tmpPending->trafficList.size());
        ...
        for (; tPtr != tmpPending->trafficList.end(); ++tPtr) {
          bot->Notice(theClient, " - IP #: %s (Channel ID: %i) (Join Count: %i)",
                      tPtr->second->ip_number.c_str(), tPtr->second->channel_id,
                      tPtr->second->join_count);
        }
      } else {
      ...
      if (theServer) {
        ...
        bot->Notice(theClient, " - Numeric: %s", theServer->getCharYY().c_str());
