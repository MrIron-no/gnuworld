...
bool REMIGNORECommand::Exec(iClient* theClient, const string& Message) {
  ...
  for (cservice::silenceListType::iterator ptr = bot->silenceList.begin();
    ...
    if (string_lower(st[1]) == string_lower(ptr->first.c_str())) {
      ...
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::unsilenced, string("Removed %s from my ignore list"))
              .c_str(),
          st[1].c_str());
      bot->logAdminMessage("%s (%s) has removed ignore: %s", theClient->getNickName().c_str(),
                           theUser->getUserName().c_str(), st[1].c_str());
      return true;
    }
  ...
  bot->Notice(theClient,
              bot->getResponse(theUser, language::couldnt_find_silence,
                               string("Couldn't find %s in my silence list"))
                  .c_str(),
              st[1].c_str());
