...
bool NEWPASSCommand::Exec(iClient* theClient, const string& Message) {
  ...
  if ((string_lower(st[1]) == string_lower(tmpUser->getUserName())) ||
    ...
    bot->Notice(theClient,
                bot->getResponse(tmpUser, language::pass_cant_be_nick,
                                 string("Your passphrase cannot be your username or current nick - "
                                        "syntax is: NEWPASS <new passphrase>")));
    return false;
  }
  ...
  std::string err;
  //	elog << "Generating SCRAM record for user " << theUser->getUserName() << " and password: ["
  //<< st.assemble( 0, pass_end ) << "]\n" ;
  ...
  if (tmpUser->commit(theClient)) {
    bot->Notice(theClient, bot->getResponse(tmpUser, language::pass_changed,
