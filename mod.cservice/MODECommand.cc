...
bool MODECommand::Exec(iClient* theClient, const string& Message) {
      ...
      if (plus) {
        ...
        if (tmpChan->getKey() != st[argPos]) {
          bot->Notice(
              theClient,
              bot->getResponse(
                     theUser, language::mode_wrongkey,
                     string("You need to provide the existing key in order to unset the key."))
                  .c_str());
          return false;
        }
  ...
  if (modeString.size() > 1 || limitPos > 0) {
    ...
    if (!banString.empty())
      bot->Notice(
          theClient,
          bot->getResponse(theUser, language::mode_banset, string("Ban-modes set for: %s")).c_str(),
          banString.c_str());
    ...
    if (wrongMode)
      bot->Notice(theClient, bot->getResponse(theUser, language::mode_opvoice,
