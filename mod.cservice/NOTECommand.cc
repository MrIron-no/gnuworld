...
bool NOTECommand::Exec(iClient* theClient, const string& Message) {
    ...
    if (string_lower(st[1]) == "send") {
        ...
        if (targetUser->getFlag(sqlUser::F_NONOTES)) {
            bot->Notice(theClient, "%s doesn't accept Notes.", targetUser->getUserName().c_str());
            return false;
        }
        ...
        if ((unsigned int)(bot->currentTime() - theUser->getLastNote()) >=
            bot->getConfnoteDuration()) {
        ...
        if (((unsigned int)(bot->currentTime() - theUser->getLastNote()) <=
             bot->getConfnoteDuration()) &&
            (theUser->getNotesSent() >= bot->getConfnoteLimit())) {
            bot->Notice(theClient, "You have exceeded the maximum number of notes you can send at "
        ...
        static const char* queryHeader =
            "INSERT INTO notes (user_id,from_user_id,message,last_updated) VALUES (";

        stringstream queryString;
        queryString << queryHeader << targetUser->getID() << ", " << theUser->getID() << ", '"
        ...
        bot->noticeAllAuthedClients(
            targetUser, "%s has just sent you a note. Type /msg %s notes read all to read it.",
    ...
    if (string_lower(st[1]) == "read") {
        if (string_lower(st[2]) == "all") {
            ...
            allNotesQuery
                << "SELECT users.user_name, notes.message, notes.last_updated, message_id "
                << "FROM notes,users "
                << "WHERE notes.from_user_id = users.id "
                << "AND notes.user_id = " << theUser->getID() << " ORDER BY notes.last_updated ASC"
                << ends;
            ...
            for (unsigned int i = 0; i < noteCount; i++) {
                ...
                bot->Notice(
                    theClient, "\002NOTE\002 (Message-Id: %i): Recieved from %s, %s ago : %s",
                    message_id, from.c_str(), prettyDuration(when).c_str(), theMessage.c_str());
            }
        }

        bot->Notice(theClient,
                    "To erase an individual note, type /msg %s notes erase <message-id>. To erase "
                    "all your notes, type /msg %s notes erase all",
    ...
    if (string_lower(st[1]) == "erase") {
        if (string_lower(st[2]) == "all") {
            stringstream queryString;
            queryString << "DELETE FROM notes where user_id = " << theUser->getID() << ends;
        ...
        stringstream queryString;
        queryString << "DELETE FROM notes where user_id = " << theUser->getID()
                    << " AND message_id = " << messageId << ends;
