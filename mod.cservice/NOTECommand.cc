...
bool NOTECommand::Exec(iClient* theClient, const string& Message) {
  ...
  if (string_lower(st[1]) == "send") {
    ...
    if (((unsigned int)(bot->currentTime() - theUser->getLastNote()) <=
      ...
      bot->Notice(theClient, "You have exceeded the maximum number of notes you can send at this "
  ...
  if (string_lower(st[1]) == "read") {
    if (string_lower(st[2]) == "all") {
      ...
      allNotesQuery << "SELECT users.user_name, notes.message, notes.last_updated, message_id "
                    ...
                    << "AND notes.user_id = " << theUser->getID()
                    << " ORDER BY notes.last_updated ASC" << ends;
    ...
    bot->Notice(theClient,
                "To erase an individual note, type /msg %s notes erase <message-id>. To erase all "
                "your notes, type /msg %s notes erase all",
