 ...
 */
template <typename T = void> static std::optional<T> fail(std::string_view msg, std::string* err) {
    if (err)
        *err = std::string(msg);
    return std::nullopt;
}
 ...
 */
std::optional<std::vector<unsigned char>> b64decode(std::string_view b64, std::string* err,
                                                    bool validatePrintable) {
    if (b64.size() % 4 != 0)
        return fail<std::vector<unsigned char>>("bad base64 length", err);
    ...
    int n = EVP_DecodeBlock(out.data(), reinterpret_cast<const unsigned char*>(b64.data()),
                            (int)b64.size());
    if (n < 0)
        return fail<std::vector<unsigned char>>("bad base64", err);
    ...
    if (real_len > out.size())
        return fail<std::vector<unsigned char>>("base64 overflow", err);
    out.resize(real_len);
    ...
    if (validatePrintable) {
        for (size_t i = 0; i < real_len; ++i) {
            if (out[i] != 0 && (out[i] < 32 || out[i] > 126)) {
                return fail<std::vector<unsigned char>>(
                    "decoded data contains non-printable characters", err);
            }
        }
    }
...
pbkdf2_sha256(std::string_view password, const unsigned char* salt, size_t salt_len, int iterations,
              std::string* err) {
    std::array<unsigned char, 32> out{};
    if (!PKCS5_PBKDF2_HMAC(password.data(), (int)password.size(), salt, (int)salt_len, iterations,
 ...
 * SHA256 hash.
 */
static inline std::optional<std::array<unsigned char, 32>> sha256(const unsigned char* data,
                                                                  size_t len, std::string* err) {
    std::array<unsigned char, 32> out{};
    unsigned int outlen = 0;
    EVP_MD_CTX* ctx = EVP_MD_CTX_new();
    if (!ctx)
        return fail<std::array<unsigned char, 32>>("EVP_MD_CTX_new failed", err);
 ...
 */
static inline std::optional<std::array<unsigned char, 32>>
hmac_sha256(const unsigned char* key, size_t key_len, const unsigned char* msg, size_t msg_len,
            std::string* err) {
    std::array<unsigned char, 32> out{};
    unsigned int outlen = 0;
 ...
 */
std::optional<ScramParsed> parse_scram_sha256_record(const std::string& rec, std::string* err) {
    ...
    if (pos1 == std::string_view::npos)
        return fail<ScramParsed>("bad record (no ':')", err);
    int iterations = 0;
    {
    ...
    if (pos3 == std::string_view::npos)
        return fail<ScramParsed>("bad record (no second ':')", err);
    ...
    if (storedOpt->size() != 32)
        return fail<ScramParsed>("storedKey wrong size", err);
    if (serverOpt->size() != 32)
        return fail<ScramParsed>("serverKey wrong size", err);

    ScramParsed out{iterations, std::move(*saltOpt), std::move(*storedOpt), std::move(*serverOpt)};
    return out;
}
 ...
 */
std::optional<std::string> make_scram_sha256_record(std::string_view password, std::string* err) {
 ...
 */
bool validate_scram_sha256_proof(const std::vector<unsigned char>& storedKey,
    ...
    HMAC(EVP_sha256(), storedKey.data(), storedKey.size(),
 ...
 */
std::string compute_server_signature(const std::vector<unsigned char>& serverKey,
                                     const std::string& authMessage) {
    unsigned int siglen = 0;
    unsigned char serverSignature[SHA256_DIGEST_LENGTH];
    HMAC(EVP_sha256(), serverKey.data(), serverKey.size(),
         reinterpret_cast<const unsigned char*>(authMessage.data()), authMessage.size(),
