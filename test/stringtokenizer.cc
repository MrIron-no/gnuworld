...
int main(int argc, char** argv) {
  ...
  if (maxLines < 1) {
    cout << "Error: Please specify a positive number "
         << "for maximum lines to read from file" << endl;
    return 0;
  }
  ...
  cout << endl
       << "Reading " << maxLines << " lines of text at a time from " << inputFileName << endl;
  ...
  while (getData(inputFile, lines)) {
    for (vector<string>::const_iterator ptr = lines.begin(), endPtr = lines.end(); ptr != endPtr;
         ++ptr) {
      begin = clock();
      ...
      if (verbose) {
        cout << "[DATA] " << *ptr << endl;
        cout << "[TOKENIZED] (" << st.size() << ") " << st << endl;
      }
    }
  ...
  cout << "Tokenized " << totalLines << " lines in " << (totalClocks1 / CLOCKS_PER_SEC)
       << " seconds" << endl;
  cout << endl;
...
void usage(const string& progName) {
  ...
  cout << "Options:" << endl;
  cout << "-f <filename> (required)" << endl;
  cout << "-h Prints this menu" << endl;
  cout << "-n <number of lines> Reads <number of lines> lines of "
       << "text from the given input " << endl
       << "                     file at a time. (" << maxLines << ")" << endl;
  cout << "-v (false) Run in verbose, be careful of this :)" << endl;
}
