...
int main(int argc, const char** argv) {
  ...
  while (keepRunning) {
    ...
    case 5:
      ...
      break;
    default:
      cout << "Unrecognized input: " << choice << endl;
      break;
    } // switch()
  } // while( keepRunning)
...
void printConfig(const EConfig& conf) {
  cout << "-------- Current Configuration --------" << endl << endl;

  for (EConfig::const_iterator itr = conf.begin(); itr != conf.end(); ++itr) {
...
void printMenu() {
  cout << endl << "========== Menu ==========" << endl;
  cout << "0: Exit" << endl;
  cout << "1: Find an entry" << endl;
  cout << "2: Delete an entry" << endl;
  cout << "3: Add an entry" << endl;
  cout << "4: Replace an entry" << endl;
...
void findEntry(const EConfig& conf) {
  ...
  for (EConfig::const_iterator itr = conf.Find(findMe);
       (itr != conf.end()) && (itr->first == findMe); ++itr) {
    foundIt = true;

    cout << "Key: " << itr->first << ", Value: " << itr->second << endl;
  }
  if (!foundIt) {
    cout << "Unable to find key \"" << findMe << "\" in the config file" << endl;
  }
}
...
void deleteEntry(EConfig& conf) {
  ...
  for (EConfig::const_iterator itr = conf.begin(); itr != conf.end(); ++itr) {
    cout << counter << ": " << itr->first << '/' << itr->second << endl;
    ++counter;
  }
...
void addEntry(EConfig& conf) {
  ...
  if (!conf.Add(st[0], st.assemble(1))) {
    cout << "Unable to add: " << st[0] << '/' << st.assemble(1) << endl;
  } else {
    cout << "Added " << st[0] << '/' << st.assemble(1) << endl;
  }
}
...
void replaceEntry(EConfig& conf) {
  ...
  if (newValue.empty()) {
    cout << "New value must be non-empty" << endl;
    return;
  }
  ...
  if (conf.Replace(itr, newValue)) {
    cout << "Successfully replaced item" << endl;
  } else {
...
void addComment(EConfig& conf) {
  ...
  if (!conf.AddComment(line)) {
    cout << "Unable to add comment line: " << line << endl;
  } else {
    cout << "Successfully added comment line: " << line << endl;
  }
}
