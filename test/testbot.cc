...
void usage(const string& progName) { cout << "Usage: " << progName << " <conf file>" << endl; }
...
testBot::testBot(const string& fileName) : keepRunning(true), uplinkPort(0), myConnect(0) {
  EConfig conf(fileName);
  setUplinkName(conf.Require("uplinkname")->second);
  setUplinkPort(static_cast<unsigned short int>(atoi(conf.Require("uplinkport")->second.c_str())));
  ...
  for (EConfig::const_iterator chanItr = conf.Find("channel");
       (chanItr != conf.end()) && (chanItr->first == "channel"); ++chanItr) {
...
void testBot::Run() {
  ...
  cout << "Run> Out of main loop, shutting down" << endl;
}

void testBot::OnDisconnect(Connection*) { keepRunning = false; }

void testBot::OnConnectFail(Connection*) { keepRunning = false; }

void testBot::OnTimeout(Connection*) { keepRunning = false; }

void testBot::OnConnect(Connection*) { cout << "Connected" << endl; }
...
void testBot::processInput(const string& line) {
  ...
  if (st.empty()) {
    cout << "processInput> Empty tokenizer" << endl;
    return;
  }
...
void testBot::joinChannels() {
  for (std::list<string>::const_iterator chanItr = channels.begin(); chanItr != channels.end();
       ++chanItr) {
    string writeMe("JOIN ");
    writeMe += *chanItr + "\r\n";
    myConnect->Write(writeMe);
