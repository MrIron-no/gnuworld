...
void usage(const string& progName) {
  cout << "Usage: " << progName << " <socket log file>" << endl;
}
...
int main(int argc, char** argv) {
  ...
  clog << endl << "Read " << numFound << " hosts" << endl << endl;

  cout << "Entering command line mode, use \"exit\" to end" << endl << endl;
  ...
  do {
    ...
    if (st[0] == "help") {
      ...
      cout << "Unrecognized command: " << st[0] << endl;
    }
...
void printHelp() {
  cout << endl
       ...
       << endl
       << "help\t\t\tThis menu" << endl
       << "exit/quit\t\tTerminate program" << endl
       << "search <mask>\t\tSearch for a given mask" << endl
       << "erase <mask>\t\tErase all values matching the given mask" << endl
...
void handleSearch(const StringTokenizer& st) {
  ...
  if (st.size() != 2) {
    cout << "Error: Search requires exactly 1 argument" << endl;
    return;
  }
  ...
  for (valuesListType::const_iterator vItr = values.begin(); vItr != values.end(); ++vItr) {
    cout << "Match: " << vItr->second << endl;
  }
  cout << "Search found " << values.size() << " match" << ((values.size() != 1) ? "es" : "")
       << endl;
}

void handleErase(const StringTokenizer& st) {
  if (st.size() != 2) {
    cout << "Error: Erase requires exactly 1 argument" << endl;
    return;
  }

  MTrie<string>::size_type eraseCount = hostTrie.erase(st[1]);
  clog << "Erased " << eraseCount << " element(s)" << endl;
}

void handleLevelDebug(const StringTokenizer& st) {
  if (st.size() != 2) {
    cout << "Error: leveldebug requires exactly 1 argument" << endl;
    return;
  }
...
void handleMinLength(const StringTokenizer& st) {
  if (st.size() != 2) {
    cout << "Error: maxlength requires exactly 1 argument" << endl;
    return;
  }
  ...
  if (!maxLengthStrings.empty()) {
    clog << "Found " << maxLengthStrings.size() << " hostnames with at least " << minLength
         << " tokens." << endl
         << endl;
  }

  for (list<string>::const_iterator itr = maxLengthStrings.begin(); itr != maxLengthStrings.end();
       ++itr) {
    StringTokenizer tokens(*itr, '.');
    clog << "Length: " << tokens.size() << ", key: " << *itr << endl;
  }
}
