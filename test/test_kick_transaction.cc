...
struct kick {
  ...
  kick(const string& destNumeric, const string& srcNumeric, const string& chan,
       const size_t& lineNumber)
      : destNumeric(destNumeric), srcNumeric(srcNumeric), chan(chan), lineNumber(lineNumber) {}
  ...
  friend ostream& operator<<(ostream& out, const kick& rhs) {
    out << rhs.lineNumber << ":" << rhs.srcNumeric << " K " << rhs.destNumeric << " " << rhs.chan;
    return out;
  }
};
...
int main(int argc, char** argv) {
  ...
  while (std::getline(inFile, line)) {
    ...
    if ((lineNumber > 0) && (0 == (lineNumber % 100000))) {
      cout << "Line " << lineNumber << ", kickMap.size(): " << kickMap.size() << endl;
...
void checkCorrespondingKick(const StringTokenizer& st) {
  ...
  for (; (kItr != kickMap.end()) && (kItr->first == st[0]); ++kItr) {
    ...
    // Else, possible match
    cout << "Possible match: " << kItr->second << " with " << lineNumber << ":" << st[0] << " L "
         << st[2] << endl;
  } // for()
}
...
void pruneKickMap(size_t age) {
  for (kickMapType::iterator kItr = kickMap.begin(); kItr != kickMap.end();) {
