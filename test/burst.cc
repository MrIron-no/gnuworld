...
public:
  ...
  localHandler() : listenPtr(0), burstPtr(0) {}
  ...
  virtual void OnConnect(Connection* cPtr) {
    burstPtr = cPtr;
    cout << "OnConnect> Received new connection: " << *cPtr << endl;
  }
};

void usage(const string& progName) {
  cout << endl << "Usage: " << progName << " [options]" << endl;
  cout << endl
       << "Options: " << endl
       << "-b <KB per second>\tNumber of kilobytes per second to "
       << "transfer" << endl
       << "-f <socket filename>\tThe socket file name (required)" << endl
       << "-p <port>\t\tPort on which to listen" << endl
       << "-h\t\t\tPrint this help menu" << endl;
  cout << endl;
}
...
int main(int argc, char** argv) {
  ...
  if (0 == lh.listenPtr) {
    cout << "Unable to establish listener on port " << portNum << endl;
    return -1;
  }

  int inputFileFD = ::open(socketFileName.c_str(), O_RDONLY);
  if (inputFileFD < 0) {
    cout << "Unable to open input file: " << socketFileName << ": " << strerror(errno) << endl;
    cm.Disconnect(&lh, lh.listenPtr);
    return -1;
  }

  cout << endl
       << "Listening for incoming connections on port " << portNum
       << ", processing from burst file: " << socketFileName << "..." << endl;
  ...
  while (!done) {
    ...
    if (lh.burstPtr->getOutputBufferSize() >= (4 * attemptReadSize)) {
    ...
    size_t sendRate = lh.burstPtr->getBytesWritten() / connectDuration;

    cout << "Duration: " << connectDuration << " seconds; Read: " << lh.burstPtr->getBytesRead()
         << " bytes total, " << (recvRate >> 10)
         << " KB/sec; Write: " << lh.burstPtr->getBytesWritten() << " bytes total, "
         << (sendRate >> 10) << " KB/sec" << endl;

    // Connection exists, pass it some input
    int readBytes = ::read(inputFileFD, inputBuffer, attemptReadSize);
    if (readBytes <= 0) {
      cout << "Done reading input file " << socketFileName.c_str() << endl;
      done = true;
      continue;
    }
