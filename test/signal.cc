...
int main(int argc, char** argv) {
  if (argc != 2) {
    cout << "Usage: " << argv[0] << " <num signals>" << endl;
    return 0;
  }
  ...
  pid_t thePID = fork();
  if (0 == thePID) {
    clog << "Child PID: " << getpid() << endl;

    handleChild();

    clog << "Child exiting..." << endl;
...
void handleChild() {
  ...
  while (true) {
    ...
    while (Signal::getSignal(theSignal)) {
      if (theSignal < 0) {
        // Error
        clog << "handleChild> Error" << endl;
        continue;
      }

      ++totalSignalsCaught;
      clog << "Child> Got signal: " << theSignal << ", total signals caught: " << totalSignalsCaught
           << endl;
    }
  }

  clog << "Child> Caught " << totalSignalsCaught << " signals" << endl;
...
void handleParent(pid_t childPID, int numSignals) {
  ...
  for (; totalSignalsSent < static_cast<size_t>(numSignals);) {
    for (size_t i = 0; i < (signalCount - 1); ++i) {
      ++totalSignalsSent;
      clog << "Parent> Sending signal: " << sigArray[i].signalID << endl;
  ...
  */
  clog << "Parent> Sent " << totalSignalsSent << " signals" << endl;
...
void setupSignals() {
  ...
  for (size_t i = 0; sigArray[i].signalID != -31337; ++i, ++signalCount) {
  }
}

int randomSignal() { return (sigArray[rand() % signalCount].signalID); }
