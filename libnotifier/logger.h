 ...
 * Usage: LOG_MSG(INFO, "User {} joined {channel}", username).with("channel",
 * chanPtr).logStructured();
 */
...
private:
  ...
  std::vector<std::pair<std::shared_ptr<notifier>, unsigned short>> notifiers;
   ...
   */
  std::map<std::type_index, std::function<bool(std::map<std::string, std::string>&,
   ...
   */
  std::string getColourReset(Verbosity v) { return getColour(v).empty() ? "" : "\003"; }
...
public:
  ...
  private:
     ...
     */
    template <typename T> std::string formatArgumentToString(T&& arg) const {
  ...
  public:
    ...
    template <typename T> MessageTemplate& with(const std::string& key, T&& value) {
      auto handlerIt =
          logger_instance->customHandlers.find(std::type_index(typeid(std::decay_t<T>)));
   ...
   */
  inline void setConsoleSQL(bool enable) { consoleSQL = enable; }

  /**
   * Returns the currently configured debug channel name.
   */
  inline std::string getChannel() const { return debugChan; }
   ...
   */
  inline void updateNotifierVerbosity(std::shared_ptr<notifier> _notifier,
   ...
   */
  template <typename T>
  inline void registerObjectHandler(
      std::function<bool(std::map<std::string, std::string>&, const std::string&, T*)> handler) {
    // Register with the pointer type since that's what with() will look for
    customHandlers[std::type_index(typeid(T*))] =
        [handler](std::map<std::string, std::string>& fields, const std::string& key,
                  const void* obj) -> bool {
   ...
   */
  void write(Verbosity v, const std::string& theMessage) { writeFunc(v, "", string(), theMessage); }
   ...
   */
  template <typename Format, typename... Args>
  void writeFunc(Verbosity v, const char* func, const std::string& jsonParams, const Format& format,
                 Args&&... args) {
#ifdef HAVE_FORMAT
    std::string fmtString = std::vformat(format, std::make_format_args(args...));
