 ...
 */
void Logger::writeLog(Verbosity v, const std::string& func, const std::string& jsonParams,
 ...
 */
void Logger::writeFunc(Verbosity v, const char* func, const string& jsonParams,
  ...
  if (v == SQL) {
    ...
    if (consoleSQL && elog.getStream())
      *(elog.getStream()) << elog.getLocalTime() << "[" << bot->getNickName() << "] - SQL   - "
                          << theMessage << endl;
    return;
  }
  ...
  if (v <= consoleVerbosity && elog.getStream())
    *(elog.getStream()) << elog.getLocalTime() << "[" << bot->getNickName() << "] - "
                        << levels[v].prefix << " - " << fmtMessage << endl;
  ...
  if (v <= chanVerbosity && bot->isConnected() && !debugChan.empty()) {
    ...
    if (!theChan) {
      elog << "[" << bot->getNickName()
           << "] - WARN  - Logger::writeFunc> Unable to locate channel " << debugChan
           << " on network!" << endl;
    } else {
      ...
      while (std::getline(stream, line))
        if (!line.empty())
          bot->getUplink()->serverNotice(theChan, std::format("{}[{}] {} {}{}", getColour(v),
                                                              bot->getNickName(), levels[v].tag,
                                                              line, getColourReset(v)));
    }
  }
#endif
}
...
std::string Logger::MessageTemplate::buildJsonFromMap() const {
  ...
  for (const auto& [k, v] : fields) {
    ...
    bool isNumeric =
        !v.empty() && (std::isdigit(v[0]) || (v[0] == '-' && v.length() > 1 && std::isdigit(v[1])));
