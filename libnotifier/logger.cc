    ...
    if (v <= chanVerbosity && bot->isConnected() && !debugChan.empty()) {
        ...
        if (!theChan) {
            ...
            while (std::getline(stream, line))
                if (!line.empty())
                    bot->getUplink()->serverNotice(
                        theChan, std::format("{}[{}] {} {}{}", getColour(v), bot->getNickName(),
...
void Logger::rotateLogs() {
    ...
    logFile.open(logFilePath, std::ios::app);
    if (!logFile.is_open()) {
        elog << "Warning: Could not reopen logfile " << logFilePath << " after rotation"
             << std::endl;
    }
}
} // namespace gnuworld
