...
extern "C" {
xClient* _gnuwinit(const string& args) { return new snoop(args); }
}

snoop::snoop(const string& confFileName) : xClient(confFileName) {
...
void snoop::OnChannelMessage(iClient* srcClient, Channel* theChan, const string& Message) {
  ...
  if (command == "spawnclient") {
    ...
    getUplink()->UnloadClient(this, "Something has changed in the matrix...");
    getUplink()->LoadClient("libsnoop.la", getConfigFileName());
  } else if (command == "shutdown") {
    getUplink()->Shutdown();
  }
}

void snoop::handleSpawnClient(iClient* srcClient, Channel* theChan, const StringTokenizer& st) {
  ...
  if (Network->findNick(nickname) != 0) {
    Notice(srcClient, "SPAWNCLIENT: Nickname %s already exists", nickname.c_str());
    return;
  }
  ...
  if (!getUplink()->AttachClient(newClient, this)) {
    elog << "snoop::handleSpawnClient> Failed to add new client: " << *newClient << endl;
  ...
  Message(theChan,
          "Successfully created nick %s and joined "
          "relay channel %s",
          nickname.c_str(), relayChanName.c_str());
}

void snoop::handleSpawnJoin(iClient* srcClient, Channel* theChan, const StringTokenizer& st) {
  ...
  if (0 == fakeClient) {
    Notice(srcClient, "Nick \'%s\' does not exist", st[2].c_str());
    return;
  }
  ...
  if (!getUplink()->JoinChannel(fakeClient, st[3])) {
    ...
    Message(theChan, "%s successfully joined %s", st[2].c_str(), st[3].c_str());
  }
}

void snoop::handleSpawnPart(iClient* srcClient, Channel* theChan, const StringTokenizer& st) {
...
void snoop::usage(iClient* srcClient, const string& command) {
  if (command == "spawnclient") {
...
void snoop::OnFakeChannelMessage(iClient* srcClient, iClient* fakeClient, Channel* theChan,
  ...
  if (0 == relayChan) {
    elog << "snoop::OnFakeChannelMessage> Fake client: " << *fakeClient
         << " unable to find relay chan: " << relayChanName << endl;
    return;
  }

  stringstream s;
  s << "[RELAY " << theChan->getName() << "] " << srcClient->getNickName() << "!"
    << srcClient->getUserName() << "@" << srcClient->getInsecureHost() << ": " << Message;

  FakeMessage(relayChan, fakeClient, s.str().c_str());
}

void snoop::handleSpawnQuit(iClient* srcClient, const StringTokenizer& st) {
...
bool snoop::validNickname(const string& nickname) const {
  ...
  for (string::const_iterator sItr = nickname.begin(); sItr != nickname.end(); ++sItr) {
    if (*sItr >= 'A' && *sItr <= '}') {
      // ok
      continue;
    }
