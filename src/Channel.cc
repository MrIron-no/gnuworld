...
void Channel::onMode(const vector<std::pair<bool, Channel::modeType>>& modeVector) {
  ...
  for (modeVectorType::const_iterator mItr = modeVector.begin(); mItr != modeVector.end(); ++mItr) {
...
void Channel::onModeV(const vector<std::pair<bool, ChannelUser*>>& voiceVector) {
  ...
  for (voiceVectorType::const_iterator ptr = voiceVector.begin(); ptr != voiceVector.end(); ++ptr) {
    if (ptr->first) {
      ptr->second->setModeV();
    } else {
