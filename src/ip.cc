...
xIP::xIP(const irc_in_addr& IP) { this->IP = IP; }

xIP::xIP(const xIP& rhs) : IP(rhs.IP) {}
...
xIP::xIP(const string& IP, bool Base64) {
  ...
  // TODO: This is the ELSE part: When do we get here, if we even ever get, and HOW do we deal with
  // this?!
  /*
...
const irc_in_addr& xIP::GetLongIP() const { return IP; }

string xIP::GetBase64IP() const {
  char ip_base64[25]; // 64
  int IsIPv6 = 1;
  if (irc_in_addr_is_ipv4(&this->IP))
    IsIPv6 = 0;
