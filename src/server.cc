...
void xServer::BurstServer(iServer* fakeServer) {
  ...
  if (fakeServer->isJupe()) {
    ...
    Write("%s S %s %d %d %d J%02d %s 0 :%s\n", getCharYY().c_str(), fakeServer->getName().c_str(),
          2, 0, fakeServer->getConnectTime(),
          10, // version
...
bool xServer::AttachClient(xClient* Client, bool doBurst) {
  ...
  iClient* theIClient = new (std::nothrow)
      iClient(getIntYY(), Client->getCharYYXXX(), Client->getNickName(), Client->getUserName(),
              "AAAAAA", Client->getHostName(), Client->getHostName(), Client->getModes(), string(),
              0, 0, string(), string(), string(), Client->getDescription(), ::time(0));
  assert(theIClient != 0);
    ...
    for (string::const_iterator ptr = st[0].begin(); ptr != st[0].end(); ++ptr) {
      ...
      case 'l': {
        if (argPos >= st.size()) {
          elog << "xServer::JoinChannel> Invalid"
               << " number of arguments to chanModes "
               << " in 'l' (" << chanModes << ") for " << theClient->getNickName() << " in "
               << chanName << endl;
          break;
        }
...
bool xServer::Mode(xClient* theClient, Channel* theChan, const string& modes, const string& args) {
  ...
  */
  ...
  for (; tokenIndex < st.size();) {
    ...
    for (size_t charIndex = 0; charIndex < st[tokenIndex].size(); ++charIndex) {
      ...
      case 'v': {
        ...
        rawModeVector.push_back(make_pair(polarityString + theChar, targetClient->getCharYYXXX()));
      } break; // 'o', 'v'
      } // switch()
    } // for( char )
  ...
  for (rawModeVectorType::size_type modeIndex = 0; modeIndex < rawModeVector.size();) {
    ...
    outputSS << modeSource << " M " << theChan->getName() << " " << outputModes << " " << outputArgs
             << ends;
...
void xServer::setBursting(bool newVal) {
  ...
  if (newVal) {
    ...
    // Completed bursting
    elog << "Completed net burst in " << (burstEnd - burstStart) << " seconds, read " << burstBytes
         << " bytes and processed " << burstLines << " commands" << endl;
  }
}
...
void xServer::doShutdown() {
  ...
  count = 0;
  // Clear the channels
  for (xNetwork::clientIterator cItr = Network->clients_begin(); cItr != Network->clients_end();) {
...
void xServer::OnTimeout(Connection* cPtr) {
  ...
  if (cPtr == serverConnection) {
    ...
    lastLoop = true;
    elog << s.str(); /* Line not added to the debug log file - possibly because the program exits
                        quickly because of lastLoop. Didn't verify */
  }
}
