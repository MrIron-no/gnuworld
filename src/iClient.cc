...
iClient::iClient(const unsigned int& /* _uplink */, const string& _yyxxx, const string& _nickName,
                 const string& _userName, const string& _hostBase64, const string& _insecureHost,
                 const string& _realInsecureHost, const string& _mode, const string& _account,
                 const unsigned int _account_id, const flagType _account_flags,
                 const string& _tls_fingerprint, const string& _description, const time_t& _nick_ts)
    : NetworkTarget(_yyxxx), nickName(_nickName), userName(_userName),
      IP(xIP(_hostBase64, true).GetLongIP()), insecureHost(_insecureHost),
      realInsecureHost(_realInsecureHost), description(_description), nick_ts(_nick_ts),
      first_nick_ts(_nick_ts), mode(0), account(_account), account_id(_account_id),
      account_flags(_account_flags), tlsFingerprint(_tls_fingerprint) {
  setModes(_mode);
  customDataMap = 0;
...
iClient::iClient(const unsigned int& /* _uplink */, const string& _yyxxx, const string& _nickName,
                 ...
                 const string& _realInsecureHost, const string& _mode, const string& _account,
                 const unsigned int _account_id, const flagType _account_flags,
                 const string& _tls_fingerprint, const string& _setHost, const string& _fakeHost,
                 const string& _description, const time_t& _nick_ts)
    : NetworkTarget(_yyxxx), nickName(_nickName), userName(_userName),
      IP(xIP(_hostBase64, true).GetLongIP()), insecureHost(_insecureHost),
      realInsecureHost(_realInsecureHost), description(_description), nick_ts(_nick_ts),
      first_nick_ts(_nick_ts), mode(0), account(_account), account_id(_account_id),
...
const iServer* iClient::getServer() { return Network->findServer(getIntYY()); }
void iClient::setModes(const string& newModes) {
  for (string::size_type i = 0; i < newModes.size(); i++) {
    ...
    default:
      // Unknown mode
      elog << "iClient> Unknown user mode: " << newModes[i] << ", in modes string: " << newModes
           << std::endl;
      break;
    } // switch
  } // for
...
bool iClient::removeChannel(Channel* theChan) {
  ...
  for (channelIterator ptr = channelList.begin(); ptr != channelList.end(); ++ptr) {
    if (*ptr == theChan) {
...
bool iClient::setCustomData(xClient* theClient, void* data) {
  ...
  return customDataMap->insert(customDataMapType::value_type(theClient, data)).second;
}
...
bool iClient::addChannel(Channel* theChan) {
  ...
  if (std::find(channelList.begin(), channelList.end(), theChan) != channelList.end()) {
...
bool iClient::findChannel(const Channel* theChan) const {
  ...
  return std::find(channelList.begin(), channelList.end(), theChan) != channelList.end();
}
