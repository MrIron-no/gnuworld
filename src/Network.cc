...
bool xNetwork::addClient(iClient* newClient) {
  ...
  if (!numericMap.insert(numericMapType::value_type(newClient->getIntYYXXX(), newClient)).second) {
  ...
  while (chanPtr != retMe->channels_end()) {
    ...
    if (NULL == theChanUser) {
      elog << "xNetwork::removeClient> Unable to find "
           << "ChannelUser in channel " << (*chanPtr)->getName() << ", for iClient: " << *retMe
           << endl;
    }
    delete theChanUser;
    theChanUser = 0;
...
void xNetwork::rehashNick(const string& yyxxx, const string& newNick, const time_t& newTS) {
  ...
  if (NULL == theClient) {
    elog << "xNetwork::rehashNick> Unable to find numeric: " << yyxxx << ", new nick: " << newNick
         << endl;
    return;
  }
  ...
  for (yyVectorType::const_iterator yyIterator = yyVector.begin(); yyIterator != yyVector.end();
    ...
    if (intYY != tmpServer->getIntYY()) {
      ...
      theServer->PostEvent(EVT_NETBREAK, static_cast<void*>(tmpServer),
                           static_cast<void*>(findServer(intYY)), static_cast<void*>(&Reason));
    }
    delete tmpServer;
  }
}
...
list<const iClient*> xNetwork::matchUserHost(const string& wildUserHost) const {
  ...
  for (list<const iClient*>::const_iterator ptr = matchingHosts.begin(); ptr != matchingHosts.end();
       ++ptr) {
...
bool xNetwork::addFakeServer(iServer* fakeServer, xClient* owningClient) {
  ...
  if (!fakeServerMap.insert(make_pair(fakeServer->getIntYY(), make_pair(fakeServer, owningClient)))
           .second) {
...
bool xNetwork::freeClientNumeric(unsigned int intYYXXX) {
  ...
  if (rsItr == reservedNumericMap.end()) {
    elog << "xNetwork::freeClientNumeric> Unable to find "
         << "server intYY for intYY/intXXX/intYYXXX: " << intYY << '/' << intXXX << '/' << intYYXXX
         << endl;
    return false;
  }
  ...
  if (numItr == rsItr->second.end()) {
    elog << "xNetwork::freeClientNumeric> Unable to find "
         << "client intXXX for intYY/intXXX/intYYXXX: " << intYY << '/' << intXXX << '/' << intYYXXX
         << endl;
    return false;
  }
  ...
  for (; sItr != fakeServers_end(); ++sItr) {
    if (!strcasecmp(sItr->second.first->getName(), name)) {
      ...
      elog << "xNetwork::removeFakeServerName> Found "
           << "matching server name for name: " << name << ", server: " << *(sItr->second.first)
           << endl;
...
list<iClient*> xNetwork::findFakeClients(xClient* owningClient) const {
  ...
  for (fakeClientMapType::const_iterator cItr = fakeClientMap.begin(); cItr != fakeClientMap.end();
       ++cItr) {
