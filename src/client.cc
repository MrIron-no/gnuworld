...
bool xClient::Message(const string& Channel, const char* Message, ...) {
  if (isConnected() && Message && Message[0] != 0) {
    ...
    return MyUplink->Write("%s P #%s :%s\r\n", getCharYYXXX().c_str(),
                           (Channel[0] == '#') ? (Channel.c_str() + 1) : Channel.c_str(), buffer);
  }
  return false;
}
...
bool xClient::Message(const string& chanName, const string& Message) {
  ...
  return MyUplink->Write("%s P %s :%s", getCharYYXXX().c_str(), chanName.c_str(), Message.c_str());
}
...
bool xClient::Kill(iClient* theClient, const string& reason, bool asServer) {
  ...
  MyUplink->PostEvent(EVT_KILL, 0, static_cast<void*>(theClient), static_cast<void*>(&localReason));
...
bool xClient::Op(Channel* theChan, const std::vector<iClient*>& clientVector) {
  ...
  for (xServer::opVectorType::const_iterator ptr = opVector.begin(), end = opVector.end();
    ...
    if ((MAX_CHAN_MODES == modeString.size()) || ((ptr + 1) == end)) {
      stringstream s;
      s << getCharYYXXX() << " M " << theChan->getName() << ' ' << "+" << modeString << ' ' << args
...
bool xClient::Voice(Channel* theChan, const std::vector<iClient*>& clientVector) {
  ...
  for (xServer::voiceVectorType::const_iterator ptr = voiceVector.begin(), end = voiceVector.end();
       ptr != end; ++ptr) {
    modeString += 'v';
    args += ptr->second->getCharYYXXX() + ' ';
...
bool xClient::Kick(Channel* theChan, iClient* theClient, const string& reason, bool modeAsServer) {
  ...
  if (NULL == theChan->findUser(theClient)) {
    elog << "xClient::Kick> Can't find " << theClient->getNickName() << " on channel "
  ...
  stringstream s;
  s << SendAs << " K " << theChan->getName() << ' ' << theClient->getCharYYXXX() << " :" << reason;
  ...
  /*
   * Remove/cleanup now the client, an eventual later part message from a server will be ignored
   * anyway In this way any own (fake)clients will be cleaned up too
   */
  ...
  if (destChanUser != NULL) {
    ...
    if (!theClient->removeChannel(theChan)) {
      elog << "xClient::Kick> Unable to remove channel " << theChan->getName()
           << " from the iClient " << *theClient << endl;
    }
    MyUplink->PostChannelKick(theChan, getInstance(), theClient, reason, localKick);

  } else
    elog << "xClient::Kick> destChanUser == NULL !!" << endl;
...
bool xClient::Kick(Channel* theChan, const std::vector<iClient*>& theClients, const string& reason,
  ...
  for (std::vector<iClient*>::const_iterator ptr = theClients.begin(); ptr != theClients.end();
       ++ptr) {
    if (0 == *ptr) {
      elog << "xClient::Kick(vector)> NULL target client" << endl;
      continue;
    }
    ...
    stringstream s;
    s << SendAs << " K " << theChan->getName() << ' ' << (*ptr)->getCharYYXXX() << " :" << reason;
...
bool xClient::Join(Channel* theChan, const string& chanModes, const time_t& joinTime, bool getOps) {
  ...
  if ((joinTime > 0) && (joinTime < theChan->getCreationTime())) {
    // The join is older than the creation time of the channel, need to remove all the modes of the
    // channel
