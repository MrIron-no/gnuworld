...
void INVITECommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (!theChannel) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::cant_find_channel,
                                 std::string("Could not find channel %s on the network."))
                    .c_str(),
                st[1].c_str());
    return;
  }
  ...
  if (!theBot) {
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::chanfix_not_in_chan, std::string("I am not in %s."))
            .c_str(),
  ...
  if (theChannelUser) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::already_in_channel,
                                 std::string("You are already in %s!"))
                    .c_str(),
                theChannel->getName().c_str());
    return;
  }
  ...
  bot->logAdminMessage("%s (%s) INVITE %s [CODER-CHAN]",
                       ...
                       theClient->getRealNickUserHost().c_str(), theChannel->getName().c_str());
