...
void DELHOSTCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (targetUser->getFlag(sqlcfUser::F_OWNER) && !theUser->getFlag(sqlcfUser::F_OWNER)) {
    bot->SendTo(
        theClient,
        bot->getResponse(
               theUser, language::cant_del_host_an_owner,
               std::string("You cannot delete a host from an owner unless you're an owner."))
            .c_str());
    return;
  }
  ...
  if (targetUser->getFlag(sqlcfUser::F_USERMANAGER) && !theUser->getFlag(sqlcfUser::F_OWNER)) {
    bot->SendTo(
        theClient,
        bot->getResponse(
               theUser, language::cant_del_host_manager,
               std::string("You cannot delete a host from a user manager unless you're an owner."))
            .c_str());
    return;
  }
  ...
  if (theUser->getFlag(sqlcfUser::F_SERVERADMIN) && !theUser->getFlag(sqlcfUser::F_USERMANAGER)) {
    if (targetUser->getGroup() != theUser->getGroup()) {
      bot->SendTo(theClient,
                  bot->getResponse(
                         theUser, language::cant_del_host_diff_group,
                         std::string("You cannot delete a host from a user in a different group."))
                      .c_str());
      return;
    }
  }
  ...
  if (!targetUser->hasHost(st[2].c_str())) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::user_doesnt_have_host,
                                 std::string("User %s doesn't have hostmask %s."))
                    .c_str(),
  ...
  if (!targetUser->delHost(bot->getLocalDBHandle(), st[2].c_str())) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::failed_deleting_host,
                                 std::string("Failed deleting hostmask %s from user %s."))
                    .c_str(),
  ...
  bot->SendTo(theClient,
              bot->getResponse(theUser, language::deleted_hostmask,
                               std::string("Deleted hostmask %s from user %s."))
                  .c_str(),
              st[2].c_str(), targetUser->getUserName().c_str());
