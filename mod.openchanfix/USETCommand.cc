...
void USETCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (st.size() == 4) {
    ...
    if (option == "NEEDOPER") {
      ...
      if (!theUser->getFlag(requiredFlags)) {
        bot->SendTo(
            theClient,
            bot->getResponse(theUser, language::requires_auth_and_flag,
                             std::string("This command requires authentication and flag '%c'."))
                .c_str(),
  ...
  if (option == "NOTICE") {
    if (value == "ON" || value == "YES" || value == "1") {
      ...
      bot->SendTo(theClient, bot->getResponse(theUser, language::send_notices,
                                              std::string("I will now send you notices."))
                                 .c_str());
      theUser->commit(bot->getLocalDBHandle());
      return;
    } else if (value == "OFF" || value == "NO" || value == "0") {
      theUser->setNotice(false);
      bot->SendTo(theClient, bot->getResponse(theUser, language::send_privmsgs,
                                              std::string("I will now send you privmsgs."))
                                 .c_str());
      theUser->commit(bot->getLocalDBHandle());
      return;
    } else {
      bot->SendTo(theClient, bot->getResponse(theUser, language::uset_notice_on_off,
                                              std::string("Please use USET NOTICE <on/off>."))
                                 .c_str());
      return;
    }
  }
  ...
  if (option == "LANG") {
    ...
    if (ptr != bot->languageTable.end()) {
      ...
      bot->SendTo(
          theClient,
          bot->getResponse(theUser, language::lang_set_to, std::string("Language is set to %s."))
              .c_str(),
          lang.c_str());
      return;
    }
