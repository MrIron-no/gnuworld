...
void ADDUSERCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (targetUser) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::user_already_exists,
                                 std::string("User %s already exists."))
                    .c_str(),
                st[1].c_str());
    return;
  }
  ...
  if (!newUser->Insert(bot->getLocalDBHandle())) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::error_creating_user,
                                 std::string("Error creating user %s. (Insertion failed)"))
                    .c_str(),
                st[1].c_str());
    return;
  }
  ...
  if (st.size() > 2) {
    if (newUser->addHost(bot->getLocalDBHandle(), st[2].c_str())) {
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::created_user_w_host,
                                   std::string("Created user %s (%s)."))
                      .c_str(),
                  st[1].c_str(), st[2].c_str());
      bot->logAdminMessage("%s (%s) ADDUSER %s %s",
                           theUser ? theUser->getUserName().c_str()
                                   : theClient->getNickName().c_str(),
                           theClient->getRealNickUserHost().c_str(), st[1].c_str(), st[2].c_str());
    } else {
      bot->SendTo(theClient,
    ...
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::created_user_wo_host, std::string("Created user %s."))
            .c_str(),
        st[1].c_str());
