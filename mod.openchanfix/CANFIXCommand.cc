...
void CANFIXCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (highscore == false) {
    bot->SendTo(
        theClient,
        bot->getResponse(
               theUser, language::can_fix_not_enough_points,
               std::string("You do not have a high enough score for %s to check canfix list."))
            .c_str(),
        st[1].c_str());
    return;
  }
  ...
  bot->SendTo(theClient,
              bot->getResponse(theUser, language::canfix_op_accounts,
                               std::string("Accounts who can issue fixes in channel %s:"))
                  .c_str(),
              st[1].c_str());

  bot->SendTo(theClient,
              bot->getResponse(theUser, language::canfix_rank_score_acc_header,
                               std::string("Account -- Time first opped / Time last opped"))
                  .c_str());
  ...
  for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
    ...
    wScore = 100 - percent;
    if ((static_cast<float>(wScore) <= bot->getTopOpPercent()) &&
        (static_cast<float>(pScore) >= bot->getMinFixScore())) {
      bot->SendTo(theClient, "%s -- %s / %s", curOp->getAccount().c_str(), firstop.c_str(),
                  lastop.c_str());
    }
