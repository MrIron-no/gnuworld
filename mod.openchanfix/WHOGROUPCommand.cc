...
void WHOGROUPCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  bot->logAdminMessage(
      "%s (%s) WHOGROUP %s", theUser ? theUser->getUserName().c_str() : "!NOT-LOGGED-IN!",
      theClient->getRealNickUserHost().c_str(), (st.size() > 1) ? st[1].c_str() : "");
  ...
  if (st.size() == 1) {
    ...
    if (!cacheCon->Exec(theQuery.str(), true)) {
      elog << "chanfix::WHOGROUPCommand> SQL Error: " << cacheCon->ErrorMessage() << std::endl;
      return;
    }

    // SQL query returned no errors
    unsigned int numGroups = 0;
    bot->SendTo(theClient, bot->getResponse(theUser, language::whogroup_list_groups,
                                            std::string("List of all groups:"))
                               .c_str());
    ...
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::number_of_groups, std::string("Number of groups: %d."))
            .c_str(),
        numGroups);
  ...
  if (theUser->getFlag(sqlcfUser::F_SERVERADMIN) && !theUser->getFlag(sqlcfUser::F_USERMANAGER)) {
    if (string_lower(st[1]) != theUser->getGroup()) {
      bot->SendTo(theClient, bot->getResponse(theUser, language::whogroup_your_group,
                                              std::string("You can only WHOGROUP on your group."))
                                 .c_str());
      return;
    }
  }
  ...
  bot->SendTo(theClient,
              bot->getResponse(theUser, language::users_with_group,
                               std::string("Users with group %s [username (flags)]:"))
                  .c_str(),
              st[1].c_str());
