...
public:
  Command(chanfix* _bot, const std::string& _commandName, const std::string& _help,
          std::string::size_type _numParams, unsigned short int _requiredFlags)
      : bot(_bot), server(0), commandName(_commandName), help(_help), numParams(_numParams),
        requiredFlags(_requiredFlags) {}
  ...
  inline std::string::size_type getNumParams() const { return numParams; }

  inline unsigned short int getRequiredFlags() const { return requiredFlags; }
...
#define DECLARE_COMMAND(commandName)                                                               \
  ...
  public:                                                                                          \
    commandName##Command(chanfix* _bot, const std::string& _commandName, const std::string& _help, \
                         std::string::size_type _numParams, unsigned short int _requiredFlags)     \
