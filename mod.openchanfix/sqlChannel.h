...
public:
  ...
  inline time_t getLastAttempt() const { return last; }

  inline time_t getLastSimAttempt() const { return simlast; }

  inline time_t getFixStart() const { return start; }

  inline time_t getSimStart() const { return simstart; }

  inline int getMaxScore() const { return maxScore; }

  inline int getAmountSimOpped() const { return amtopped; }

  inline int getTMaxScore() const { return tmaxScore; }

  inline bool getModesRemoved() const { return modesRemoved; }

  inline bool getSimModesRemoved() const { return simModesRemoved; }

  inline bool useSQL() const { return inSQL; }
  ...
  inline void setChannel(const std::string& _channel) { channel = _channel; }
  ...
  inline void setLastAttempt(time_t _last) { last = _last; }

  inline void setLastSimAttempt(time_t _simlast) { simlast = _simlast; }

  inline void setFixStart(time_t _start) { start = _start; }

  inline void setSimStart(time_t _simstart) { simstart = _simstart; }

  inline void setAmountSimOpped(unsigned int _amtopped) { amtopped = _amtopped; }

  inline void setMaxScore(unsigned int _maxScore) { maxScore = _maxScore; }

  inline void setTMaxScore(unsigned int _tmaxScore) { tmaxScore = _tmaxScore; }

  inline void setModesRemoved(bool _modesRemoved) { modesRemoved = _modesRemoved; }

  inline void setSimModesRemoved(bool _simModesRemoved) { simModesRemoved = _simModesRemoved; }

  inline void setUseSQL(bool _inSQL) { inSQL = _inSQL; }
