...
void CHANFIXCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (!bot->doChanFix()) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::manual_fix_disabled,
                                 std::string("Sorry, manual chanfixes are currently disabled."))
                    .c_str());
    return;
  }
  ...
  if (bot->getState() != chanfix::RUN) {
    bot->SendTo(
        theClient,
        bot->getResponse(
               theUser, language::not_enough_servers,
               std::string(
  ...
  if (theChan->getMaxScore() <=
    ...
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::highscore_channel,
                                 std::string("The highscore in channel %s is %d which is lower "
                                             "than the minimum score required (%.2f * %d = %d)."))
                    .c_str(),
                theChan->getChannel().c_str(), theChan->getMaxScore(), FIX_MIN_ABS_SCORE_END,
                MAX_SCORE, static_cast<int>(static_cast<float>(FIX_MIN_ABS_SCORE_END) * MAX_SCORE));
    return;
  }
  ...
  if (bot->isBeingAutoFixed(netChan)) {
    if (!override) {
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::channel_being_auto_fixed,
                                   std::string("The channel %s is being automatically fixed. "
                                               "Append the OVERRIDE flag to force a manual fix."))
                      .c_str(),
                  netChan->getName().c_str());
      return;
    } else {
  ...
  if (bot->isTempBlocked(theChan->getChannel()) && !override) {
    bot->SendTo(
        theClient,
        bot->getResponse(
               theUser, language::channel_temp_blocked,
               std::string(
                   "The channel %s is TEMPBLOCKED. To fix it, please append the OVERRIDE flag."))
            .c_str(),
        theChan->getChannel().c_str());
    return;
  }
  ...
  if (theChan->getFlag(sqlChannel::F_ALERT) && !override) {
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::channel_has_notes,
                         std::string("Alert: The channel %s has notes. Use \002INFO %s\002 to read "
                                     "them. Append the OVERRIDE flag to force a manual fix."))
            .c_str(),
  ...
  theChan->addNote(bot->getLocalDBHandle(), sqlChannel::EV_CHANFIX, theClient,
                   (override) ? "[override]" : "");

  /* Log the chanfix */
  bot->SendTo(theClient,
              bot->getResponse(theUser, language::manual_chanfix_ack,
                               std::string("Manual chanfix acknowledged for %s"))
                  .c_str(),
              netChan->getName().c_str());
