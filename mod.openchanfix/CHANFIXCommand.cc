...
void CHANFIXCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
        ...
        if (!override) {
            bot->SendTo(
                ...
                bot->getResponse(theUser, language::channel_being_auto_fixed,
                                 std::string("The channel %s is being automatically fixed. Append "
                                             "the OVERRIDE flag to force a manual fix."))
                    .c_str(),
                netChan->getName().c_str());
            return;
        } else {
    ...
    if (bot->isTempBlocked(theChan->getChannel()) && !override) {
        bot->SendTo(theClient,
                    bot->getResponse(theUser, language::channel_temp_blocked,
                                     std::string("The channel %s is TEMPBLOCKED. To fix it, please "
                                                 "append the OVERRIDE flag."))
                        .c_str(),
                    theChan->getChannel().c_str());
        return;
    }
    ...
    if (theChan->getFlag(sqlChannel::F_ALERT) && !override) {
        bot->SendTo(
            theClient,
            bot->getResponse(
                   theUser, language::channel_has_notes,
                   std::string("Alert: The channel %s has notes. Use \002INFO %s\002 to read them. "
