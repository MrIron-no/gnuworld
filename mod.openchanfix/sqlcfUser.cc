...
sqlcfUser::sqlcfUser(sqlManager* _myManager)
    : id(0), user_name(), created(0), last_seen(0), last_updated(0), last_updated_by(),
      language_id(0), group(), flags(0), isSuspended(false), useNotice(true), needOper(true) {
...
bool sqlcfUser::commit(dbHandle* cacheCon) {
  ...
  userCommit << "UPDATE users SET "
             ...
             << "needoper = " << (needOper ? "'t'" : "'f'") << " WHERE "
             << "id = " << id;

  if (!cacheCon->Exec(userCommit.str())) {
    elog << "sqlcfUser::commit> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
    retval = false;
  } else
    retval = true;
...
bool sqlcfUser::Insert(dbHandle* cacheCon) {
  ...
  if (!cacheCon->Exec(insertString.str())) {
    elog << "sqlcfUser::Insert> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
    retval = false;
    maxUserId--;
  } else
    retval = true;
...
bool sqlcfUser::Delete(dbHandle* cacheCon) {
  ...
  if (!cacheCon->Exec(hostString.str())) {
    elog << "sqlcfUser::Delete> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
    retval = false;
  } else
    retval = true;
...
void sqlcfUser::loadHostList(dbHandle* cacheCon) {
  ...
  // dbHandle* cacheCon = myManager->getConnection();
  // if (!cacheCon)
  // 	elog << "[sqlcfUser::loadHostList() Could not get a connection to the database from the
  // manager." << std::endl;

  /* Retrieve the hosts */
  std::stringstream theQuery;

  theQuery << "SELECT host FROM hosts WHERE user_id = " << id << ends;
...
bool sqlcfUser::addHost(dbHandle* cacheCon, const std::string& _theHost) {
  ...
  insertString << "INSERT INTO hosts "
               << "(user_id, host) VALUES "
               << "(" << id << ", '" << escapeSQLChars(_theHost).c_str() << "')";

  if (!cacheCon->Exec(insertString.str())) {
    elog << "sqlcfUser::addHost> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
    retval = false;
  } else
    retval = true;
...
bool sqlcfUser::delHost(dbHandle* cacheCon, const std::string& _theHost) {
  ...
  deleteString << "DELETE FROM hosts "
               << "WHERE user_id = " << id << " AND lower(host) = '"
               << escapeSQLChars(string_lower(_theHost)).c_str() << "'";

  if (!cacheCon->Exec(deleteString.str())) {
    elog << "sqlcfUser::delHost> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
    retval = false;
  } else
    retval = true;
...
bool sqlcfUser::matchHost(const std::string& _host) {
  if (hostList.size() < 1)
    return false;
  for (hostListType::iterator itr = hostList.begin(); itr != hostList.end(); ++itr) {
    if (match(*itr, _host) == 0)
      return true;
  }
  return false;
}
