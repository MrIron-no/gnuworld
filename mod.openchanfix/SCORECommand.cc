...
void SCORECommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
    ...
    if (st[2][0] == '=') {
      ...
      if (!curClient) {
        if (compact)
          ...
          bot->SendTo(
              theClient,
              bot->getResponse(theUser, language::no_such_nick, std::string("No such nick %s."))
                  .c_str(),
              scUser);
        return;
      } else {
        ...
        for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
          ...
          if (string_lower(curOp->getAccount()) == string_lower(curClient->getAccount())) {
            ...
            if (compact) {
              ...
              bot->SendTo(theClient, "~U %s %s %u", st[1].c_str(), curOp->getAccount().c_str(),
                          (curOp->getPoints() + curOp->getBonus()));
            } else {
              bot->SendTo(
                  theClient,
                  bot->getResponse(
                         theUser, language::score_for_channel_nick,
                         std::string("Score for %s (%s) in channel %s: %u. (Ranked #%u of %d)"))
                      .c_str(),
                  curClient->getNickName().c_str(), curOp->getAccount().c_str(), st[1].c_str(),
        ...
        if (string_lower(curOp->getAccount()) == string_lower(scUser)) {
          if (compact)
            ...
            bot->SendTo(
                theClient,
                bot->getResponse(
                       theUser, language::score_for_channel_account,
                       std::string("Score for account %s in channel %s: %u. (Ranked #%u of %d)"))
                    .c_str(),
  ...
  if (compact) {
    ...
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::top_scores_for_nonops,
  ...
  if (theChan) {
    ...
    if (theChan->getFlag(sqlChannel::F_BLOCKED))
      bot->SendTo(
          theClient,
          bot->getResponse(theUser, language::info_chan_blocked, std::string("%s is BLOCKED."))
              .c_str(),
      ...
      bot->SendTo(
          theClient,
          bot->getResponse(theUser, language::info_chan_alerted, std::string("%s is ALERTED."))
              .c_str(),
          theChan->getChannel().c_str());
  }
