...
void SCORECommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
    ...
    if (st.size() > 2) {
        bot->logAdminMessage("%s (%s) %s %s",
                             ...
                             theClient->getRealNickUserHost().c_str(), compact ? "CSCORE" : "SCORE",
                             st.assemble(1).c_str());
        ...
        if (st[2][0] == '=') {
            ...
            if (!curClient) {
                if (compact)
                    bot->SendTo(theClient,
                                bot->getResponse(theUser, language::compact_no_such_nick,
                                                 std::string("~U %s no@such.nick 0"))
                                    .c_str(),
                                st[1].c_str());
                else
                    bot->SendTo(theClient,
                                bot->getResponse(theUser, language::no_such_nick,
                                                 std::string("No such nick %s."))
                                    .c_str(),
                                scUser);
                return;
            } else {
                ...
                for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end();
                     opPtr++) {
                    curOp = *opPtr;
                    ranking++;
                    if (string_lower(curOp->getAccount()) ==
                        string_lower(curClient->getAccount())) {
                        // Score for "reed@local.host" in channel "#coder-com": 4.
                        // Do it like they do on OCF, baby.
                        if (compact) {
                            //~U #coder-com mc@local.host 176
                            bot->SendTo(theClient, "~U %s %s %u", st[1].c_str(),
                                        curOp->getAccount().c_str(),
                                        (curOp->getPoints() + curOp->getBonus()));
                        } else {
                            bot->SendTo(theClient,
                                        bot->getResponse(theUser, language::score_for_channel_nick,
                                                         std::string("Score for %s (%s) in channel "
                                                                     "%s: %u. (Ranked #%u of %d)"))
                                            .c_str(),
                                        curClient->getNickName().c_str(),
                                        curOp->getAccount().c_str(), st[1].c_str(),
                                        (curOp->getPoints() + curOp->getBonus()), ranking,
                                        numMyOps);
                        }
                        return;
                    }
                ...
                if (compact)
                    bot->SendTo(theClient,
                                bot->getResponse(theUser, language::compact_no_score_for_nick,
                                                 std::string("~U %s no@score.for.nick 0"))
                                    .c_str(),
                                st[1].c_str());
                else
                    bot->SendTo(theClient,
                                bot->getResponse(theUser, language::no_score_for_nick,
                                                 std::string("No score for %s on %s."))
                                    .c_str(),
                                curClient->getNickName().c_str(), st[1].c_str());
                return;
            } // else
        } else {
            /* Account */
            ...
            for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end();
                ...
                if (string_lower(curOp->getAccount()) == string_lower(scUser)) {
                    if (compact)
                        bot->SendTo(theClient, "~U %s %s %u", st[1].c_str(),
                                    curOp->getAccount().c_str(), curOp->getPoints());
                    else
                        bot->SendTo(theClient,
                                    bot->getResponse(theUser, language::score_for_channel_account,
                                                     std::string("Score for account %s in channel "
                                                                 "%s: %u. (Ranked #%u of %d)"))
                                        .c_str(),
                                    curOp->getAccount().c_str(), st[1].c_str(),
                                    (curOp->getPoints() + curOp->getBonus()), ranking, numMyOps);
            ...
            if (compact)
                bot->SendTo(theClient,
                            bot->getResponse(theUser, language::compact_no_score_for_account,
                                             std::string("~U %s no@score.for.account 0"))
                                .c_str(),
                            st[1].c_str());
            else
                bot->SendTo(theClient,
    ...
    if (compact) {
        ...
        bot->SendTo(theClient,
                    bot->getResponse(theUser, language::top_scores_for_chan,
                                     std::string("Top %u scores for channel %s in the database:"))
                        .c_str(),
        ...
        if (strScoresDB.str() == ".")
            bot->SendTo(
                theClient,
                bot->getResponse(theUser, language::score_none, std::string("None.")).c_str());
        else
            bot->SendTo(theClient, strScoresDB.str());

        if (!netChan) {
            bot->SendTo(theClient,
                        bot->getResponse(theUser, language::no_such_channel,
        ...
        bot->SendTo(
            theClient,
            bot->getResponse(theUser, language::top_scores_for_ops,
                             std::string("Top %u scores for current ops in %s in the database:"))
                .c_str(),
            minScoreReply, st[1].c_str());
        if (strScoresOP.str() == ".")
            bot->SendTo(
        ...
        bot->SendTo(theClient,
                    bot->getResponse(
                           theUser, language::top_scores_for_nonops,
                           std::string("Top %u scores for current non-ops in %s in the database:"))
                        .c_str(),
                    minScoreReply, st[1].c_str());
        if (strScoresNOP.str() == ".")
            bot->SendTo(
    ...
    if (theChan) {
        ...
        if (theChan->getFlag(sqlChannel::F_BLOCKED))
            bot->SendTo(theClient,
                        bot->getResponse(theUser, language::info_chan_blocked,
                                         std::string("%s is BLOCKED."))
                            .c_str(),
                        theChan->getChannel().c_str());
        else if (bot->isTempBlocked(theChan->getChannel()))
            bot->SendTo(theClient,
                        bot->getResponse(theUser, language::info_chan_temp_blocked,
                                         std::string("%s is TEMPBLOCKED."))
                            .c_str(),
                        theChan->getChannel().c_str());
        else if (theChan->getFlag(sqlChannel::F_ALERT))
            bot->SendTo(theClient,
                        bot->getResponse(theUser, language::info_chan_alerted,
                                         std::string("%s is ALERTED."))
                            .c_str(),
                        theChan->getChannel().c_str());
    }
