...
void LASTCOMCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (st.size() == 1) {
    ...
    theQuery << queryHeader << " ORDER BY ts DESC"
             << " LIMIT " << NumOfCom << std::ends;
  } else {
    ...
    if (st.size() > 2) {
      ...
      if (Days > 365) {
        bot->SendTo(theClient,
                    bot->getResponse(theUser, language::lastcom_too_far_back,
                                     std::string("You cannot see logs more than one year ago."))
                        .c_str());
        return;
      }
      static const char* queryHeader = "SELECT * FROM comlog where ts >";
      theQuery << queryHeader << (::time(0) - Days * 24 * 3600) << " ORDER BY ts ASC"
               << " LIMIT " << NumOfCom << std::ends;
    } else {
      static const char* queryHeader = "SELECT * FROM comlog ";
      theQuery << queryHeader << " ORDER BY ts DESC"
  ...
  elog << "chanfix::LASTCOM> " << theQuery.str().c_str() << std::endl;

  if (!cacheCon->Exec(theQuery.str(), true)) {
    elog << "chanfix::LASTCOM> SQL Error: " << cacheCon->ErrorMessage() << std::endl;
    return;
  }

  // SQL Query succeeded
  bot->SendTo(theClient,
              bot->getResponse(theUser, language::lastcom_listing_messages,
                               std::string("Listing last %d messages from day %d."))
                  .c_str(),
              NumOfCom, Days);

  // TODO: Fix this warning
  for (int i = (cacheCon->Tuples() - 1); i >= 0; i--) {
    bot->SendTo(theClient, "[ %s - %s ] %s", bot->convertToAscTime(atoi(cacheCon->GetValue(i, 0))),
                cacheCon->GetValue(i, 1).c_str(), cacheCon->GetValue(i, 2).c_str());
  }

  bot->SendTo(theClient,
              bot->getResponse(theUser, language::end_of_log, std::string("End of LASTCOM report."))
                  .c_str());
