...
void OPLISTCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (oCnt > OPCOUNT && !all)
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::top_unique_op_accounts,
                                 std::string("Top %d unique op accounts in channel %s:"))
                    .c_str(),
                OPCOUNT, st[1].c_str());
  else
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::found_unique_op_accounts,
                                 std::string("Found %d unique op accounts in channel %s:"))
                    .c_str(),
                oCnt, st[1].c_str());

  bot->SendTo(theClient,
              bot->getResponse(
                     theUser, language::rank_score_acc_header,
                     std::string("Rank Score Account -- Time first opped / Time last opped / Nick"))
                  .c_str());
  ...
  for (chanfix::chanOpsType::iterator opPtr = myOps.begin();
    ...
    bot->SendTo(theClient, "%3d. %4d  %s -- %s / %s%s%s%s%s%s", opCount,
                (curOp->getPoints() + curOp->getBonus()), curOp->getAccount().c_str(),
                firstop.c_str(), lastop.c_str(), inChan ? " / " : "",
                inChan ? nickName.c_str() : "", (days) ? " [" : "",
  ...
  bot->SendTo(
      theClient,
      bot->getResponse(theUser, language::end_of_oplist, std::string("End of OPLIST for %s"))
          .c_str(),
      st[1].c_str());
