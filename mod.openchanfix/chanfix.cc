...
chanfix::chanfix(const std::string& configFileName) : xClient(configFileName) {
    ...
    RegisterCommand(
        new LISTHOSTSCommand(this, "LISTHOSTS", "[username]", 1, sqlcfUser::F_LOGGEDIN));
    ...
    RegisterCommand(new SIMULATECommand(this, "SIMULATE", "<#channel> <auto/manual>", 3,
                                        0 /* Set to 0 to allow all opers to access it, otherwise
                                             this should be sqlcfUser::F_CHANFIX */
                                        ));
...
#endif
    RegisterCommand(
        new USERSCORESCommand(this, "USERSCORES", "<account>", 2, sqlcfUser::F_LOGGEDIN));
    RegisterCommand(
...
void chanfix::OnPrivateMessage(iClient* theClient, const std::string& Message, bool) {
    ...
    if (currentState == BURST) {
        if (theUser)
            SendTo(theClient,
                   getResponse(theUser, language::no_commands_during_burst,
                               std::string("Sorry, I do not accept commands during a burst."))
                       .c_str());
        else
            SendTo(theClient,
                   getResponse(
                       theUser, language::no_commands_during_burst_noper,
    ...
    if (commHandler == commandMap.end()) {
        SendTo(theClient,
               getResponse(theUser, language::unknown_command, std::string("Unknown command."))
                   .c_str());
        return;
    }
    ...
    */
    if (theClient->getAccount().length() == 0) {
        if (Command != "CANFIX" && Command != "HELP" && Command != "REQUESTOP") {
            SendTo(theClient,
                   getResponse(theUser, language::need_to_auth,
...
bool chanfix::logLastComMessage(iClient* theClient, const std::string& Message) {
    ...
    static const char* Main = "INSERT into comlog (ts,user_name,command) VALUES "
    ...
    if (!cacheCon->Exec(insertString.str())) {
        elog << "sqlChannel::Insert> Something went wrong: " << cacheCon->ErrorMessage()
             << std::endl;
    }
    ...
    if (localDBHandle->Exec(theQuery.str(), true)) {
        for (unsigned int i = 0; i < localDBHandle->Tuples(); i++) {
            ...
            if (ptr != sqlChanOps.end()) {
                ptr->second.insert(
                    sqlChanOpsType::mapped_type::value_type(newOp->getAccount(), newOp));
            } else {
...
void chanfix::precacheUsers() {
    ...
    if (!cacheCon)
        elog << "*** [chanfix::precacheUsers]: Error getting a new SQL connection through the "
                "manager."
             << std::endl;
...
bool chanfix::fixChan(sqlChannel* sqlChan, bool autofix) {
    ...
    unsigned int currentOps = countChanOps(netChan);
    if (currentOps >= maxOpped) {
        elog << "chanfix::fixChan> DEBUG: Enough clients opped on " << netChan->getName()
             << std::endl;
        return true;
    }
    ...
    for (chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
        ...
        if ((curOp->getPoints() + curOp->getBonus()) >= min_score) {
            ...
            while (acctPtr != end) {
                curClient = *acctPtr;
                if (curClient && !netChan->findUser(curClient)->isModeO()) {
                    elog << "chanfix::fixChan> DEBUG: Decided to op: " << curClient->getNickName()
                         << " on " << netChan->getName() << ". Client has "
                         << (curOp->getPoints() + curOp->getBonus())
                         << " points. ABS_MIN = " << min_score_abs
                         << " and REL_MIN = " << min_score_rel << std::endl;
                    modes += "o";
                    if (!args.empty())
                        args += " ";
                    args += curClient->getNickName();
...
void chanfix::processQueue() {
    ...
    for (fixQueueType::iterator ptr = manFixQ.begin(); ptr != manFixQ.end();) {
        ...
        if (ptr->second <= currentTime()) {
             ...
             */
            if (isFixed ||
                currentTime() - sqlChan->getFixStart() > CHANFIX_MAXIMUM + CHANFIX_DELAY) {
                Channel* theChan = Network->findChannel(sqlChan->getChannel());
...
void chanfix::rotateDB() {
     ...
     * then loop back through and pick out the users with 0 points total and check their
     * ts_firstopped if it is older than 1 day, delete the user cache: acct,chan
     */
    ...
    if (localDBHandle->Exec(langQuery.str(), true))
        for (unsigned int i = 0; i < localDBHandle->Tuples(); i++)
            languageTable.insert(languageTableType::value_type(
                localDBHandle->GetValue(i, 1), std::make_pair(atoi(localDBHandle->GetValue(i, 0)),
                                                              localDBHandle->GetValue(i, 2))));
