...
void ADDHOSTCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (targetUser->getFlag(sqlcfUser::F_OWNER) && !theUser->getFlag(sqlcfUser::F_OWNER)) {
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::cant_add_host_an_owner,
                         std::string("You cannot add a host to an owner unless you're an owner."))
            .c_str());
    return;
  }
  ...
  if (targetUser->getFlag(sqlcfUser::F_USERMANAGER) && !theUser->getFlag(sqlcfUser::F_OWNER)) {
    bot->SendTo(
        theClient,
        bot->getResponse(
               theUser, language::cant_add_host_manager,
               std::string("You cannot add a host to a user manager unless you're an owner."))
            .c_str());
    return;
  }
  ...
  if (theUser->getFlag(sqlcfUser::F_SERVERADMIN) && !theUser->getFlag(sqlcfUser::F_USERMANAGER)) {
    if (targetUser->getGroup() != theUser->getGroup()) {
      bot->SendTo(
          theClient,
          bot->getResponse(theUser, language::cant_add_host_diff_group,
                           std::string("You cannot add a host to a user in a different group."))
              .c_str());
      return;
    }
  }
  ...
  if (targetUser->matchHost(st[2].c_str())) {
    if ((st[2] != "*!*@*") && targetUser->hasHost("*!*@*")) {
      ...
      bot->SendTo(
          theClient,
          bot->getResponse(theUser, language::removed_default_hostmask,
                           std::string("Removed the default hostmask of *!*@* from user %s."))
              .c_str(),
          targetUser->getUserName().c_str());
    } else {
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::already_has_hostmask,
                                   std::string("User %s already has hostmask %s."))
                      .c_str(),
  ...
  if (!targetUser->addHost(bot->getLocalDBHandle(), st[2].c_str())) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::failed_adding_hostmask,
                                 std::string("Failed adding hostmask %s to user %s."))
                    .c_str(),
  ...
  bot->SendTo(theClient,
              bot->getResponse(theUser, language::adding_hostmask_to_user,
                               std::string("Added hostmask %s to user %s."))
                  .c_str(),
              st[2].c_str(), targetUser->getUserName().c_str());
