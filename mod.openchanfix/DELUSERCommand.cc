...
void DELUSERCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (targetUser->getFlag(sqlcfUser::F_OWNER) && !theUser->getFlag(sqlcfUser::F_OWNER)) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::cant_delete_an_owner,
                                 std::string("You cannot delete an owner unless you're an owner."))
                    .c_str());
    return;
  }
  ...
  if (targetUser->getFlag(sqlcfUser::F_USERMANAGER) && !theUser->getFlag(sqlcfUser::F_OWNER)) {
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::cant_delete_manager,
                         std::string("You cannot delete a user manager unless you're an owner."))
            .c_str());
    return;
  }
  ...
  if (theUser->getFlag(sqlcfUser::F_SERVERADMIN) && !theUser->getFlag(sqlcfUser::F_USERMANAGER)) {
    if (targetUser->getGroup() != theUser->getGroup()) {
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::cant_delete_from_diff_group,
                                   std::string("You cannot delete a user in a different group."))
                      .c_str());
      return;
    }
  }
  ...
  if (targetUser->Delete(bot->getLocalDBHandle())) {
    ...
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::deleted_user, std::string("Deleted user %s.")).c_str(),
        targetUser->getUserName().c_str());
    bot->logAdminMessage("%s (%s) DELUSER %s", theClient->getAccount().c_str(),
    ...
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::error_deleting_user,
                                 std::string("Error deleting user %s."))
                    .c_str(),
                st[1].c_str());
  }
