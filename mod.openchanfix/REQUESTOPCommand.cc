...
namespace gnuworld {
namespace cf {
/* 20110830 - Modified the REQUESTOP command to support the new CFV that was passed for criteria on
 * Undernet A REQUESTOP succeeds if the following criteria are met:
 ...
 * 4. There is nobody opped on the channel with a 'first opped date' within 3 days of the oldest
 * first opped date.
...
void REQUESTOPCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (!bot->doChanFix()) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::canfix_manual_fix_disabled,
                                 std::string("Sorry, I cant fix channels at the moment."))
                    .c_str());
    return;
  }
  ...
  if (bot->getState() != chanfix::RUN) {
    bot->SendTo(
        theClient,
        bot->getResponse(
               theUser, language::not_enough_servers_non_oper,
               std::string(
                   "Sorry, chanfix cannot fix channels at the moment, please try again soon."))
            .c_str());
    return;
  }
  ...
  for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
    ...
    // bot->SendTo(theClient, "Current Op: %s -- You: %s -- pScore: %d",
    // curOp->getAccount().c_str(), theClient->getAccount().c_str(), pScore);
  ...
  if (!isRequesterScoreHighEnough) {
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::score_not_high_enough,
                         std::string("Your score for %s is not high enough to issue a fix."))
            .c_str(),
        netChan->getName().c_str());
    return;
  }
  ...
  for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
    ...
    if (inChan && pScore >= 2000) {
      ...
      // Are they opped?
      if (nickName[0] == '@') {
        bot->SendTo(theClient, "Sorry, there is somebody currently opped on %s with a high score.",
                    st[1].c_str());
        return;
      }
    }
  ...
  for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
    ...
    if (inChan && pScore >= requesterScore) {
      ...
      if (nickName[0] == '@') {
        bot->SendTo(theClient,
                    "Sorry, there is somebody currently opped on %s with a higher score than you.",
                    st[1].c_str());
        return;
      }
    }
  ...
  // Is there anyone opped on the channel with a 'first opped date' within 3 days of the oldest
  // first opped date? First, get the oldest first opped TS and add 3 days to it
  ...
  // Got the first opped TS, now we make sure there's not an op with a first_opped_ts within 3 days
  // of this
  for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
    curOp = *opPtr;
    // bot->SendTo(theClient, "curOp: %s Time First Opped: %d ts_firstOpped: %d",
    // curOp->getAccount().c_str(), curOp->getTimeFirstOpped(), ts_firstOpped);
    inChan = bot->accountIsOnChan(st[1], curOp->getAccount());
    if (inChan) {
      nickName = bot->getChanNickName(st[1], curOp->getAccount());
      if (nickName[0] == '@' && !curOp->isOldestOp() &&
          curOp->getTimeFirstOpped() <= ts_firstOpped) {
        bot->SendTo(theClient, "Sorry, there is somebody opped on %s that has been opped longer.",
                    st[1].c_str());
        return;
      }
    }
  ...
  for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
    ...
    if (string_lower(curOp->getAccount()) == string_lower(theClient->getAccount()) &&
        opCount > 10) {
      bot->SendTo(theClient, "Sorry, you are not within the top 10 channel ops in %s.",
                  st[1].c_str());
      return;
    }
  }
  ...
  if (bot->isBeingChanFixed(netChan)) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::already_being_man_fixed,
                                 std::string("The channel %s is already being fixed."))
                    .c_str(),
                netChan->getName().c_str());
    return;
  }
  ...
  if (mintime < bot->getMinRequestOpTime()) {
    ...
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::requestop_must_wait,
                                 std::string("You must wait %s seconds before %s can be fixed "
                                             "again with this command, or contact a help channel."))
                    .c_str(),
  ...
  if (bot->isBeingAutoFixed(netChan)) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::channel_being_auto_fixed_noper,
                                 std::string("The channel %s is being automatically fixed. You "
                                             "cannot issue another fix at the moment."))
                    .c_str(),
                netChan->getName().c_str());
    return;
  }
  ...
  if ((bot->isTempBlocked(theChan->getChannel())) || (theChan->getFlag(sqlChannel::F_BLOCKED))) {
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::channel_blocked_non_oper,
                                 std::string("The channel %s cannot be fixed, please contact a "
                                             "help channel for further information."))
                    .c_str(),
  ...
  bot->SendTo(
      theClient,
      bot->getResponse(theUser, language::requestop_ack, std::string("Fixing %s, please wait."))
          .c_str(),
      netChan->getName().c_str());
