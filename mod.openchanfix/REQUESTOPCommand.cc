...
void REQUESTOPCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
    ...
    for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
        ...
        if (inChan && pScore >= 2000) {
            ...
            if (nickName[0] == '@') {
                bot->SendTo(theClient,
                            "Sorry, there is somebody currently opped on %s with a high score.",
                            st[1].c_str());
                return;
            }
        }
    ...
    // Got the first opped TS, now we make sure there's not an op with a first_opped_ts within 3
    // days of this
    for (chanfix::chanOpsType::iterator opPtr = myOps.begin(); opPtr != myOps.end(); opPtr++) {
        curOp = *opPtr;
        ...
        if (inChan) {
            ...
            if (nickName[0] == '@' && !curOp->isOldestOp() &&
                ...
                bot->SendTo(theClient,
                            "Sorry, there is somebody opped on %s that has been opped longer.",
                            st[1].c_str());
                return;
            }
        }
    ...
    if (mintime < bot->getMinRequestOpTime()) {
        ...
        bot->SendTo(
            ...
            bot->getResponse(theUser, language::requestop_must_wait,
                             std::string("You must wait %s seconds before %s can be fixed again "
                                         "with this command, or contact a help channel."))
                .c_str(),
