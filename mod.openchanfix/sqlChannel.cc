...
sqlChannel::sqlChannel(sqlManager* _myManager)
    : id(0), channel(), user_name(), last(0), start(0), maxScore(0), modesRemoved(false), flags(0),
      inSQL(false) {
...
bool sqlChannel::Delete(dbHandle* cacheCon) {
  ...
  if (!cacheCon->Exec(deleteString.str())) {
    elog << "sqlChannel::Delete> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
    retval = false;
  } else
    retval = true;
...
bool sqlChannel::commit(dbHandle* cacheCon) {
  ...
  if (!cacheCon->Exec(chanCommit.str())) {
    elog << "sqlChannel::commit> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
    retval = false;
  } else
    retval = true;
...
void sqlChannel::addNote(dbHandle* cacheCon, unsigned short eventType, iClient* theUser,
  ...
  theLog << "INSERT INTO notes (ts, channelID, user_name, event, message) "
         ...
         << ", " << id << ", '" << theUser->getAccount() << "', " << eventType << ", "
         << "'" << escapeSQLChars(theMessage) << "')";

  if (!cacheCon->Exec(theLog.str())) {
    elog << "sqlChannel::addNote> Something went wrong: " << cacheCon->ErrorMessage() << std::endl;
  }
...
const std::string sqlChannel::getLastNote(dbHandle* cacheCon, unsigned short eventType,
                                          time_t& eventTime) {
  std::string retval;
  ...
  queryString << "SELECT message,ts"
              << " FROM notes WHERE channelID = " << id << " AND event = " << eventType
...
bool sqlChannel::deleteNote(dbHandle* cacheCon, unsigned int messageId) {
  ...
  /* Create the DELETE statement */
  std::stringstream deleteString;
  deleteString << "DELETE FROM notes WHERE channelID = " << id << " AND id = " << messageId;

  if (!cacheCon->Exec(deleteString.str())) {
    elog << "sqlChannel::deleteNote> Something went wrong: " << cacheCon->ErrorMessage()
         << std::endl;
    retval = false;
  } else
    retval = true;
...
bool sqlChannel::deleteOldestNote(dbHandle* cacheCon) {
  ...
  selectString << "SELECT id FROM notes WHERE channelID = " << id << " ORDER BY ts ASC LIMIT 1";
  ...
  if (cacheCon->Exec(selectString.str(), true)) {
    if (cacheCon->Tuples() > 0) {
      ...
      std::stringstream deleteString;
      deleteString << "DELETE FROM notes WHERE id = " << note_id;
...
bool sqlChannel::deleteAllNotes(dbHandle* cacheCon) {
  ...
  /* Create the DELETE statement */
  std::stringstream deleteString;
  deleteString << "DELETE FROM notes WHERE channelID = " << id;

  if (!cacheCon->Exec(deleteString.str())) {
    elog << "sqlChannel::deleteAllNotes> Something went wrong: " << cacheCon->ErrorMessage()
         << std::endl;
    retval = false;
  } else
    retval = true;
...
size_t sqlChannel::countNotes(dbHandle* cacheCon, unsigned short eventType) {
  ...
  /* Count the notes */
  std::stringstream queryString;
  queryString << "SELECT count(id) FROM notes WHERE channelID = " << id;
  if (eventType) {
    queryString << " AND event = " << eventType;
  }
