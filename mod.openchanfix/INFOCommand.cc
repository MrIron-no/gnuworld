...
void INFOCommand::Exec(iClient* theClient, sqlcfUser* theUser, const std::string& Message) {
  ...
  if (!theChan) {
    if (isBlocked) {
      bot->SendTo(
          theClient,
          bot->getResponse(theUser, language::temporarily_blocked,
                           std::string("%s is temporarily blocked. (Use OVERRIDE to bypass)"))
              .c_str(),
          st[1].c_str());
      return;
    }
    ...
    bot->SendTo(theClient,
                bot->getResponse(theUser, language::no_info_for_chan,
                                 std::string("No information on %s in the database."))
                    .c_str(),
                st[1].c_str());
    return;
  }
  ...
  bot->SendTo(theClient,
              bot->getResponse(theUser, language::information_on, std::string("Information on %s:"))
                  .c_str(),
  ...
  if (netChan && bot->isBeingFixed(netChan)) {
    if (bot->isBeingChanFixed(netChan))
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::info_chan_being_fixed,
                                   std::string("%s is being chanfixed."))
                      .c_str(),
                  theChan->getChannel().c_str());
    if (bot->isBeingAutoFixed(netChan))
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::info_chan_being_autofixed,
                                   std::string("%s is being autofixed."))
                      .c_str(),
                  theChan->getChannel().c_str());

    if (theChan->getFixStart() > 0)
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::info_fix_started,
                                   std::string("Current fix has been running for %s"))
                      .c_str(),
                  prettyDuration(theChan->getFixStart()).c_str());
    else
      bot->SendTo(theClient,
                  bot->getResponse(theUser, language::info_fix_waiting,
                                   std::string("Current fix is on hold (waiting for ops to join)"))
                      .c_str());
  } else {
    ...
    bot->SendTo(theClient, "Last fix: %s (%s ago)", dateTimeOfLastFix.c_str(),
                prettyDuration(lastfix).c_str());
  }
  ...
  if (!theChan->useSQL()) {
    ...
    bot->SendTo(theClient, bot->getResponse(theUser, language::end_of_information,
                                            std::string("End of information."))
                               .c_str());
    return;
  }
  ...
  allNotesQuery << "SELECT notes.id, notes.ts, notes.user_name, notes.event, notes.message "
                << "FROM notes "
                << "WHERE notes.channelID = " << theChan->getID() << " ORDER BY notes.ts DESC";

  if (!cacheCon->Exec(allNotesQuery.str(), true)) {
    elog << "INFOCommand> SQL Error: " << cacheCon->ErrorMessage() << std::endl;
  ...
  if (noteCount > 0) {
    bot->SendTo(
        theClient,
        bot->getResponse(theUser, language::info_notes_count, std::string("Notes (%d):")).c_str(),
        noteCount);
    ...
    for (unsigned int i = 0; i < noteCount; i++) {
      ...
      bot->SendTo(
          theClient,
          bot->getResponse(theUser, language::info_notes, std::string("[%d:%s] %s \002%s\002%s%s"))
              .c_str(),
          note_id, from.c_str(), prettyTime(when).c_str(), bot->getEventName(event).c_str(),
